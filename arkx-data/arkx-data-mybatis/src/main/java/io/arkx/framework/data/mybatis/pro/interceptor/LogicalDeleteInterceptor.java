...
public class LogicalDeleteInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (props.getLogicalDeleteType() == LogicalDeleteType.BACKUP) {
			 ...
			 */
			String selectSql = new StringJoiner(" ").add("SELECT")
				.add("*")
				.add("FROM")
				.add(tableName)
				.add("WHERE")
				.add(delete.getWhere().toString())
				.toString();
			...
			MappedStatement m = new Builder(config, "com.[plugin]mybatis_pro_backup._inner_select",
					new StaticSqlSource(config, selectSql), SqlCommandType.SELECT)
				.build();
			...
			if (!CollectionUtils.isEmpty(result)) {
				String insert = new StringJoiner(" ").add("INSERT INTO")
					...
					.add("(table_name, data, delete_time) VALUES (?, ?, ?)")
					.toString();
