...
public class LimitColumnInterceptor implements Interceptor, ApplicationListener<ContextRefreshedEvent> {
    ...
    private String getCols(String id, BoundSql boundSql, Map<String, String> alias) {
         ...
         * 对于创建BoundSql有两种方式，一种是直接创建，一种是创建SqlSource（比如StaticSqlSource）然后调用getBoundSql
         * 2.2. 这里不能直接使用第二种，因为如果是DyanamicSqlSource创建出来的SqlSource是带有AddtionalParameter的，
         ...
         * ParameterMapping>来自sqlSource（DynamicSqlSource，
         * RawSqlSource调用getBoundSql是相同的原理） //
