...
import static io.arkx.framework.data.mybatis.pro.core.consts.MapperLabel.*;
import static io.arkx.framework.data.mybatis.pro.core.util.ClassUtil.getMethodName2ReturnType;
import static io.arkx.framework.data.mybatis.pro.core.util.ClassUtil.getSpecialMethods;
import static io.arkx.framework.data.mybatis.pro.core.util.DocumentUtil.*;
import static io.arkx.framework.data.mybatis.pro.core.util.SqlUtil.toLine;
import static io.arkx.framework.data.mybatis.pro.core.util.XmlUtil.getNamespaceFromXmlResource;
...
public class MyBatisProUtil {
	...
	public static Resource[] buildMyBatisPro(Resource[] xmlResources, Set<String> mapperPackages) {
		...
		Set<Class<?>> presentXmlMappers = stream(ofNullable(xmlResources).orElse(new Resource[0]))
			...
			.collect(toSet());
		Set<Class<?>> allMappers = ofNullable(mapperPackages).orElseGet(HashSet::new)
			.stream()
			.map(ClassPathUtil::resolvePackage)
			.flatMap(Arrays::stream)
			.map(mapperPkgName -> scanPackageBySuper(mapperPkgName, Mapper.class))
			.flatMap(Set::stream)
			.collect(toSet());
	...
	private static Set<String> validateDuplicateMethods(Class<?> mapperCls, Resource resource) {
		...
		Set<String> xmlMethodNames = methods.stream()
			.map(node -> node.getStringAttribute(ID.getCode()))
			.collect(toSet());
