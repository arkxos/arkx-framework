...
//		assertThat(closureService.queryDescendantIds(parent.getId(), "dept", IdType.LONG)).isEmpty();
	}
	...
	private void updateParentAndVerify(Dept dept, String newParentId) {
		...
		assertThat(deptRepository.findById(dept.getId()))
			.get()
			.extracting(Dept::getParentId)
	...
	public List<Dept> getDescendants(String deptId) {
		// 1. 获取dept元数据
//		BizTableMeta meta = businessTableMetaJdbcRepository.findByBusinessTable("dept")
//				.orElseThrow(() -> new RuntimeException("dept元数据未配置"));
		...
		meta.setIdType(IdType.LONG);
				// 2. 查询后代ID列表（闭包服务）
		List<String> descendantIds = closureService.queryDescendantIds(
				deptId,                // nodeId
				meta
		);

		// 3. 查询业务表节点（业务侧）
		String sql = "SELECT * FROM TEST_DEPT WHERE id IN (" + String.join(", ", Collections.nCopies(descendantIds.size(), "?")) + ")";
