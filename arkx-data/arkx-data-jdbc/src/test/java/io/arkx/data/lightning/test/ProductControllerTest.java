...
public class ProductControllerTest {
	...
	void createProduct_ShouldReturnCreated() throws Exception {
		...
		mockMvc
			.perform(post("/api/products").contentType(MediaType.APPLICATION_JSON)
	...
	void updateProduct_ExistingId_ShouldReturnUpdated() throws Exception {
		...
		mockMvc
			.perform(put("/api/products/1").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(inputDTO)))
			.andExpect(status().isOk())
	...
	void getAllProducts_WithPagination_ShouldReturnPage() throws Exception {
		...
		mockMvc
			.perform(get("/api/products").param("page", "0").param("size", "2").contentType(MediaType.APPLICATION_JSON))
			.andExpect(status().isOk())
