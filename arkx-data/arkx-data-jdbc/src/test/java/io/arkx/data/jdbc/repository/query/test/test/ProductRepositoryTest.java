...
public class ProductRepositoryTest {
	...
	void findBySqlToy_WithPagination_ShouldReturnPage() {
		// 插入测试数据
		productRepository.saveAll(List.of(
				Product.builder().name("Apple").category("Fruit").price(new BigDecimal("0")).build(),
		...
		// 查询名称含"App"且类别为"Fruit"的第一页（每页2条）
		Page<Product> result = productRepository.findBySqlToyQuery(
				"App", "Fruit", PageRequest.of(0, 1));
	...
	void findByPriceRangeAndActive_WithPagination_ShouldReturnPage() {
		// 插入测试数据
		productRepository.saveAll(List.of(
				Product.builder().price(new BigDecimal("100.0")).active(true).build(),
