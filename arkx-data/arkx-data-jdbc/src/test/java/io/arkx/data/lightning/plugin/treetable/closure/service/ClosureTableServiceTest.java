 ...
 * 闭包表功能集成测试类
 *
 * 使用H2内存数据库进行真实场景测试，全面验证闭包表服务的核心功能：
 * 1. 公共闭包表和独立闭包表的CRUD操作
 * 2. 多层级关系维护
 * 3. 节点移动和子树删除操作
 * 4. 后代查询功能
 *
...
public class ClosureTableServiceTest {
	...
	void testInsertRootNode_CommonTable() {
		...
		List<Map<String, Object>> relations = jdbcTemplate.queryForList(
				"SELECT * FROM ark_tree_closure_long WHERE biz_table='TEST_DEPT' and descendant_id = 1"
		);
	 ...
	 * 验证点：
	 * 1. 子节点生成自引用关系
	 * 2. 建立与父节点的直接关系(depth=1)
	 * 3. 建立与祖先节点的间接关系(depth>1)
	 * 4. 所有关系深度计算正确
	 */
	@Test
	void testInsertChildNode_CommonTable() {
		...
		// 验证闭包关系
		List<Map<String, Object>> relations = jdbcTemplate.queryForList(
				"SELECT * FROM ark_tree_closure_long " +
					"WHERE biz_table='TEST_DEPT' " +
	...
	void testInsertMultiLevelNode_CommonTable() {
		...
		List<Map<String, Object>> relations = jdbcTemplate.queryForList(
				"SELECT * FROM ark_tree_closure_long " +
					"WHERE biz_table='TEST_DEPT' and descendant_id = 3 " +
					"ORDER BY depth ASC, ancestor_id ASC, descendant_id ASC"
		);
		...
		assertTrue(relations.stream().anyMatch(r ->
				r.get("ancestor_id").equals(3L) &&
						r.get("descendant_id").equals(3L) &&
						r.get("depth").equals(0)), "缺少节点3的自引用关系");

		assertTrue(relations.stream().anyMatch(r ->
				r.get("ancestor_id").equals(2L) &&
						r.get("descendant_id").equals(3L) &&
						r.get("depth").equals(1)), "缺少节点2->3的直接关系");

		assertTrue(relations.stream().anyMatch(r ->
				r.get("ancestor_id").equals(1L) &&
						r.get("descendant_id").equals(3L) &&
						r.get("depth").equals(2)), "缺少节点1->3的间接关系");
	}
	...
	void testUpdateClosureRelations() {
		...
		// 验证初始关系总数
		List<Map<String, Object>> allRelations = jdbcTemplate.queryForList(
				"SELECT * FROM ark_tree_closure_long WHERE biz_table='TEST_DEPT'"
		);
		...
		// 验证整个闭包表的关系
		allRelations = jdbcTemplate.queryForList(
				"SELECT * FROM ark_tree_closure_long " +
						"WHERE biz_table='TEST_DEPT' " +
						"ORDER BY depth ASC, ancestor_id ASC, descendant_id ASC"
		);
	...
	// 辅助方法：验证关系存在且深度匹配
	private void assertRelationExists(List<Map<String, Object>> relations,
									  long expectedAncestor,
		...
		for (Map<String, Object> relation : relations) {
			...
			if (ancestor == expectedAncestor &&
					descendant == expectedDescendant &&
					depth == expectedDepth) {
				return;
			}
		}
		fail(String.format("未找到关系: (%d->%d, depth=%d)",
				expectedAncestor, expectedDescendant, expectedDepth));
	}

	// 辅助方法：验证关系不存在（任意深度）
	private void assertRelationNotExists(List<Map<String, Object>> relations,
										 long expectedAncestor,
										 long expectedDescendant) {
		for (Map<String, Object> relation : relations) {
			long ancestor = (long) relation.get("ancestor_id");
	...
	private void assertRelationNotExists(List<Map<String, Object>> relations,
		...
		for (Map<String, Object> relation : relations) {
			...
			if (ancestor == expectedAncestor &&
					descendant == expectedDescendant &&
					depth == expectedDepth) {
				fail(String.format("不应存在关系: (%d->%d, depth=%d)",
						expectedAncestor, expectedDescendant, expectedDepth));
			}
		}
	}
	...
	void testIndependentClosureTableOperations() {
		...
		// 更新关系
//		closureTableService.updateClosureRelations("grandchild", "child2", independentMeta);

		// 验证新关系
		List<Map<String, Object>> relations = jdbcTemplate.queryForList(
				"SELECT * FROM menu_closure WHERE descendant_id = 'grandchild'"
		);

		// 验证关系内容
		assertTrue(relations.stream().anyMatch(r ->
				r.get("ancestor_id").equals("child2") &&
						r.get("descendant_id").equals("grandchild")), "缺少child2->grandchild的关系");

		// 删除节点
//		closureTableService.deleteClosureRelations("child1", independentMeta);

		// 验证删除
		List<Map<String, Object>> deletedRelations = jdbcTemplate.queryForList(
				"SELECT * FROM menu_closure WHERE descendant_id = 'child1' OR ancestor_id = 'child1'"
		);
...
//		jdbcTemplate.update("INSERT INTO dept (id, name, parent_id) VALUES (2, '研发部', 1)");
//		jdbcTemplate.update("INSERT INTO dept (id, name, parent_id) VALUES (3, '市场部', 1)");
//		jdbcTemplate.update("INSERT INTO dept (id, name, parent_id) VALUES (4, '前端组', 2)");
	}
...
//		jdbcTemplate.update("INSERT INTO menu (id, name, parent_id) VALUES ('file', '文件', 'root')");
//		jdbcTemplate.update("INSERT INTO menu (id, name, parent_id) VALUES ('edit', '编辑', 'root')");
//		jdbcTemplate.update("INSERT INTO menu (id, name, parent_id) VALUES ('new', '新建', 'file')");
	}
