 ...
 * 闭包表功能集成测试类
 *
 * 使用H2内存数据库进行真实场景测试，全面验证闭包表服务的核心功能： 1. 公共闭包表和独立闭包表的CRUD操作 2. 多层级关系维护 3. 节点移动和子树删除操作 4.
 * 后代查询功能
 *
 * 测试覆盖以下关键场景： - 根节点插入（无父节点） - 子节点插入（直接父子关系） - 多层级节点插入（祖先关系） - 节点移动（更新关系） - 子树删除（级联删除） -
...
public class ClosureTableServiceTest {
	 ...
	 * 测试公共闭包表子节点插入
	 *
	 * 验证点： 1. 子节点生成自引用关系 2. 建立与父节点的直接关系(depth=1) 3. 建立与祖先节点的间接关系(depth>1) 4. 所有关系深度计算正确
	 */
	@Test
	...
	void testInsertMultiLevelNode_CommonTable() {
		...
		// 验证闭包关系
		List<Map<String, Object>> relations = jdbcTemplate
			.queryForList("SELECT * FROM ark_tree_closure_long " + "WHERE biz_table='TEST_DEPT' and descendant_id = 3 "
					+ "ORDER BY depth ASC, ancestor_id ASC, descendant_id ASC");
		...
		assertTrue(relations.stream()
			.anyMatch(r -> r.get("ancestor_id").equals(3L) && r.get("descendant_id").equals(3L)
					&& r.get("depth").equals(0)),
				"缺少节点3的自引用关系");

		assertTrue(relations.stream()
			.anyMatch(r -> r.get("ancestor_id").equals(2L) && r.get("descendant_id").equals(3L)
					&& r.get("depth").equals(1)),
				"缺少节点2->3的直接关系");

		assertTrue(relations.stream()
			.anyMatch(r -> r.get("ancestor_id").equals(1L) && r.get("descendant_id").equals(3L)
					&& r.get("depth").equals(2)),
				"缺少节点1->3的间接关系");
	}
	...
	void testIndependentClosureTableOperations() {
		...
		// 验证关系内容
		assertTrue(relations.stream()
			.anyMatch(r -> r.get("ancestor_id").equals("child2") && r.get("descendant_id").equals("grandchild")),
