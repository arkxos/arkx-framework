...
public class ClosureTableServiceImpl implements ClosureTableService {
    ...
    public ClosureTableServiceImpl(JdbcTemplate jdbcTemplate,
     ...
     * 插入直接子节点关系（depth=1）
     */
    private <T> void insertDirectChildRelation(String closureTable, T nodeId, T ancestorId, BizTableMeta meta, IdType idType) {
        ClosureTableSqlProvider provider = sqlProviderFactory.getProvider();
        String insertSql = meta.isUseIndependent() ?
                provider.insertIndependentClosureSql(closureTable, idType) :
                provider.insertCommonClosureSql(closureTable, idType);
        ...
        if (meta.isUseIndependent()) {
            jdbcTemplate.update(insertSql, ancestorId, nodeId); // 独立闭包表：ancestor=parentId, descendant=nodeId
        } else {
            jdbcTemplate.update(insertSql, ancestorId, nodeId, meta.getBizTable()); // 公共闭包表：ancestor=parentId, descendant=nodeId, biz_table=业务表名
        }
    }
     ...
     */
    private <T> void insertAncestorRelations(String closureTable, T nodeId, T ancestorId, BizTableMeta meta, IdType idType, int currentDepth) {
        ClosureTableSqlProvider provider = sqlProviderFactory.getProvider();
        ...
        if (meta.isUseIndependent()) {
            jdbcTemplate.update(insertSql, ancestorId, nodeId); // 独立闭包表：ancestor=ancestorId, descendant=nodeId
        } else {
     ...
     */
    private <T> List<T> queryDescendantIdsFromClosureTable(String closureTable, T nodeId, BizTableMeta meta, IdType idType) {
        ClosureTableSqlProvider provider = sqlProviderFactory.getProvider();
     ...
     *
     * @param nodeId   当前节点ID
     * @param parentId 新的父节点ID（null表示根节点）
     * @param meta     业务表名
     */
    @Transactional
     ...
     *
     * @param nodeId 起始节点ID（null表示查询整棵树）
     * @param meta   业务表元数据
     * @return 树节点列表（包含子节点嵌套结构）
     */
    @Override
    public <T, R extends TreeNodeData<T>> Treex<T, R> queryTreeData(T nodeId, BizTableMeta meta,
                                                                    Class<R> targetClass) {
        // 1. 解析闭包表名
        String closureTable = resolveClosureTableName(meta);
     ...
     * 执行树查询并返回带层级信息的平面列表
     */
	private <T, R> List<R> executeTreeQuery(String sql,
											T nodeId,
											BizTableMeta meta,
											Class<R> targetClass) {
