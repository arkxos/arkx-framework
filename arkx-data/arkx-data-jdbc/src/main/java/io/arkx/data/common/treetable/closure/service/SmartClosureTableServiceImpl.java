...
import io.arkx.data.common.treetable.closure.sql.ClosureTableSqlProvider;
import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;
import java.util.List;

public class SmartClosureTableServiceImpl<ID> implements SmartClosureTableService<ID> {
	 ...
	 */
	@Transactional
	public void insertClosureRelations(ID nodeId, ID parentId, String businessTable, IdType idType) {
		// 1. 获取业务表元数据（校验闭包表类型）
	 ...
	 */
	public List<ID> queryDescendantIds(ID nodeId, String businessTable, IdType idType) {
		// 1. 获取业务表元数据（校验闭包表类型）
		BusinessTableMeta meta = metaRepository.findByBusinessTable(businessTable)
				.orElseThrow(() -> new IllegalArgumentException("业务表未配置: " + businessTable));
	...
	private String resolveClosureTableName(BusinessTableMeta meta) {
		if (meta.isUseIndependent()) {
			return meta.getBusinessTable() + "_closure";
		} else {
			return "ark_common_closure_" + meta.getIdType().name().toLowerCase();
		}
	}
	 ...
	 * 插入闭包表自引用关系（depth=0）
	 */
	private void insertSelfClosureRelation(String closureTable, ID nodeId, BusinessTableMeta meta, IdType idType) {
		...
		if (meta.isUseIndependent()) {
			...
			jdbcTemplate.update(insertSql, nodeId, nodeId, meta.getBusinessTable()); // 公共闭包表：ancestor=nodeId, descendant=nodeId, biz_table=业务表名
		}
	}
	 ...
	 * 插入直接子节点关系（depth=1）
	 */
	private void insertDirectChildRelation(String closureTable, ID nodeId, ID parentId, BusinessTableMeta meta, IdType idType) {
		ClosureTableSqlProvider provider = sqlProviderFactory.getProvider();
		String insertSql = meta.isUseIndependent() ?
				provider.insertIndependentClosureSql(closureTable, idType) :
				provider.insertCommonClosureSql(closureTable, idType);
		...
		if (meta.isUseIndependent()) {
			...
			jdbcTemplate.update(insertSql, parentId, nodeId, meta.getBusinessTable()); // 公共闭包表：ancestor=parentId, descendant=nodeId, biz_table=业务表名
		}
	}
	 ...
	 */
	private void insertAncestorRelations(String closureTable, ID nodeId, ID ancestorId, BusinessTableMeta meta, IdType idType, int currentDepth) {
		ClosureTableSqlProvider provider = sqlProviderFactory.getProvider();
		...
		if (meta.isUseIndependent()) {
			...
			jdbcTemplate.update(insertSql, ancestorId, nodeId, meta.getBusinessTable()); // 公共闭包表：ancestor=ancestorId, descendant=nodeId, biz_table=业务表名
		}

		// 2. 递归查询上一级祖先（ancestorId的父节点）
		ID parentOfAncestor = getParentIdFromClosureTable(closureTable, ancestorId, meta, idType);
	 ...
	 */
	private ID getParentIdFromClosureTable(String closureTable, ID ancestorId, BusinessTableMeta meta, IdType idType) {
		ClosureTableSqlProvider provider = sqlProviderFactory.getProvider();
		String querySql =
		...
		Object[] params = meta.isUseIndependent() ?
				...
				new Object[]{ancestorId, meta.getBusinessTable()};

		return jdbcTemplate.queryForObject(querySql, (rs, rowNum) -> (ID)rs.getObject("parent_id"), params);
	}

	/**
	 * 从闭包表查询后代ID列表（动态适配ID类型）
	 */
	private List<ID> queryDescendantIdsFromClosureTable(String closureTable, ID nodeId, BusinessTableMeta meta, IdType idType) {
		ClosureTableSqlProvider provider = sqlProviderFactory.getProvider();
		...
		Object[] params = meta.isUseIndependent() ?
				new Object[]{nodeId} :
				new Object[]{nodeId, meta.getBusinessTable()};

		return jdbcTemplate.query(querySql, (rs, rowNum) -> (ID)rs.getObject("descendant_id"), params);
	}
}