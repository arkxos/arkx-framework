 ...
 * @Version V1.0
 */
public class BaseService<T, ID extends Serializable, R extends BaseJpaRepository<T, ID>>
		implements IBaseService<T, ID, R> {
	...
	public List<T> findAllByCriteria(CriteriaQueryWrapper<T> criteria) {
		return entityRepository.findAll(
				(root, criteriaQuery, criteriaBuilder) -> QueryHelp.buildPredicate(root, criteria, criteriaBuilder));
	}
	...
	public void deleteByCriteria(CriteriaQueryWrapper<T> criteria) {
		List<T> data = entityRepository.findAll(
				(root, criteriaQuery, criteriaBuilder) -> QueryHelp.buildPredicate(root, criteria, criteriaBuilder));
		for (T entity : data) {
			delete(entity);
		}
	...
	public List list(T t, String... properties) {
		return entityRepository.findAll(Example.of(t, ExampleMatcher.matching()),
