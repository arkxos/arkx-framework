...
public class BeanTransformerAdapter<T> implements ResultTransformer {
	...
	/**
	 * Create a new BeanPropertyRowMapper, accepting unpopulated properties in the target
	 * bean.
	 * <p>
	 * Consider using the {@link #newInstance} factory method instead, which allows for
	 * specifying the mapped type once only.
	 ...
	 * properties have been mapped from corresponding database fields
	 */
	...
	/**
	 * Static factory method to create a new BeanPropertyRowMapper (with the mapped class
	 * specified only once).
	 * @param mappedClass the class that each row should be mapped to
	 * @param <T> class type
	 * @return instance
	...
	/**
	 * Convert a name in camelCase to an underscored name in lower case. Any upper case
	 * letters are converted to lower case with a preceding underscore.
	 ...
	 * {@link JdbcUtils#getResultSetValue(java.sql.ResultSet, int, Class)}. Subclasses may
	 * override this to check specific value types upfront, or to post-process values
