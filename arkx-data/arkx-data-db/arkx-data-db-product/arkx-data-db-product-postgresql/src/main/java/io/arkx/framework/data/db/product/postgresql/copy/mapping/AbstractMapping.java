package io.arkx.framework.data.db.product.postgresql.copy.mapping;
...
import io.arkx.framework.data.db.product.postgresql.copy.function.ToBooleanFunction;
import io.arkx.framework.data.db.product.postgresql.copy.function.ToFloatFunction;
import io.arkx.framework.data.db.product.postgresql.copy.model.ColumnDefinition;
import io.arkx.framework.data.db.product.postgresql.copy.model.TableDefinition;
...
public abstract class AbstractMapping<TEntity> {
    ...
    protected <TElementType, TCollectionType extends Collection<TElementType>> void mapCollection(String columnName,
            DataType dataType, Function<TEntity, TCollectionType> propertyGetter) {
    ...
    public String getCopyCommand() {
        String commaSeparatedColumns = columns.stream().map(x -> x.getColumnName())
                .map(x -> usePostgresQuoting ? PostgreSqlUtils.quoteIdentifier(x) : x)
                .collect(Collectors.joining(", "));

        return "COPY %1$s(%2$s) FROM STDIN BINARY".formatted(table.GetFullyQualifiedTableName(usePostgresQuoting),
