    ...
    for (int i = 0; i < fieldNames.size(); ++i) {
      ...
      if (!this.columnType.containsKey(s)) {
        throw new RuntimeException(
            "表%s.%s 中不存在字段名为%s的字段，请检查参数传入!".formatted(schemaName, tableName, s));
      }
    ...
    try (Connection connection = getDataSource().getConnection();
      ...
      for (Object[] objects : recordValues) {
        if (fieldNames.size() != objects.length) {
          throw new RuntimeException(
              "传入的参数有误，字段列数%d与记录中的值个数%d不相符合".formatted(fieldNames.size(), objects.length));
        }
        ...
        switch (fieldType) {
          ...
          case Types.LONGNVARCHAR:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表[%s.%s]的字段名[%s]数据类型转换错误，应该为java.lang.String/java.sql.Clob，而实际的数据类型为%s".formatted(
          ...
          case Types.NCLOB:
            if (null == fieldValue) {
              ...
              String val = ObjectCastUtils.castToString(fieldValue);
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转换错误，应该为java.lang.String/java.sql.Clob，而实际的数据类型为%s".formatted(
                    schemaName, tableName, fieldName, fieldValue.getClass().getName()));
              }
          ...
          case Types.TINYINT:
            if (null == fieldValue) {
              ...
              try {
                ...
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转错误，%s".formatted(
                    schemaName, tableName, fieldName, e.getMessage()));
              }

              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转错误，应该为java.lang.Byte，而实际的数据类型为%s".formatted(schemaName,
          ...
          case Types.SMALLINT:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转换错误，应该为java.lang.Short，而实际的数据类型为%s".formatted(schemaName,
                    tableName, fieldName, fieldValue.getClass().getName()));
              }
          ...
          case Types.INTEGER:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转换错误，应该为java.lang.Integer，而实际的数据类型为%s".formatted(
                    schemaName, tableName, fieldName, fieldValue.getClass().getName()));
              }
          ...
          case Types.BIGINT:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转换错误，应该为java.lang.Long，而实际的数据类型为%s".formatted(schemaName,
                    tableName, fieldName, fieldValue.getClass().getName()));
              }
          ...
          case Types.DECIMAL:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转换错误，应该为java.lang.Number，而实际的数据类型为%s".formatted(schemaName,
                    tableName, fieldName, fieldValue.getClass().getName()));
              }
          ...
          case Types.REAL:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转换错误，应该为java.lang.Float，而实际的数据类型为%s".formatted(schemaName,
                    tableName, fieldName, fieldValue.getClass().getName()));
              }
          ...
          case Types.DOUBLE:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转换错误，应该为java.lang.Double，而实际的数据类型为%s".formatted(schemaName,
                    tableName, fieldName, fieldValue.getClass().getName()));
              }
              row.setDouble(i, val);
            }
          ...
          case Types.BIT:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型错误，应该为java.lang.Boolean，而实际的数据类型为%s".formatted(schemaName,
                    tableName, fieldName, fieldValue.getClass().getName()));
              }
              if (Types.BIT == fieldType) {
          ...
          case Types.TIME:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转换错误，应该为java.sql.Time，而实际的数据类型为%s".formatted(schemaName,
                    tableName, fieldName, fieldValue.getClass().getName()));
              }
              row.setTime(i, val);
            }
          ...
          case Types.DATE:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型转换错误，应该为java.sql.Date，而实际的数据类型为%s".formatted(schemaName,
                    tableName, fieldName, fieldValue.getClass().getName()));
              }
              row.setDate(i, val);
            }
          ...
          case Types.TIMESTAMP:
            if (null == fieldValue) {
              ...
              if (null == val) {
                throw new RuntimeException("表名[%s.%s]的字段名[%s]数据类型错误，应该为java.sql.Timestamp，而实际的数据类型为%s".formatted(schemaName,
                    tableName, fieldName, fieldValue.getClass().getName()));
              }
          ...
          default:
            throw new RuntimeException(
                "不支持的数据库字段类型,表名[%s.%s] 字段名[%s].".formatted(schemaName,
                    tableName, fieldName));
        }
      }
    };
