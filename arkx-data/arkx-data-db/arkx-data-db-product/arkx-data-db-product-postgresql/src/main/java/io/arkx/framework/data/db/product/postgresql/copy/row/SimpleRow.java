package io.arkx.framework.data.db.product.postgresql.copy.row;
...
public class SimpleRow {
    ...
    public <TElementType, TCollectionType extends Collection<TElementType>> void setCollection(String columnName,
            DataType type, TCollectionType value) {
        final int ordinal = Objects.requireNonNull(lookup.get(columnName), columnName + " not found");

        setCollection(ordinal, type, value);
    }

    public <TElementType, TCollectionType extends Collection<TElementType>> void setCollection(int ordinal,
            DataType type, TCollectionType value) {
    ...
    public void setTextArray(String columnName, Collection<String> value) {
        Collection<String> values = value.stream().map(x -> nullCharacterHandler.apply(x)).collect(Collectors.toList());
