...
public class SybaseMetadataQueryProvider extends AbstractMetadataProvider {
    ...
    public List<TableDescription> queryTableList(Connection connection, String schemaName) {
        ...
        try (ResultSet tables = connection.getMetaData().getTables(connection.getCatalog(), schemaName, "%", types)) {
            while (tables.next()) {
    ...
    public List<String> queryTableColumnName(Connection connection, String schemaName, String tableName) {
        ...
        try (ResultSet rs = connection.getMetaData().getColumns(connection.getCatalog(), schemaName, tableName, null)) {
            while (rs.next()) {
    ...
    public List<String> queryTablePrimaryKeys(Connection connection, String schemaName, String tableName) {
        Set<String> ret = new LinkedHashSet<>();
        try (ResultSet primaryKeys = connection.getMetaData().getPrimaryKeys(connection.getCatalog(), schemaName,
                tableName)) {
    ...
    public List<ColumnDescription> queryTableColumnMeta(Connection connection, String schemaName, String tableName) {
        ...
        // 补充一下注释信息
        try (ResultSet columns = connection.getMetaData().getColumns(connection.getCatalog(), schemaName, tableName,
                null)) {
            while (columns.next()) {
    ...
    public String getTableDDL(Connection connection, String schemaName, String tableName) {
        ...
        return GenerateSqlUtils.getDDLCreateTableSQL(this, columnDescriptions, pks, schemaName, tableName, false);
    }
