...
public final class StarRocksUtils {
	...
	public void getHttpPort(DataSource dataSource) {
		...
		try {
			...
			List<FrontendEntity> leader = frontendEntities.stream()
				.filter(i -> i.getRole().equals("LEADER"))
	...
	private void sendData(String content) throws Exception {
		final String loadUrl = "http://%s:%s/api/%s/%s/_stream_load".formatted(this.host, this.httpPort, this.dbName,
				this.tbName);
		...
		try (CloseableHttpClient client = httpClientBuilder.build()) {
			...
			try (CloseableHttpResponse response = client.execute(put)) {
				...
				// statusCode 200 just indicates that starrocks be service is ok, not
				// stream
				// load
				// you should see the output content to find whether stream load is
				// success
				if (statusCode != 200) {
