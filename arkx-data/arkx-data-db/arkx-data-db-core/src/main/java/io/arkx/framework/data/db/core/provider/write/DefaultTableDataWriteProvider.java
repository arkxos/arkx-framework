...
public class DefaultTableDataWriteProvider
  ...
  public void prepareWrite(String schemaName, String tableName, List<String> fieldNames) {
    ...
    if (this.columnType.isEmpty()) {
      throw new RuntimeException(
          "获取表:%s.%s 的字段的元信息时失败. 请联系 DBA 核查该库、表信息.".formatted(
  ...
  public long write(List<String> rawFieldNames, List<Object[]> rawRecordValues) {
     ...
     * 执行单批次插入
     */
    private int executeBatch(PlatformTransactionManager tx, String sql,
                             List<Object[]> batch, int[] argTypes) {
        TransactionStatus status = tx.getTransaction(getDefaultTransactionDefinition());
        try {
            jdbcTemplate.batchUpdate(sql, batch, argTypes);
            tx.commit(status);
            if (log.isDebugEnabled()) {
                log.debug("Inserted batch: {} records, ~{} bytes",
                        batch.size(), ByteUtil.calculateBatchSize(batch));
            }
            return batch.size();
        } catch (Exception e) {
            tx.rollback(status);
