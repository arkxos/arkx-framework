...
public class ColumnMappingUtils {
	...
	public static void init() {
		// 缓存所有字段映射信息
		COLUMN_MAP = DataDumpCenter.columnMappingDAO_getAllColumnMapping()
			.get()
			.stream()
			.collect(
		...
		// 缓存所有字段映射附加信息
		COLUMN_PLUS_MAP = DataDumpCenter.columnPlusDAO_getAllColumnPlus()
			.get()
			.stream()
			.collect(Collectors.toMap(ColumnPlusEntity::getColumnMappingId, entity -> entity,
