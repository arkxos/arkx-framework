...
public abstract class AbstractMetadataProvider extends AbstractCommonProvider implements MetadataProvider {
		...
		try (ResultSet rs = connection.getMetaData().getIndexInfo(catalogName, schemaName, tableName, false, true)) {
			while (rs.next()) {
				...
				if (StringUtils.isNotBlank(indexName)) {
					indexTypesMap.putIfAbsent(indexName, (null != nonUnique)
							? (nonUnique ? TableIndexEnum.NORMAL : TableIndexEnum.UNIQUE) : TableIndexEnum.NORMAL);
	...
	protected ColumnDescription buildColumnDescription(ResultSetMetaData m, int i) throws SQLException {
		...
		if (0 != cd.getFieldType()) {
			...
			if (columnTypeName.contains("UNSIGNED")) {
				columnTypeName = columnTypeName.replace(" UNSIGNED", "");
			}
			cd.setFieldTypeName(columnTypeName);
