...
public class DefaultTableDataSynchronizeProvider
  ...
  @Override
  public void prepare(String schemaName, String tableName,
...
//        if (e instanceof java.sql.BatchUpdateException) {
          ...
          for (Object[] dataList : records) {
            try {
              i++;
              jdbcTemplate.update(this.insertStatementSql, new MyArgumentTypePreparedStatementSetter(fieldOrders, columnTypeNames, dataList, this.insertArgsType));
            } catch (Exception ex) {
              System.err.println("[" + i + "]Failed to insert by SQL: "+this.insertStatementSql+", value: +" +
...
//        }
  ...
  protected String getUpdatePrepareStatementSql(String schemaName, String tableName,
    ...
    List<String> uw = pks.stream()
            .map(pk -> "%s=?".formatted(quoteName(pk)))
            .collect(Collectors.toList());
