...
public class MultiFormatDateParser {
    ...
    private static final Pattern PATTERN2 =
            Pattern.compile("^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}[+-]\\d{1,2}$");

    // 格式1: "2023-01-30-09.25.10.000000"
    private static final DateTimeFormatter FORMAT1 =
            DateTimeFormatter.ofPattern("yyyy-MM-dd-HH.mm.ss.nnnnnn", Locale.ENGLISH);

    // 格式2: "2023-08-23 17:32:47+08"
    private static final DateTimeFormatter FORMAT2 =
            DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ssX", Locale.ENGLISH);
      ...
      /*  List<String> testDates = Arrays.asList(
        ...
        for (String date : testDates) {
            try {
                ...
                System.out.println("原始: " + date + " -> 转换后: " + parsedDate);
            } catch (IllegalArgumentException e) {
                System.out.println("处理失败: " + date + " - " + e.getMessage());
            }
        }*/
