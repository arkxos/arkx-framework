...
public class DataProcessor {
    ...
    public Object[] process(String schemaName, String tableName, List<String> columns, Object[] originalResult) {
        if (CollectionUtils.isEmpty(sourceColumnDescriptions)
            || CollectionUtils.isEmpty(targetColumnDescriptions)
            || ObjectUtils.isEmpty(originalResult)) { // originalResult 有可能为空，不知道是不是bug
        ...
        try {
            for (int i = 0; i < originalResult.length; i++) {
                int finalI = i;
                ColumnMappingEntity columnMappingEntity =
                    ColumnMappingUtils.getColumnMappingEntity(schemaName, tableName, columns.get(i));
                ...
                Optional.ofNullable(ColumnMappingUtils.getColumnPlusEntity(columnMappingEntity.getId()))
                    .map(plus -> {
                        if ("DICT_MAPPING".equals(plus.getHandlerCode())) { // 非级联字典或级联字典一级
                            ...
                            // 父级字段映射信息
                            ColumnMappingEntity columnMappingEntity1 =
                                ColumnMappingUtils.getColumnMappingEntity(schemaName, finalTableName, split[0]);
                            if (columnMappingEntity1 == null) {
                                return null;
                            }
                            // 父级字典映射附加信息（字典映射、默认值、以及子字典映射等）
                            ColumnPlusEntity columnPlus =
                                ColumnMappingUtils.getColumnPlusEntity(columnMappingEntity1.getId());
                            if (columnPlus == null) {
                                return null;
                            }
