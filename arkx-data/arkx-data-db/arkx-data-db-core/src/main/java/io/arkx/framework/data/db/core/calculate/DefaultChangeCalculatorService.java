...
public final class DefaultChangeCalculatorService implements RecordRowChangeCalculator {
  ...
  public void executeCalculate(TaskParamEntity task, RecordRowHandler handler) {
    ...
    List<String> fieldsMappedPrimaryKeyNew = fieldsPrimaryKeyNew.stream()
     ...
     fieldsAllColumnNew = newMd
            .queryTableColumnName(task.getNewSchemaName(), task.getNewTableName())
            .stream()
            .map(String::toLowerCase)
            .collect(Collectors.toList());
    // todo 暂时写死处理
//    if (fieldsAllColumnNew.contains("ark_shard_db_code") && fieldsAllColumnNew.contains("ark_sync_raw_id")) {
//        fieldsPrimaryKeyNew = Arrays.asList("ark_shard_db_code", "ark_sync_raw_id");
//        fieldsMappedPrimaryKeyNew = Arrays.asList("ark_shard_db_code", "ark_sync_raw_id");
//    }
...
//        }
      ...
      if (!isColumnCountSame) {
        ...
        for (int k = 1; k < metaData.getColumnCount(); ++k) {
          ...
          if (checkJdbcType) {
            ...
            if (type1 != type2) {
              throw new RuntimeException("字段 [name=%s -> %s] 的数据类型不同，因 %s!=%s !".formatted(
                  key1, key2,
   ...
   * @throws SQLException
   */
  private int compare(TaskParamEntity task, Object[] rawObj1, Object[] obj2, int[] fieldnrs, ResultSetMetaData metaData)
      throws SQLException {
    Object[] obj1 = rawObj1;
    if (SyncUtil.isShardDbSync(task.getDbSyncMode())) {
