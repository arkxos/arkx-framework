...
public final class DefaultChangeCalculatorService implements RecordRowChangeCalculator {
    ...
    public void setFetchSize(int size) {
        ExamineUtils.check(size >= Constants.MINIMUM_FETCH_SIZE, "设置的批量处理行数的大小fetchSize不得小于%d",
    ...
    public void executeCalculate(TaskParamEntity task, RecordRowHandler handler) {
        ...
        List<String> fieldsMappedPrimaryKeyNew = fieldsPrimaryKeyNew.stream().map(s -> {
            String rev = columnsMap.get(s);
            if (rev == null) {
                rev = columnsMap.get(s.toUpperCase());
            }
            if (rev == null) {
                return s;
            }
        ...
        fieldsAllColumnNew = newMd.queryTableColumnName(task.getNewSchemaName(), task.getNewTableName()).stream()
                .map(String::toLowerCase).collect(Collectors.toList());
        // todo 暂时写死处理
        // if (fieldsAllColumnNew.contains("ark_shard_db_code") &&
        // fieldsAllColumnNew.contains("ark_sync_raw_id")) {
        // fieldsPrimaryKeyNew = Arrays.asList("ark_shard_db_code", "ark_sync_raw_id");
        // fieldsMappedPrimaryKeyNew = Arrays.asList("ark_shard_db_code",
        // "ark_sync_raw_id");
        // }
        fieldsPrimaryKeyNew = fieldsPrimaryKeyOld;

        List<String> fieldsMappedAllColumnNew = fieldsAllColumnNew.stream().map(s -> {
            String rev = columnsMap.get(s);
            if (rev == null) {
                rev = columnsMap.get(s.toUpperCase());
            }
        ...
        // "计算变化量的表中存在无主键的表"
        // );
        // List<String> fixedPks = new ArrayList<>(fieldsMappedPrimaryKeyNew);
        // 从库同步模式
        // if (SyncUtil.isShardDbSync(task.getDbSyncMode())) {
        // fieldsPrimaryKeyNew.add(0, "ark_shard_db_code");
        // fieldsAllColumnNew.add(0, "ark_shard_db_code");
        // }
        // ExamineUtils.check(
        // isListEqual(fieldsPrimaryKeyOld, fieldsPrimaryKeyNew),
        // "两个表的主键映射关系不匹配"
        // );
        ...
        // }
        // List<String> ingoreSlaveDbCodePks = new ArrayList<>(fieldsPrimaryKeyOld);
        // ingoreSlaveDbCodePks.remove("ark_shard_db_code");
        // List<String> newIngoreSlaveDbCodePks = new ArrayList<>(fieldsPrimaryKeyNew);
        // newIngoreSlaveDbCodePks.remove("ark_shard_db_code");
        //
        // boolean same = (mappedFieldColumns.containsAll(ingoreSlaveDbCodePks)
        // && task.getFieldColumns().containsAll(newIngoreSlaveDbCodePks));
        // if (!same) {
        ...
        mappedQueryFieldColumn = queryFieldColumn.stream().map(s -> {
            String rev = columnsMap.get(s);
            if (rev == null) {
                rev = columnsMap.get(s.toUpperCase());
            }
        ...
        try {
            ...
            TableDataQueryProvider oldQuery = ProductProviderFactory
                    .newProvider(task.getOldProductType(), task.getOldDataSource()).createTableDataQueryProvider();
            oldQuery.setQueryFetchSize(this.queryFetchSize);
            TableDataQueryProvider newQuery = ProductProviderFactory
                    .newProvider(task.getNewProductType(), task.getNewDataSource()).createTableDataQueryProvider();
            ...
            if (SyncUtil.isShardDbSync(task.getDbSyncMode())) {
                mappedQueryFieldColumn.addFirst("ark_shard_db_code");
                fieldsMappedPrimaryKeyNew.addFirst("ark_shard_db_code");

                rsold = oldQuery.queryTableData(task.getOldSchemaName(), task.getOldTableName(), task.getSlaveDbCode(),
                        mappedQueryFieldColumn, fieldsMappedPrimaryKeyNew);
                Optional.ofNullable(checkInterrupt).ifPresent(Runnable::run);
            } else {
                rsold = oldQuery.queryTableData(task.getOldSchemaName(), task.getOldTableName(), "",
                        mappedQueryFieldColumn, fieldsMappedPrimaryKeyNew);
                Optional.ofNullable(checkInterrupt).ifPresent(Runnable::run);
            }
            ...
            boolean isColumnCountSame = oldcnt == newcnt;
            if (SyncUtil.isShardDbSync(task.getDbSyncMode())) {
                isColumnCountSame = oldcnt == newcnt + 1;
            }
            if (!isColumnCountSame) {
                throw new RuntimeException("两个表的字段总个数不相等，即：%d!=%d".formatted(oldcnt, newcnt));
            } else {
                for (int k = 1; k < metaData.getColumnCount(); ++k) {
                    ...
                    if (checkJdbcType) {
                        ...
                        if (type1 != type2) {
                            throw new RuntimeException("字段 [name=%s -> %s] 的数据类型不同，因 %s!=%s !".formatted(key1, key2,
            ...
            Object[] two = transformer.doTransform(task.getNewSchemaName(), task.getNewTableName(), queryFieldColumn,
            ...
            while (true) {
                ...
                if (one == null && two == null) {
                    ...
                    int compare = this.compare(task, one, two, keyNumbers, metaData);
                    if (0 == compare) {
                        int compareValues = this.compare(task, one, two, valNumbers, metaData);
                        if (compareValues == 0) {
     ...
     * @throws SQLException
     */
    private int compare(TaskParamEntity task, Object[] rawObj1, Object[] obj2, int[] fieldnrs,
            ResultSetMetaData metaData) throws SQLException {
        Object[] obj1 = rawObj1;
        if (SyncUtil.isShardDbSync(task.getDbSyncMode())) {
            obj1 = new Object[rawObj1.length - 1];
            for (int i = 1; i < rawObj1.length; i++) {
                obj1[i - 1] = rawObj1[i];
            }
        }
        boolean isColumnCountSame = obj1.length == obj2.length;
        if (!isColumnCountSame) {
    ...
    private int typeCompare(int type, Object o1, Object o2) {
        ...
        if (JdbcTypesUtils.isString(type)) {
        ...
        } else if (JdbcTypesUtils.isNumeric(type) && o1 instanceof java.lang.Number s1
                && o2 instanceof java.lang.Number s2) {
            return Double.compare(s1.doubleValue(), s2.doubleValue());
        } else if (JdbcTypesUtils.isInteger(type) && o1 instanceof java.lang.Number s1
                && o2 instanceof java.lang.Number s2) {
            return Long.compare(s1.longValue(), s2.longValue());
        } else if (JdbcTypesUtils.isDateTime(type)) {
            if (o1 instanceof LocalTime time) {
                o1 = java.sql.Time.valueOf(time);
            }
            if (o1 instanceof LocalDate date) {
                o1 = java.sql.Date.valueOf(date);
            }
            if (o1 instanceof LocalDateTime time) {
                o1 = java.sql.Timestamp.valueOf(time);
            }
            if (o2 instanceof LocalTime time) {
                o2 = java.sql.Time.valueOf(time);
            }
            if (o2 instanceof LocalDate date) {
                o2 = java.sql.Date.valueOf(date);
            }
            if (o2 instanceof LocalDateTime time) {
                o2 = java.sql.Timestamp.valueOf(time);
            }
            if (o1 instanceof java.sql.Time t1 && o2 instanceof java.sql.Time t2) {
                return t1.compareTo(t2);
            } else if (o1 instanceof java.sql.Timestamp t1 && o2 instanceof java.sql.Timestamp t2) {
                return t1.compareTo(t2);
            } else if (o1 instanceof java.sql.Date t1 && o2 instanceof java.sql.Date t2) {
                return t1.compareTo(t2);
            } else {
            ...
            try {
                return compareTo(ObjectCastUtils.castToByteArray(o1), ObjectCastUtils.castToByteArray(o2));
            } catch (Exception e) {
