...
public class GenericTypeUtils {
     ...
     *
     * @param clazz
     *            目标类的 Class 对象（需显式继承泛型父类，如 `class Sub extends Base<T>`）
     * @param <T>
     *            泛型父类的类型参数（如 `Base<T>` 中的 T）
     ...
     *            目标类的 Class 对象
     * @param parentClass
     *            目标泛型父类的 Class 对象（如 `Base.class`，若为 null 则取直接父类）
     * @param <P>
     *            泛型父类的类型（如 `Base
     *            <P>
     *            ` 中的 P）
     ...
     *
     * @param clazz
     *            目标类的 Class 对象（需显式实现泛型接口，如 `class Impl implements MyInterface<T>`）
     * @param interfaceClass
     *            目标泛型接口的 Class 对象（如 `MyInterface.class`）
     * @param <T>
     *            泛型接口的类型参数（如 `MyInterface<T>` 中的 T）
     * @return 第一个泛型参数的 Class 对象（若未指定泛型参数，抛出异常）
     * @throws IllegalStateException
     *             当泛型参数未显式指定或类型擦除时
     */
     ...
     *
     * @param type
     *            待解析的类型（可能是 TypeVariable、WildcardType、ParameterizedType 等）
