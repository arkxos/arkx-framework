...
public class ChildTabTag extends ArkTag {
	...
	private static Pattern PZIcon = Pattern.compile(" src\\=\"[^\"]*icons\\/([^\"\\/]+)\\.png\"", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
	...
	public int doStartTag() throws TemplateRuntimeException {
		...
		if ("Disabled".equalsIgnoreCase(type)) {
			...
			StringFormat sf = new StringFormat("id='?' ? class='z-tab' ? onclick=\"??Ark.TabPage.onChildTabClick(this);?return false;\">");
			sf.add(id);
			sf.add(vStr);
			sf.add(src);
			sf.add(beforeClick);
