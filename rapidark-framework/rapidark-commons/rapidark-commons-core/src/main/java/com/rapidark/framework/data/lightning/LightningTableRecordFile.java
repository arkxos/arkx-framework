...
//			openReadFileChannel();
			...
			dt = lightningTableConvertor.createTable(lightningTableType, tableInfo.tableName, columns.toArray(new LightningColumn[0]));
...
//			long rowSizeLength = tableInfo.rowSize;
	...
	protected RecordUnit readRow(TableInfo tableInfo, FileChannel fileChannel, 
...
//		fileChannel.position(INT_LENGTH + headerLength + recordStart * rowDataLength);
	...
	public <T extends ILightningTable> PkList save(T dt) {
		...
		try {
			...
			ILightningTableConvertor<T> lightningTableConvertor = (ILightningTableConvertor<T>)LightningTableConvertRegister.get(dt.getClass());
			...
			MappedByteBuffer rowMappedByteBuffer = fileChannel.map(FileChannel.MapMode.READ_WRITE, fileLength, dt.getRowCount()*rowDataLength);
			allRowBuffer.flip();
