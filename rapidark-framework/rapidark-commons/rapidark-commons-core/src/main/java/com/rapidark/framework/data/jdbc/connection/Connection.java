package com.rapidark.framework.data.jdbc.connection;
...
//		ConnectionPool pool = (ConnectionPool) ConnectionPoolManager.PoolMap.get(this.DBConfig.getDatabaseName() + ".");
...
//		}
//
//		logger.error(this.DBConfig.getPoolName() + ":检测到连接使用超时" + (now - LastSuccessExecuteTime) + "，程序可能存在连接池泄漏，将自动关闭连接。以下是最后执行的SQL及调用堆栈:");
