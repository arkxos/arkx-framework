...
//			}
	...
	public RemoteHttpPartFile split() {
		...
		if (remainingCenter > minSize) {
			...
			RemoteHttpPartFile newSplitedRunnable = new RemoteHttpPartFile(downloadMonitor, fileUrl, saveDirectory, saveFileName, centerPosition + 1, end);
	...
	public String getId() {
		String id = this.getFileUrl() + "-" + this.getStartPosition() + "-" + this.getEndPosition() + "-" + this.getTaskStatus();
		return id;
	}
