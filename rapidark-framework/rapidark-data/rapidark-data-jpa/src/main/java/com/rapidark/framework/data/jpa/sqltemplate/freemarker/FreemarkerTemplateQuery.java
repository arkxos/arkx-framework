...
import com.rapidark.framework.data.jpa.AopTargetUtils;
import com.rapidark.framework.data.jpa.QueryBuilder;
...
//        ParameterAccessor accessor = new ParametersParameterAccessor(parameters, values);
    ...
    protected TypedQuery<Long> doCreateCountQuery(JpaParametersParameterAccessor accessor) {
        TypedQuery query = (TypedQuery) getEntityManager()
                .createNativeQuery(QueryBuilder.toCountQuery(getQuery(accessor.getValues())));
        bind(query, accessor.getValues());
        return query;
    }
