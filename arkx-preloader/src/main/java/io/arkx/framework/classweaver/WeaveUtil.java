	...
	public static void invokeStaticMethod(String weaverClass, String targetMethod, Object[] args) throws Exception {
		...
		if (cnt != null) {
			...
			for (int i = 0; i < j; i++) {
				...
				if (((m.getModifiers() & 0x8) != 0) && (m.getName().equals(targetMethod))
						&& (matchArgs(m.getParameterTypes(), args))) {
					flag = true;
					m.invoke(null, args);
					break;
				}
			...
			if (!flag) {
				throw new RuntimeException(
						"WeaveUtil.invokeStaticMethod() failed:Weaver=" + weaverClass + ",Method=" + targetMethod);
			}
		}
	}

	public static void invokeInstanceMethod(String weaverClass, Object[] constructorArgs, String targetMethod,
		...
		if (cnt != null) {
			...
			if (!flag) {
				throw new RuntimeException(
						"WeaveUtil.invokeInstanceMethod() failed:Weaver=" + weaverClass + ",Constructor not found!");
			}
			...
			if (!flag) {
				throw new RuntimeException(
						"WeaveUtil.invokeInstanceMethod() failed:Weaver=" + weaverClass + ",Method=" + targetMethod);
			}
		}
	}
