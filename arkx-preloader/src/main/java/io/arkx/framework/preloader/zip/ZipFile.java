 ...
 *
 * <p>
 * This class adds support for file name encodings other than UTF-8 (which is required to
 * work on ZIP files created by native zip tools and is able to skip a preamble like the
 * one found in self extracting archives. Furthermore it returns instances of
 * <code>org.apache.tools.zip.ZipEntry</code> instead of
 * <code>java.util.zip.ZipEntry</code>.
 * </p>
 *
 * <p>
 * It doesn't extend <code>java.util.zip.ZipFile</code> as it would have to reimplement
 * all methods anyway. Like <code>java.util.ZipFile</code>, it uses RandomAccessFile under
 * the covers and supports compressed and uncompressed entries.
 * </p>
 *
 * <p>
 * The method signatures mimic the ones of <code>java.util.zip.ZipFile</code>, with a
...
public class ZipFile {
	 ...
	 *
	 * <p>
	 * For a list of possible values see <a href=
	 * "http://java.sun.com/j2se/1.5.0/docs/guide/intl/encoding.doc.html">http://java.sun.com/j2se/1.5.0/docs/guide/intl/encoding.doc.html</a>.
	...
	/**
	 * Opens the given file for reading, assuming the platform's native encoding for file
	 * names.
	 * @param f the archive.
	...
	/**
	 * Opens the given file for reading, assuming the specified encoding for file names
	 * and scanning for unicode extra fields.
	 * @param f the archive.
	 * @param encoding the encoding to use for file names, use null for the platform's
	 * default encoding
	 * @throws IOException if an error occurs while reading the file.
	 */
	 ...
	 * @param encoding the encoding to use for file names, use null for the platform's
	 * default encoding
	 * @param useUnicodeExtraFields whether to use InfoZIP Unicode Extra Fields (if
	 * present) to set the file names.
	 * @throws IOException if an error occurs while reading the file.
	 */
	 ...
	 * @return the ZipEntry corresponding to the given name - or <code>null</code> if not
	 * present.
	 */
	...
	private static final int CFH_LEN = /* version made by */ SHORT
	...
	/**
	 * Reads the central directory of the given archive and populates the internal tables
	 * with ZipEntry instances.
	 *
	 * <p>
	 * The ZipEntrys will know all data that can be obtained from the central directory
	 * alone, but not the data that requires the local file header or additional data to
	 * be read.
	 * </p>
	 * @return a Map&lt;ZipEntry, NameAndComment>&gt; of zipentries that didn't have the
	 * language encoding flag set when read.
	 */
	private Map populateFromCentralDirectory() throws IOException {
		...
		while (sig == cfhSig) {
			...
			final boolean hasUTF8Flag = (generalPurposeFlag & ZipOutputStream.UFT8_NAMES_FLAG) != 0;
			...
			// FIXME this is actually not very cpu cycles friendly as we are converting
			// from
	...
	private static final int MIN_EOCD_SIZE = /* end of central dir signature */ WORD
	...
	/**
	 * Searches for the &quot;End of central dir record&quot;, parses it and positions the
	 * stream at the first central directory record.
	 */
	private void positionAtCentralDirectory() throws IOException {
		boolean found = false;
		long off = archive.length() - MIN_EOCD_SIZE;
		final long stopSearching = Math.max(0L, archive.length() - MAX_EOCD_SIZE);
		if (off >= 0) {
	...
	/**
	 * Number of bytes in local file header up to the &quot;length of filename&quot;
	 * entry.
	 */
	...
	/**
	 * Walks through all recorded entries and adds the data available from the local file
	 * header.
	 *
	 * <p>
	...
	private void resolveLocalFileHeaderData(Map entriesWithoutUTF8Flag) throws IOException {
		...
		while (e.hasMoreElements()) {
			...
			/*
			 * dataOffsets.put(ze, Long.valueOf(offset + LFH_OFFSET_FOR_FILENAME_LENGTH +
	...
	/**
	 * Checks whether the archive starts with a LFH. If it doesn't, it may be an empty
	 * archive.
	 */
	...
	/**
	 * If the entry has Unicode*ExtraFields and the CRCs of the names/comments match those
	 * of the extra fields, transfer the known Unicode values from the extra field.
	 */
	...
	/**
	 * If the stored CRC matches the one of the given name, return the Unicode name of the
	 * given field.
	 *
	 * <p>
	...
	/**
	 * InputStream that delegates requests to the underlying RandomAccessFile, making sure
	 * that only bytes from a certain range can be read.
	 */
