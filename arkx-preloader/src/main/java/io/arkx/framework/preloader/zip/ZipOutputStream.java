...
/**
 * Reimplementation of {@link java.util.zip.ZipOutputStream java.util.zip.ZipOutputStream}
 * that does handle the extended functionality of this package, especially
 * internal/external file attributes and extra fields with different layouts for local
 * file data and central directory entries.
 *
 * <p>
 * This class will try to use {@link java.io.RandomAccessFile RandomAccessFile} when you
 * know that the output is going to go to a file.
 * </p>
 *
 * <p>
 * If RandomAccessFile cannot be used, this implementation will use a Data Descriptor to
 * store size and CRC information for {@link #DEFLATED DEFLATED} entries, this means, you
 * don't need to calculate them yourself. Unfortunately this is not possible for the
 * {@link #STORED STORED} method, here setting the CRC and uncompressed size information
...
public class ZipOutputStream extends FilterOutputStream {
	...
	/*
	 * Apparently Deflater.setInput gets slowed down a lot on Sun JVMs when it gets handed
	 * a really big buffer. See https://issues.apache.org/bugzilla/show_bug.cgi?id=45396
	 *
	...
	/**
	 * Offset for CRC entry in the local file header data for the current entry starts
	 * here.
	 *
	 * @since 1.15
	 */
	 ...
	 * This class used to extend {@link java.util.zip.DeflaterOutputStream
	 * DeflaterOutputStream} up to Revision 1.13.
	 * </p>
	 *
	 * @since 1.14
	 */
	...
	/**
	 * This method indicates whether this archive is writing to a seekable stream (i.e.,
	...
	/**
	 * Whether to fall back to UTF and the language encoding flag if the file name cannot
	...
	public void closeEntry() throws IOException {
		...
		if (entry.getMethod() == DEFLATED) {
			...
			if (entry.getSize() != written - dataStart) {
				throw new ZipException("bad size for entry " + entry.getName() + ": " + entry.getSize() + " instead of "
						+ (written - dataStart));
			}
		}
	...
	/**
	 * Assumes a negative integer really is a positive integer that has wrapped around and
	 * re-creates the original value.
	...
	public static final class UnicodeExtraFieldPolicy {
		...
		/**
		 * Create Unicode extra fields for filenames that cannot be encoded using the
		 * specified encoding.
		 */
