...
public class SlidingWindow<T> {
    ...
    private Consumer<T> consumerDataPacketFunction = t -> {
    }; // 消费数据包函数，默认空
    ...
    public void start() {
        ...
        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(1, (int) finalWindowSize, 10, TimeUnit.SECONDS,
                new SynchronousQueue<>(true), new ThreadPoolExecutor.CallerRunsPolicy());
        try {
            List<CompletableFuture<T>> windowList = new LinkedList<>();
            LongStream.rangeClosed(1, finalWindowSize).forEach(index -> windowList.add(
                    CompletableFuture.supplyAsync(() -> producerDataPacketFunction.apply(index), threadPoolExecutor)));
            long current = 1;
            do {
                ...
                if (dataPacketTotal - finalWindowSize >= current) {
                    ...
                    windowList.add(CompletableFuture.supplyAsync(() -> producerDataPacketFunction.apply(index),
                            threadPoolExecutor));
                }
