...
public class SlidingWindow<T> {
	...
	public void start() {
		...
		try {
			...
			LongStream.rangeClosed(1, finalWindowSize)
				.forEach(index -> windowList.add(CompletableFuture
					.supplyAsync(() -> producerDataPacketFunction.apply(index), threadPoolExecutor)));
			long current = 1;
			do {
