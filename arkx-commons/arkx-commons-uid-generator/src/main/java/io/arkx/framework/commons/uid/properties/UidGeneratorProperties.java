package io.arkx.framework.commons.uid.properties;
...
@Getter
@Setter
@ConfigurationProperties(prefix = "arkx.uid")
public class UidGeneratorProperties {

    /**
     * boostPower：RingBuffer size扩容参数, 可提高UID生成的吞吐量 默认为3，原bufferSize=8192, 扩容后
     * bufferSize= 8192 << 3 = 65536
     */
    private int boostPower = 3;

    /**
     * Specified bits & epoch as your demand. No specified the default value will be
     * used\
     */
     ...
     * {"workerBits":23,"timeBits":31,"seqBits":9}时, 可支持28个节点以整体并发量14400
     * UID/s的速度持续运行68年. 对于节点重启频率频繁、期望长期使用的应用, 可增加workerBits和timeBits位数, 减少seqBits位数.
     * 例如节点采取用完即弃的WorkerIdAssigner策略, 重启频率为24*12次/天,
     * 那么配置成{"workerBits":27,"timeBits":30,"seqBits":6}时, 可支持37个节点以整体并发量 2400 UID/s
     * 的速度持续运行34年.
     */
    private int seqBits;
     ...
     * 如需使用请指定Schedule线程时间间隔, 单位:秒
     */
    private long scheduleInterval;

    /**
     * Generator模式 {@link Mode} 见注释解释 {@link UidGenerator} 根据项目情况来选择是否开启缓存模式
     */
