...
import io.arkx.framework.commons.uid.buffer.RejectedTakeBufferHandler;
 ...
 * <li><b>boostPower:</b> RingBuffer size boost for a power of 2, Sample: boostPower is 3,
 * it means the buffer size will be
 * <code>({@link BitsAllocator#getMaxSequence()} + 1) &lt;&lt;
 * {@link #boostPower}</code>, Default as {@value #DEFAULT_BOOST_POWER}
 * <li><b>paddingFactor:</b> Represents a percent value of (0 - 100). When the count of
 * rest available UIDs reach the threshold, it will trigger padding buffer. Default
 * as{@link RingBuffer#DEFAULT_PADDING_PERCENT} Sample: paddingFactor=20, bufferSize=1000
 * -> threshold=1000 * 20 /100, padding buffer will be triggered when
 * tail-cursor<threshold
 * <li><b>scheduleInterval:</b> Padding buffer in a schedule, specify padding buffer
 * interval, Unit as second
 * <li><b>rejectedPutBufferHandler:</b> Policy for rejected put buffer. Default as discard
 * put request, just do logging
 * <li><b>rejectedTakeBufferHandler:</b> Policy for rejected take buffer. Default as
