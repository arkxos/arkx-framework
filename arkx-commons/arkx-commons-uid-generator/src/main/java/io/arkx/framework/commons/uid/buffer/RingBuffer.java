 ...
 * Using array could improve read element performance due to the CUP cache line. To
 * prevent the side effect of False Sharing, {@link PaddedAtomicLong} is using on 'tail'
 * and 'cursor'
 * <p>
 *
 * A ring buffer is consisted of:
 * <li><b>slots:</b> each element of the array is a slot, which is be set with a UID
 * <li><b>flags:</b> flag array corresponding the same index with the slots, indicates
 * whether can take or put slot
...
public class RingBuffer {
	 ...
	 * @param bufferSize must be positive & a power of 2
	 * @param paddingFactor percent in (0 - 100). When the count of rest available UIDs
	 * reach the threshold, it will trigger padding buffer<br>
	 * Sample: paddingFactor=20, bufferSize=1000 -> threshold=1000 * 20 /100, padding
	 * buffer will be triggered when tail-cursor<threshold
	 */
	 ...
	 * We use 'synchronized' to guarantee the UID fill in slot & publish new tail sequence
	 * as atomic operations<br>
	 *
	 * <b>Note that: </b> It is recommended to put UID in a serialize way, cause we once
	 * batch generate a series UIDs and put the one by one into the buffer, so it is
	 * unnecessary put in multi-threads
	 * @param uid
	...
	/**
	 * Take an UID of the ring at the next cursor, this is a lock free operation by using
	 * atomic cursor
	 * <p>
	 *
	 * Before getting the UID, we also check whether reach the padding threshold, the
	 * padding buffer operation will be triggered in another thread<br>
	...
	/**
	 * Policy for {@link RejectedTakeBufferHandler}, throws {@link RuntimeException} after
	 * logging
	 */
