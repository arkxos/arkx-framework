package io.arkx.framework.commons.websocket.holder;
...
import io.arkx.framework.commons.websocket.config.WebSocketProperties;
...
public class MapSessionWebSocketHandlerDecorator extends WebSocketHandlerDecorator {
     ...
     *
     * @param delegate
     *            被装饰的原始 {@link WebSocketHandler}。
     * @param sessionKeyGenerator
     *            会话密钥生成器，用于生成会话的唯一标识。
     * @param webSocketProperties
     *            WebSocket 配置属性，包含发送时间限制和缓冲区大小限制。
     */
     ...
     * <p>
     * 此方法会生成会话的唯一标识，并将带有并发控制的 {@link WebSocketSession} 存储到
     * {@link WebSocketSessionHolder} 中。 {@link ConcurrentWebSocketSessionDecorator}
