package com.arkxos.framework.message.tcp.extend;
...
import com.arkxos.framework.message.tcp.struct.ResponseMessage;
...
public class HeartBeatResponseHandler extends SimpleChannelInboundHandler<NettyMessage> {
	...
	public void channelRead0(ChannelHandlerContext ctx, NettyMessage message) throws Exception {
		// 返回心跳应答消息
		if(message.getType() == MessageType.REQUEST && message.getBusinessType() == NettyBusinessType.HEARTBEAT.value()) {
