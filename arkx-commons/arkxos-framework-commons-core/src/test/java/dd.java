...
//		System.out.println(DateUtil.lastDayOfWeek(LocalDate.parse("2018-08-01")));
		...
		System.out.println("\"hidden\" name=\"subId\" value=\"<%=request.getParameter(\"subId\") %>\"/>".split("<%=request\\.getParameter(\"subId\") %>")[0]);
		System.out.println(StringUtil.splitEx("\"hidden\" name=\"subId\" value=\"<%=request.getParameter(\"subId\") %>\"/>", "<%=request.getParameter(\"subId\") %>")[0]);
	}
