...
public class LocalFileTemplate implements FileTemplate {
    ...
    public List<SimpleBucket> getAllBuckets() {
        return Arrays.stream(FileUtil.ls(properties.getLocal().getBasePath())).filter(FileUtil::isDirectory)
                .map(dir -> new SimpleBucket(dir.getName())).toList();
    }
    ...
    public List<SimpleObjectSummary> getAllObjectsByPrefix(String bucketName, String prefix, boolean recursive) {
        ...
        return Arrays.stream(FileUtil.ls(dir)).filter(file -> file.getName().startsWith(prefix))
                .map(file -> new SimpleObjectSummary(file.getName())).toList();
    }
