...
public class ForeachFixTransformor {
	...
	// public static final Pattern PatternField2 =
	// Pattern.compile("c:forEach([\\w\\\.\\s]+?)items=\"<c:out\\svalue=\"${TAG_DICT_RESULT.children}\"\s/>\"");
	// <c:forEach var="trace" items="<c:out
	// value="${pageContext.exception.stackTrace}" />">
	public static final Pattern PatternField = Pattern.compile("<c:forEach([\\s\\S])*?items=\"(([\\s\\S]*?)/>)\"");

	// public static final Pattern PatternField =
	// Pattern.compile("c:forEach([\\w\\.\\s]+?)items=\"<c:out
	// value=\"\\$\\{([\\w\\.\\s]+?)\\} />\"");
	// public static final Pattern PatternField =
	// Pattern.compile("\\$\\{([\\w\\.\\s]+?)\\}");
