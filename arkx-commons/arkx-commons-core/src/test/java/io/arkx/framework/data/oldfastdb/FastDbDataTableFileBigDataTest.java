 ...
 * @since infinity 1.0
 */
public class FastDbDataTableFileBigDataTest {
	...
	private FastDataTable preparedDataTable() {
		...
		FastDataTable dt = new FastDataTable(TEST_TABLE_NAME, new DataColumn[] { 
				dcDate, dcString, dcFixedString, dcInt, dcFloat, dcDouble, dcLong });
...
//		System.out.println(dtFromFile.getDataRow(10*10000));
		...
		filterDtFromFile = fastdb.select(TEST_TABLE_NAME, "fixedstring='102001'");
		
		assertEquals(365*yearCount, filterDtFromFile.getRowCount());
		timeWatch.stopAndPrint();
