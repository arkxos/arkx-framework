...
public class FactoryGroupedClasses {
     ...
     *             if another annotated class with the same id is already present.
     */
    ...
    public void generateCode(Elements elementUtils, Filer filer) throws IOException {
        ...
        MethodSpec.Builder method = MethodSpec.methodBuilder("create").addModifiers(Modifier.PUBLIC)
                .addParameter(String.class, "id").returns(TypeName.get(superClassName.asType()));

        // check if id is null
        method.beginControlFlow("if (id == null)").addStatement("throw new IllegalArgumentException($S)", "id is null!")
                .endControlFlow();
     ...
     *             elementUtils.getTypeElement(qualifiedClassName); String
     *             factoryClassName = superClassName.getSimpleName() + SUFFIX;
     *
     *             JavaFileObject jfo = filer.createSourceFile(qualifiedClassName +
     *             SUFFIX); Writer writer = jfo.openWriter(); JavaWriter jw = new
     *             JavaWriter(writer);
     *
     *             // Write package PackageElement pkg =
     *             elementUtils.getPackageOf(superClassName); if (!pkg.isUnnamed())
     ...
     *
     *             jw.beginControlFlow("if (id == null)"); jw.emitStatement("throw
     ...
     *
     *             jw.emitStatement("throw new IllegalArgumentException(\"Unknown id
     *             = \" + id)"); jw.endMethod();
     *
     *             jw.endType();
     *
