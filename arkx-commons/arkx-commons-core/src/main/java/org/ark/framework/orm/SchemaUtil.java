...
public class SchemaUtil {
							...
							for (i = 0; i < schema.operateColumnOrders.length; ++i) {
								if (schema.Columns[schema.operateColumnOrders[i]].getColumnType() == 10) {
									if ((ConnectionPoolManager.getConnection().getDBConfig().isOracle())
										...
										newSchema.setV(schema.operateColumnOrders[i],
												LobUtil.clobToString(rs.getClob(i + 1)));
									else
									...
									newSchema.setV(schema.operateColumnOrders[i],
											LobUtil.blobToBytes(rs.getBlob(i + 1)));
								else
									newSchema.setV(schema.operateColumnOrders[i], rs.getObject(i + 1));
							}
						}
		...
		try {
			...
			for (int i = 0; i < bset.Columns.length; i++) {
				if ((bset.Columns[i].isPrimaryKey())
						&& (!bset.Columns[i].getColumnName().equalsIgnoreCase("BackupNo"))) {
	...
	public static void setParam(SchemaColumn sc, PreparedStatement pstmt, Connection conn, int i, Object v)
			throws SQLException {
		if (v == null) {
			if (sc.getColumnType() == 7)
				...
				if (conn.getDBConfig().isSybase())
					DBTypeService.getInstance()
						.get(conn.getDBConfig().getDatabaseType())
						.setClob(conn, pstmt, i + 1, "");
				else
