...
public class ZDTParser {
    ...
    protected static SchemaColumn[] parseColumns(String columns) {
        ...
        for (String str : arr) {
            ...
            SchemaColumn sc = new SchemaColumn(name, type, order, length, precision, mandatory, pk, "");
            list.add(sc);
        }
        ...
        label545 : for (String str : arr) {
            ...
            for (String v : arr2) {
                if (ObjectUtil.empty(v))
                    vs[i] = null;
                else if (ObjectUtil.in(new Object[]{Integer.valueOf(scs[i].getColumnType()), Integer.valueOf(1),
                        Integer.valueOf(10)})) {
                    if (v.equals("null"))
                        vs[i] = null;
                    else
                        vs[i] = StringUtil.javaDecode(v.substring(1, v.length() - 1));
                } else if (ObjectUtil.in(
                        new Object[]{Integer.valueOf(scs[i].getColumnType()), Integer.valueOf(8), Integer.valueOf(9)}))
                    ...
                    vs[i] = Float.valueOf(Float.parseFloat(v));
                        ...
                        new Object[]{Integer.valueOf(scs[i].getColumnType()), Integer.valueOf(6), Integer.valueOf(4)}))
