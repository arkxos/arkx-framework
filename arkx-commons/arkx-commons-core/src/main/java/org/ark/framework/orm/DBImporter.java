...
public class DBImporter {
    ...
    // }
    //
    // public boolean importDB(String file, String poolName, ArrayList<String>
    // tableList) {
    ...
    // }
    //
    // public boolean importDB(String file, XConnection conn, boolean autoCreate,
    ...
    // Mapx map = new Mapx();
    // TableCreator tc = new
    // TableCreator(this.da.getConnection().getDBConfig().getDBType());
    // int j = 0;
    // for (ZDTParser.ZDTTableInfo ti : parser.getTables()) {
    ...
    // }
    // j++;
    // this.task.setPercent(Double.valueOf(j * 100.0D /
    // parser.getTables().size()).intValue());
    ...
    // }
    //
    // private boolean importDataTable(SchemaColumn[] scs, DataTable dt, String
    // name) throws Exception {
    // QueryBuilder qb = getSession().createQueryBuilder("insert into " + name + "
    // values (", new Object[0]);
    ...
    // LogUtil.warn(name + "'s column " + scs[j].getColumnName() + " can't be
    // empty");
    // dt.deleteRow(i);
    // i--;
    // break;
                ...
                try {
                    ...
                    try {
                        ...
                        if (false) {// !map.containsKey(currentImportTableName)) {
                            DataTable cdt = getSession().createQuery(
                            ...
                            for (int j = 0; j < scs.length; j++) {
                                ...
                                SchemaColumn sc = new SchemaColumn(cdr.getString("Code"), type, j, cdr.getInt("Length"),
                                        0, "Y".equals(cdr.getString("isMandatory")),
                            ...
                            StringBuffer sb = new StringBuffer("insert into " + currentImportTableName + "(");
        ...
        if (session.getConnection().getDBConfig().isOracle()) {
            for (int i = 0; i < set.size(); i++) {
                ...
                for (int j = 0; j < schema.getColumns().length; j++) {
                    ...
                    if ((schema.getColumns()[j].isMandatory()) && ((v == null) || (v.equals("")))) {
                        log.warn("表" + schema.getTableCode() + "的" + schema.getColumns()[j].getColumnName() + "列不能为空!");
                ...
                // System.out.println((Double.valueOf(i * 100.0D / set.size()).intValue())
                // + ",
                // " + i + "/" + set.size());
            }
        }
        ...
        try {
            ...
            for (int i = 0; i < dt.getRowCount(); i++) {
                for (int j = 0; j < dt.getColumnCount(); j++) {
                    if ((j == dt.getColumnCount() - 1) && ((dt.getDataColumn(j).getColumnName().equalsIgnoreCase("RNM"))
                            || (dt.getDataColumn(j).getColumnName().equalsIgnoreCase("_RowNumber")))) {
                        break;
                    }
