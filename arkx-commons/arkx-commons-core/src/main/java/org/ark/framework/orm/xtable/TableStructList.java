...
public class TableStructList {
		...
		synchronized (mutex) {
			...
			try {
				dt = SqlHelper.ExecuteDatatable(
						"SELECT (SELECT isnull(syscolumns.name, '') AS colname FROM syscolumns INNER JOIN "
				...
				dtcols = SqlHelper.ExecuteDatatable(
						"SELECT id, name, xtype,length,isnullable,cdefault,colstat,colid,isnull((select text from syscomments where id=syscolumns.cdefault) , '') as defvalue FROM syscolumns WHERE id in (select id from sysobjects WHERE (xtype = 'U') AND sysobjects.name <> 'dtproperties' ) ORDER BY id,syscolumns.colorder,xtype");
			}
			catch (Exception e) {
				_HandleError = true;
				throw e;
			...
			for (int i = 0; i < dt.getRowCount(); i++) {
				...
				dtcolremark = SqlHelper.ExecuteDatatable(
						"SELECT objname,cast(value as varchar(8000)) as remark FROM ::fn_listextendedproperty('MS_Description', 'USER', 'dbo', 'TABLE', '"
								+ m_Name + "', 'COLUMN', NULL) [::fn_listextendedproperty_1] order by objname");
				...
				for (; colpos < dtcols.getRowCount(); colpos++) {
					...
					ColInfos.Add(new ColInfo(m_Name, colname, xtype, length, isNullAble, haveDefault, isPK, isIncrement,
							remark, colDefault));
				}
	...
	private static void TestSpeed(String sql) {
		// SqlHelper.ExecuteNonQuery("insert into TablesBasicDebug (tbl , sqls) values
		// ('tablestructlist TestSpeed' , '"
