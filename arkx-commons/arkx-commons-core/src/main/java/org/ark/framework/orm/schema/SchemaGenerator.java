...
public abstract class SchemaGenerator {
    ...
    private void timeOperation(String name, Runnable runnable) {
        ...
        try {
            ...
            perfCounters.computeIfAbsent(name, k -> new AtomicLong(0)).addAndGet(elapsed);
        }
    }
    ...
    private void reportPerformance() {
        ...
        // 输出表生成耗时排名
        log.info("===== 表生成耗时排名 =====");
        tableGenerateTimes.entrySet().stream().sorted((e1, e2) -> Long.compare(e2.getValue(), e1.getValue())) // 从高到低排序
                .forEach(entry -> {
                    log.info("表[{}]生成耗时: {}ms", entry.getKey(), entry.getValue());
                });

        log.info("===== 操作耗时统计 =====");
        perfCounters.entrySet().stream().filter(entry -> !entry.getKey().equals("generate"))
                .sorted((e1, e2) -> Long.compare(e2.getValue().get(), e1.getValue().get())).forEach(entry -> {
    ...
    protected void generateOneSechma(String tableName, String tableCode, String tableComment, SchemaColumn[] scs) {
        ...
        timeOperation("generateOneSechma(" + finalTableCode + ").codeGeneration", () -> {
            ...
            insertsb.append("\tprotected static final String _InsertAllSQL = \"insert into ").append(finalTableCode)
                    .append(" (");
            updatesb.append("\tprotected static final String _UpdateAllSQL = \"update ").append(finalTableCode)
                    .append(" set ");
            pksb.append(" where ");
            ...
            for (int i = 0; i < scs.length; i++) {
                ...
                if ((type.startsWith("nvarchar")) || (type.startsWith("varchar")) || (type.startsWith("char"))
                        || (type.startsWith("nchar")) || (type.startsWith("bpchar")) || (type.startsWith("enum"))) {
                    type = "String";
                    ctype = "STRING";
                    vtype = type;
                        ...
                        || (type.startsWith("clob") || (type.startsWith("jsonb")))) {
                    type = "String";
                    ctype = "CLOB";
                    vtype = type;
                    ...
                    vtype = "Float";
                        ...
                        || (type.startsWith("money")) || (type.startsWith("numeric"))) {
                    type = "double";
                    ctype = "DOUBLE";
                    vtype = "Double";
                ...
                if ((vtype.equals("Float")) || (vtype.equals("Integer")) || (vtype.equals("Long"))
                        || (vtype.equals("Double"))) {
                ...
                csb.append("\tpublic void set").append(uCode).append("(").append(type).append(" ").append(tCode)
                        .append(") {\n");
                if ((vtype.equals("Float")) || (vtype.equals("Integer")) || (vtype.equals("Long"))
                        || (vtype.equals("Double")))
                    csb.append("\t\tthis.").append(code).append(" = new ").append(vtype).append("(").append(tCode)
                            .append(");\n");
                else {
                ...
                hsb.append("\t\tnew SchemaColumn(\"").append(code).append("\", DataTypes.").append(ctype)
                        .append(".code(), ").append(i).append(", ").append(scs[i].Length).append(" , ")
            ...
            hsb.append(insertsb);
            hsb.append(updatesb);
            hsb.append("\tprotected static final String _DeleteSQL = \"delete from ").append(finalTableCode).append(" ")
                    .append(pksb.toString()).append("\";\n\n");
            hsb.append("\tprotected static final String _FillAllSQL = \"select * from ").append(finalTableCode)
                    .append(" ").append(pksb.toString()).append("\";\n\n");
            ...
            hsb.append("\tpublic ").append(finalTableCode)
                    .append("Set query(Query qb , int pageSize, int pageIndex){\n");
    ...
    private void generateSet(String tableCode) {
        ...
        timeOperation("generateSet(" + finalTableCode + ")", () -> {
            ...
            sb.append("public class ").append(finalTableCode).append("Set extends SchemaSet<").append(finalTableCode)
                    .append("Schema> {\n");
            ...
            sb.append("\tpublic ").append(finalTableCode).append("Set(int initialCapacity,int capacityIncrement) {\n");
    ...
    class SchemaColumn {
        ...
        public void setMandatory(String mandatory) {
            if ((mandatory == null) || (mandatory.equals(""))
                    || (mandatory.equals("0") || "NO".equalsIgnoreCase(mandatory) || "N".equalsIgnoreCase(mandatory)))
                Mandatory = false;
            else {
                Mandatory = true;
            }
