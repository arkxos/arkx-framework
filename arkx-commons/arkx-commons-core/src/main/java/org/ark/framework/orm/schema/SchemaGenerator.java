...
public abstract class SchemaGenerator {
	...
	protected void generateOneSechma(String tableName, String tableCode, String tableComment, SchemaColumn[] scs) {
		...
		timeOperation("generateOneSechma(" + finalTableCode + ").codeGeneration", () -> {
			...
			insertsb.append("\tprotected static final String _InsertAllSQL = \"insert into ")
				.append(finalTableCode)
				.append(" (");
			updatesb.append("\tprotected static final String _UpdateAllSQL = \"update ")
				.append(finalTableCode)
				.append(" set ");
			pksb.append(" where ");
			...
			for (int i = 0; i < scs.length; i++) {
				...
				hsb.append("\t\tnew SchemaColumn(\"")
					...
					.append(scs[i].Length)
					.append(" , ")
					.append(scs[i].Precision)
					.append(" , ")
					.append(scs[i].Mandatory)
					.append(" , ")
					.append(scs[i].isPrimaryKey)
					.append(",\"")
					.append(scs[i].Comment)
					.append("\")");
				if (i < scs.length - 1)
					hsb.append(",\n");
				else {
			...
			hsb.append(insertsb);
			hsb.append(updatesb);
			hsb.append("\tprotected static final String _DeleteSQL = \"delete from ")
				.append(finalTableCode)
				.append(" ")
				.append(pksb.toString())
				.append("\";\n\n");
			hsb.append("\tprotected static final String _FillAllSQL = \"select * from ")
				.append(finalTableCode)
				.append(" ")
				.append(pksb.toString())
				.append("\";\n\n");
	...
	private void generateSet(String tableCode) {
		...
		timeOperation("generateSet(" + finalTableCode + ")", () -> {
			...
			sb.append("\tpublic ")
				.append(finalTableCode)
				.append("Schema[] createSchemaSet(int initialCapacity){")
				.append("\n\t\treturn new ")
				.append(finalTableCode)
				.append("Schema[initialCapacity];\n")
				.append("\t}\n\n");
