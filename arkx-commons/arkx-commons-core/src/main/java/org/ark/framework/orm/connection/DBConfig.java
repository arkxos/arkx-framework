...
/**
 * @class org.ark.framework.orm.connection.DBConfig 配置示例 <database name="Default">
 * <Type>MYSQL</Type> <ServerAddress>localhost</ServerAddress> <Port>3306</Port>
 * <Name>bbs</Name> <UserName>root</UserName>
 * <Password>$KEYw//j8IH+fBtReFR4D88/Hw==</Password> <MaxConnCount>1000</MaxConnCount>
 * <InitConnCount>0</InitConnCount> <TestTable>article</TestTable> </database>
 * @author Darkness
 * @version 1.0
 * @since JDF 1.0
 */
...
public class DBConfig {
	...
	/**
	 * @tag category name = "Get Connection Info" color = "Blue"
	 */
	...
	static void loadDataBasesConfig() {
		...
		if (databases != null) {
			...
			for (int i = 0; i < dbs.size(); i++) {
				...
				ConnectionConfig database = null;// BeanCopyer.copy(ele,
													// XConnectionInfo.class);

				database.setPoolName(ele.attributeValue("name").trim());

				// database.setJdbcUrl(new StringFormat(database.getJdbcUrl(),
				// propsMap).toString());
				// database.setDBUserName(new StringFormat(database.getDBUserName(),
				// propsMap).toString());
				// database.setDBPassword(new StringFormat(database.getDBPassword(),
				// propsMap).toString());
				// database.setTestTable(new StringFormat(database.getTestTable(),
				// propsMap).toString());
				// database.setDBPassword(new StringFormat(database.getDBPassword(),
				// propsMap).toString());
	...
	public static void removeConnectionConfig(String poolName) {
		...
		// XmlUtil.write(Thread.currentThread().getContextClassLoader().getResource("xconnection.xml"),
		// doc);
	}
