...
public class Category extends TablesBasic {
    ...
    /// <summary>
    /// Extchar1name : 扩展字符属性1
    /// 扩展字符属性1，当一个字典本身并不仅是“键值+名称”对时，在此处需表明categoryvalue表中extchar1字段是有用的并提供给用户extchar1字段的输入框供维护（注意，当exttablename字段中存储了一个表名时，此extchar1name应为对应的exttablename表中的某个字段名），这种情况下，当用户维护本字典信息时，系统会同时更新exttablename表中的相应字段的信息
    /// 默认值 :
    /// 字段类型 : VARCHAR VARCHAR
    /// 字段特性 : 不可以为空 限制长度为50字节
    /// </summary>
    ...
    /// <summary>
    /// Extint1categoryid : 扩展数字属性1本身所引用的下拉选框ID
    /// 扩展数字属性1本身所引用的下拉选框ID（当extint1name字段本身所存储的是一个映射到某字典的值时，此处选择其绑定到的字典）
    /// 默认值 :
    /// 字段类型 : VARCHAR VARCHAR
    ...
    /// <summary>
    /// Extint2categoryid : 扩展数字属性2本身所引用的下拉选框ID
    /// 扩展数字属性2本身所引用的下拉选框ID，见extint1categoryid字段描述
    /// 默认值 :
    /// 字段类型 : VARCHAR VARCHAR
    /// 字段特性 : 不可以为空 限制长度为40字节
    /// </summary>
    ...
    /// <summary>
    /// Extint3categoryid : 扩展数字属性3本身所引用的下拉选框ID
    /// 扩展数字属性3本身所引用的下拉选框ID，见extint1categoryid字段描述
    /// 默认值 :
    /// 字段类型 : VARCHAR VARCHAR
    /// 字段特性 : 不可以为空 限制长度为40字节
    /// </summary>
    ...
    /// <summary>
    /// Refidorder : 非杂凑字典
    /// 是否使用refid的order（属于杂凑字典还是级别字典），如果此处值为1，则表明按照字典项键值的排序是有意义的（例如行政区划字典），否则是无意义的（例如系统配置项字典）
    /// 默认值 : 0
    /// 字段类型 : INT TINYINT
    /// 字段特性 : 不可以为空
    /// </summary>
    ...
    /// <summary>
    /// Refidint : 使用数字型键名
    /// 使用数字型键名，如果此处存储值为1，则表明存储于categoryvalue表的refid字段的值为一个int型的值，注意，如果客户提供的字典中有形如“01”的代码值，则此处应是“否”，即取值为“0”
    /// 默认值 : 0
    /// 字段类型 : INT TINYINT
    /// 字段特性 : 不可以为空
    /// </summary>
    ...
    /// <summary>
    /// Exttablename : 扩展存储表名
    /// 扩展存储表名（此表名不为空时，系统对extchar及extint将映射到相应的表中对应的字段上，此exttablename中应至少有id,chinaname,sortcode,indexid,delstatus字段，如果此字典的extchar1字段不为空，则该exttablename中应存在相应名称的字符型字段，对于extchar2……等字段也一样）
    /// 默认值 :
    /// 字段类型 : VARCHAR VARCHAR
    /// 字段特性 : 不可以为空 限制长度为50字节
    /// </summary>
    ...
    private String GetChildrenRefid(String m_Sortcode, boolean blnRecursion, boolean blnContainDeleted) {
        ...
        if (blnRecursion) {
            ...
            i = 0;
            m_where = m_where_delete + "categoryid='" + this.getId() + "' and sortcode like '" + m_Sortcode
                    + "%' and len(sortcode) = " + (m_Sortcode.length() + 5);
        }
    ...
    public String ValuesName(Object m_Refids, String m_SplitChar) {
        ...
        // {
        // m_ReturnValue += (m_ReturnValue.Length > 0 ? m_SplitChar : "") +
        // dt.Rows[i]["chinaname"].toString();
        // }
        // }
    ...
    public boolean UpdateRelation() {
        ...
        for (int i = 0; i < dt.getRowCount(); i++) {
            ...
            dr = SqlHelper.ExecuteDatarow("select id,chinaname,parentid,indexid,sortcode,delstatus from "
                    + this.getExttablename() + " where id=" + obj.getRefid().toString());

            if (dr == null) {
                args = "insert into " + this.getExttablename() + " (id,chinaname,parentid,indexid,sortcode,delstatus"
                        ...
                        + (this.getExtint3name().length() > 0 ? ("," + this.getExtint3name()) : "") + ") values ("
                        + obj.getRefid().toString() + ",'" + obj.getChinaname() + "',"
                        + (CategoryvalueViews.Child(obj.getParentid()).getRefid().length() > 0
                                ? CategoryvalueViews.Child(obj.getParentid()).getRefid()
                                : "-1")
                        + "," + obj.getIndexid() + ",'" + obj.getSortcode() + "'," + obj.getDelstatus()
                        + (this.getExtchar1name().length() > 0
                                ? (",'" + MyString.SqlEncode(obj.getExtchar1()) + "'")
                                : "")
                        + (this.getExtchar2name().length() > 0
                                ? (",'" + MyString.SqlEncode(obj.getExtchar2()) + "'")
                                : "")
                        + (this.getExtchar3name().length() > 0
                                ? (",'" + MyString.SqlEncode(obj.getExtchar3()) + "'")
                                : "")
                        + (this.getExtchar4name().length() > 0
                                ? (",'" + MyString.SqlEncode(obj.getExtchar4()) + "'")
                                : "")
                        + (this.getExtint1name().length() > 0
                                ? (",'" + MyString.SqlEncode(obj.getExtint1() + "") + "'")
                                : "")
                        + (this.getExtint2name().length() > 0
                                ? (",'" + MyString.SqlEncode(obj.getExtint2() + "") + "'")
                                : "")
                        + (this.getExtint3name().length() > 0
                                ? (",'" + MyString.SqlEncode(obj.getExtint3() + "") + "'")
                                : "")
                        + ")";
            } else {
                args = "update " + this.getExttablename() + " set " + "chinaname = '" + obj.getChinaname() + "',"
                        + "parentid = "
                        + (CategoryvalueViews.Child(obj.getParentid()).getRefid().length() > 0
                                ? ("'" + CategoryvalueViews.Child(obj.getParentid()).getRefid() + "'")
                                : "-1")
                        + "," + "indexid = " + obj.getIndexid() + "," + "sortcode = '" + obj.getSortcode() + "',"
                        + "delstatus = " + obj.getDelstatus()
                        + (this.getExtchar1name().length() > 0
                                ? ("," + this.getExtchar1name() + "= '" + MyString.SqlEncode(obj.getExtchar1()) + "'")
                                : "")
                        + (this.getExtchar2name().length() > 0
                                ? ("," + this.getExtchar2name() + "= '" + MyString.SqlEncode(obj.getExtchar2()) + "'")
                                : "")
                        + (this.getExtchar3name().length() > 0
                                ? ("," + this.getExtchar3name() + "= '" + MyString.SqlEncode(obj.getExtchar3()) + "'")
                                : "")
                        + (this.getExtchar4name().length() > 0
                                ? ("," + this.getExtchar4name() + "= '" + MyString.SqlEncode(obj.getExtchar4()) + "'")
                                : "")
                        + (this.getExtint1name().length() > 0
                                ? ("," + this.getExtint1name() + "= '" + MyString.SqlEncode(obj.getExtint1() + "")
                                        + "'")
                                : "")
                        + (this.getExtint2name().length() > 0
                                ? ("," + this.getExtint2name() + "= '" + MyString.SqlEncode(obj.getExtint2() + "")
                                        + "'")
                                : "")
                        + (this.getExtint3name().length() > 0
                                ? ("," + this.getExtint3name() + "= '" + MyString.SqlEncode(obj.getExtint3() + "")
                                        + "'")
                                : "")
    ...
    public String getAllSonRefid() {

        {
            ...
            DataTable dt = CategoryvalueViews.DataTables("categoryid='" + this.getId() + "' and len(sortcode)=5",
                    "sortcode");
            if (dt.getRowCount() == 0) {
                allSonRefid = "-1";
                return allSonRefid;
