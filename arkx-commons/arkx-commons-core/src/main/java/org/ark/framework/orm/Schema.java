		...
		try {
			...
			if (columnType == 12) {
				...
				if (value instanceof java.time.LocalDateTime) {
					...
					return Date
						.from(((java.time.LocalDateTime) value).atZone(java.time.ZoneId.systemDefault()).toInstant());
				}
				else if (value instanceof java.time.LocalDate) {
					LogUtil.debug("类型转换: 将 LocalDate 类型的值 '" + value + "' 转换为Date类型");
					return Date
						.from(((java.time.LocalDate) value).atStartOfDay(java.time.ZoneId.systemDefault()).toInstant());
				}
