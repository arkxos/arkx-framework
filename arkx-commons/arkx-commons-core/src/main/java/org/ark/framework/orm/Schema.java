...
/**
 * @class org.ark.framework.orm.Schema CRUD SQL 表编码 列信息 类包名 指定操作列【查询、更新】 是否操作列状态
 *
 *        插入 更新 删除 删除、插入？ 删除、备份 备份 获取备份schema
 *
 *        schema功能： 插入 删除 根据主键填充对象 更新 设置需要操作的列【填充】 查询所有结果集SchemaSet
 *        查询分页的结果集SchemaSet 获取备份的schema：系统应该有对应的备份表、备份类
 *
...
public abstract class Schema implements Serializable, Cloneable {
    ...
    /**
     * 表代码到Schema类的映射注册表 用于通过表代码动态创建对应的Schema实例
     */
    ...
    /**
     * 检查当前Session是否已经在事务中 无法直接检查事务状态，所以使用这个方法标记是否由外部管理事务
     */
    ...
    /**
     * @tag category name = "OP" color = "Blue"
     */
    ...
    /**
     * 执行更新或插入操作（upsert） 首先尝试更新记录，如果更新影响的行数为0（即没有找到匹配的记录），则执行插入操作
     *
    ...
    public SchemaUpsertEntity upsert() {
        ...
        try {
            ...
            // 如果这个表没有主键 则跳过随机验证
            SchemaColumn[] primaryKeyColumns = SchemaUtil
                    .getPrimaryKeyColumns(SchemaUtil.getPrimaryKeyColumns(getColumns()));
            // 必须有主键 才更新
    ...
    /**
     * 确保值类型与列定义类型匹配的安全转换方法 主要处理数据库VARCHAR字段返回数值类型的情况
     */
        ...
        try {
            ...
            if (columnType == 12) {
                ...
                if (value instanceof java.time.LocalDateTime) {
                    ...
                    return Date.from(
                            ((java.time.LocalDateTime) value).atZone(java.time.ZoneId.systemDefault()).toInstant());
                } else if (value instanceof java.time.LocalDate) {
                    LogUtil.debug("类型转换: 将 LocalDate 类型的值 '" + value + "' 转换为Date类型");
                    return Date.from(
                            ((java.time.LocalDate) value).atStartOfDay(java.time.ZoneId.systemDefault()).toInstant());
            ...
            if ((columnType == 1 || columnType == 10) && !(value instanceof String)) {
                ...
                LogUtil.debug("类型转换: 将 " + value.getClass().getName() + " 类型的值 '" + value + "' 转换为String类型");
                        ...
                        if (schema.bOperateFlag) {
                            for (int i = 0; i < schema.operateColumnOrders.length; ++i) {
                                ...
                                try {
                                    ...
                                    // 获取值前记录
                                    LogUtil.debug("准备获取列[" + columnName + "]值，结果集索引=" + rsIndex + ", 模型类型=" + columnType
                                            + ", JDBC类型=" + jdbcType + "(" + jdbcTypeName + ")");
                                    ...
                                    LogUtil.debug("获取到列[" + columnName + "]值: " + value + ", 实际类型=" + valueType);

                                    // 类型安全转换：确保值类型与列定义匹配
                                    value = ensureCorrectType(value, column);

                                    if (value != null && !value.getClass().getName().equals(valueType)) {
                                        LogUtil.debug("列[" + columnName + "]值类型已转换: " + valueType + " -> "
                                                + value.getClass().getName());
                                    }

                                    // 执行赋值前记录
                                    LogUtil.debug("准备设置对象[" + newSchema.getClass().getName() + "]属性，索引=" + columnOrder
                                            + ", 值类型=" + (value == null ? "null" : value.getClass().getName()));
                                    ...
                                    // 记录异常详情
                                    LogUtil.error("设置列值异常: 列索引=" + i + ", 列顺序=" + columnOrder + ", 列名="
                                            + column.getColumnName() + ", 值=" + value + ", 值类型="
                                            + (value != null ? value.getClass().getName() : "null") + ", 异常: "
                                            + e.getMessage());
                                    e.printStackTrace();
                                    // 继续处理其他列而不是直接抛出异常
                                    continue;
                            ...
                            for (int i = 0; i < schema.Columns.length; ++i) {
                                ...
                                try {
                                    ...
                                    // 执行赋值前记录
                                    LogUtil.debug("准备设置对象[" + newSchema.getClass().getName() + "]属性，索引=" + i + ", 值类型="
                                    ...
                                    // 记录异常详情
                                    LogUtil.error("设置列值异常: 列索引=" + i + ", 列名=" + column.getColumnName() + ", 值=" + value
    ...
    public SchemaSet<? extends Schema> querySet(Query queryBuilder, int pageSize, int pageIndex) {
        ...
        if ((queryBuilder != null) && queryBuilder.getSQL() != null
                && (!(queryBuilder.getSQL().trim().toLowerCase().startsWith("where")))) {
     ...
     * 根据不同数据库类型适配分页语法
     */
    private void adaptPaginationSyntax(Query queryBuilder, Connection conn, StringBuffer baseSql, int pageSize,
            int pageIndex) {
            ...
            case "SQLSERVER" :
                ...
                if (isNewVersion) {
                    ...
                    if (!baseQuery.toLowerCase().contains("order by")) {
                        ...
                        pagedSql = baseQuery + " ORDER BY " + orderColumn + " OFFSET " + offset + " ROWS FETCH NEXT "
                                + pageSize + " ROWS ONLY";
                    } else {
    ...
    private String generateDmPagingQuery(String baseQuery, int pageSize, int offset) {
        return "SELECT * FROM (SELECT ROWNUM RN, T.* FROM (" + baseQuery + ") T) " + "WHERE RN > " + offset
    ...
    private String generateOraclePagingQuery(String baseQuery, int pageSize, int offset) {
        return "SELECT * FROM (SELECT ROWNUM RN, T.* FROM (" + baseQuery + ") T WHERE ROWNUM <= " + (offset + pageSize)
                + ") WHERE RN > " + offset;
    }
    ...
    private String generateOldSqlServerPagingQuery(String baseQuery, int pageSize, int offset) {
        ...
        return "SELECT * FROM (SELECT ROW_NUMBER() OVER (" + (orderBy.isEmpty() ? "ORDER BY (SELECT 0)" : orderBy)
                + ") AS RowNum, * FROM (" + baseQuery + ") AS T) AS RowConstrainedResult " + "WHERE RowNum > " + offset
    ...
    private String generateDB2PagingQuery(String baseQuery, int pageSize, int offset) {
        ...
        return "SELECT * FROM (SELECT ROW_NUMBER() OVER (" + (orderBy.isEmpty() ? "ORDER BY 1" : orderBy)
                + ") AS RowNum, T.* FROM (" + baseQuery + ") AS T) AS RowConstrainedResult " + "WHERE RowNum BETWEEN "
                + (offset + 1) + " AND " + (offset + pageSize);
    }
    ...
    /**
     * 获取字段值 作为getV(String columnName)的别名方法
     *
     * @param fieldName
     *            字段名称
     * @return 字段值
    ...
    public <T extends Serializable> T querySet(String fieldQuery, String valueQuery, String orderBy, int pageSize,
