...
public class ColsremarkViews {
		...
		synchronized (mut) {
			if (children.size() <= getAll().getRowCount()) {
				...
				for (int i = 0; i < getAll().getRowCount(); i++) {
					...
					Add(all.getString(i, "tablename").toLowerCase() + "\r" + all.getString(i, "colname").toLowerCase(),
							(Colsremark) children.get(all.getString(i, "id").toLowerCase()));
					Add(all.getString(i, "tablename").toLowerCase() + "\r" + all.getString(i, "alias").toLowerCase(),
							(Colsremark) children.get(all.getString(i, "id").toLowerCase()));
				}
			}
		}
	...
	public static Colsremark Child(Object m_Tablename, Object m_Colname) {
		if (booleanUseCache == false) {
			return Find(
					"tablename='" + (m_Tablename.toString()) + "'" + " and colname='" + (m_Colname.toString()) + "'");
		}
	...
	public static Colsremark Find(String m_where) {
		if (booleanUseCache == false) {
			return new Colsremark(SqlHelper.ExecuteDatarow("select  top 1 * from Colsremark where "
					+ MyString.RuleSqlWhereClause(m_where) + " order by tablename,colorder"));
		}
