	...
	private Map<String, String> optimizeColumnTypes(Iterable<SchemaColumn> columns, String tableCode) {
		...
		for (SchemaColumn sc : columns) {
			...
			if (sc.getLength() >= LARGE_VARCHAR_THRESHOLD) {
				...
				log.info("表{}的超大列{}(长度:{})自动转换为{}类型",
						tableCode, sc.getColumnName(), sc.getLength(), textType);
		...
		log.warn("表{}的预估行大小({}字节)超出安全阈值({}字节)，需要转换部分列",
				tableCode, estimatedRowSize, SAFE_ROW_SIZE_THRESHOLD);
		...
		for (ColumnSizeInfo info : columnSizeInfos) {
			...
			String textType = determineTextType(sc.getLength());
			optimizedTypes.put(sc.getColumnName().toLowerCase(), textType);
			log.info("表{}的列{}(长度:{})转换为{}类型，节省约{}字节",
					tableCode, sc.getColumnName(), sc.getLength(), textType, savedSpace);
		...
		log.info("表{}的优化结果: 转换了{}个列，减少了{}字节，最终行大小约{}字节",
				tableCode, convertedColumns, reducedSize, finalRowSize);
