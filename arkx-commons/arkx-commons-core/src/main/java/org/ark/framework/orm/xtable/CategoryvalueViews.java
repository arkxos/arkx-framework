...
import java.util.Hashtable;
	...
	public  class CategoryvalueViews
	{
			...
			synchronized (mut) {
				...
				if (children.size() <= getAll().getRowCount())
				{
					...
					for (int i = 0; i < getAll().getRowCount(); i++)
					{
						...
						Add(all.getString(i, "categoryid").toLowerCase() + "\r" + all.getString(i, "refid").toLowerCase(), (Categoryvalue)children.get(all.getString(i, "id").toLowerCase()));
					}
				}
			}
		...
		public static Categoryvalue Child(Object m_Categoryid, Object m_Refid)
		{
			if (booleanUseCache == false)
			{
				return Find("categoryid='" + MyString.SqlEncode(m_Categoryid.toString()) + "'" + " and refid='" + MyString.SqlEncode(m_Refid.toString()) + "'");
			}
		...
		public static Categoryvalue Find(String m_where)
		{
			if (booleanUseCache == false)
			{
				return new Categoryvalue(SqlHelper.ExecuteDatarow("select  top 1 * from Categoryvalue where " + MyString.RuleSqlWhereClause(m_where) + " order by categoryid,sortcode"));
			}
