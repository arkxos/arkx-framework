...
public class Log {
    ...
    // / <returns></returns>
    public static boolean Add(Object operateUser, Object dutyUser, String m_OperateGlobalId, String tableName,
            Object rowId, String colName, Object oldValue, Object newValue) {
        if (ExistsDic(tableName, colName) == false)
            return false;
        String oldValueE = oldValue.toString().trim();
        String newValueE = newValue.toString().trim();
        if (ColInfos.Get(tableName, colName).getSimpleType() == SimpleTypes.DOUBLE
                || ColInfos.Get(tableName, colName).getSimpleType() == SimpleTypes.INT) {
        ...
        if (colName.length() > 1 && ColInfos.Get(tableName, colName).getSqlType() == SqlTypes.DATETIME
            ...
            return false;
                ...
                && (ColInfos.Get(tableName, colName).getSimpleType() == SimpleTypes.DOUBLE
                        || ColInfos.Get(tableName, colName).getSimpleType() == SimpleTypes.INT)
        ...
        String sql = "insert into operatelog (operateUser,dutyUser,OperateGlobalId,tableName,rowId,colName,oldValue,newValue,logTime) values ("
                + operateUser + "," + dutyUser + ",'" + m_OperateGlobalId + "','" + tableName + "','" + rowId.toString()
                + "','" + colName + "','" + oldValueE + "','" + newValueE + "','" + new Date() + "')";
