...
/**
 * @class org.ark.framework.schedule.CronMonitor
 * 
 * @author Darkness
 * @date 2013-1-31 下午12:21:30 
 * @version V1.0
 */
							...
							for (String id : tmap.keySet())
								try {
									if (CronMonitor.isOnTime(current, tm.getTaskCronExpression(id)))
										if (tm.isRunning(id)) {
											LogUtil.warn("Cron Task is on time ,but its last execution isn't finished!");
										} else {
											tm.execute(id);

											ExtendManager.invoke("org.ark.framework.AfterCronTaskExecutedAction", new Object[] { tm.getExtendItemID(), id });
										}
								} catch (Exception e) {
									e.printStackTrace();
								}
