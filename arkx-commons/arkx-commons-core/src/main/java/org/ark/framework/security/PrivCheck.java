...
/**
 * @class org.ark.framework.security.PrivCheck
 * 权限检测
 * 
 * @author Darkness
...
public class PrivCheck {
		...
		if (!check(m)) {
			try {
				...
				if (flag) {
					...
					noPrivPage = Config.getContextPath() + "NoPrivilege.zhtml?Method=" + m.getDeclaringClass().getName() + "." + m.getName() + "&login=" + priv.login() + "&loginType="
							+ priv.loginType() + "&userType=" + priv.userType() + "&value=" + priv.value();
				} else {
					noPrivPage = Config.getContextPath() + "NoPrivilege.zhtml?Method=" + m.getDeclaringClass().getName() + "." + m.getName();
				}
	...
	public static boolean check(Method m, RequestData request, ResponseData response) {
		...
		if (!check(m)) {
			...
			if (flag) {
				...
				if ((priv.loginType() == Priv.LoginType.User) && (!Account.isLogin())) {
					response.put("_ARK_SCRIPT", "window.location.href='" + Config.getContextPath() + Config.getLoginPage() + "';");
					return false;
				}
				noPrivPage = Config.getContextPath() + "NoPrivilege.zhtml?Method=" + m.getDeclaringClass().getName() + "." + m.getName() + "&login=" + priv.login() + "&loginType=" + priv.loginType()
	...
	public static boolean check(Method m) {
		...
		if (flag) {
			...
			return check(priv.login(), priv.loginType(), priv.userType(), priv.value(), parentPriv, Current.getPlaceHolderContext());
		}
		return false;
	}

	public static boolean check(boolean login, Priv.LoginType loginType, String userType, String priv, PlaceHolderContext context) {
	 ...
	 * 检查规则：
	 * 	1、不需要登录，返回true
	 * 	2、loginType 为 Priv.LoginType.User，User没有登录，返回false
	 * 	3、loginType 为 Priv.LoginType.Member，Member没有登录，返回false
	 * 
	 * @param login 是否需要登录
	 * @param loginType Priv.LoginType
	 * @author Darkness
	 * @date 2012-8-5 下午7:32:59 
	 * @version V1.0
	 */
	public static boolean check(boolean login, Priv.LoginType loginType, String userType, String priv, String parentPriv, PlaceHolderContext context) {
		if (!login) {
			return true;
		}
		if (login) {
