		...
		if (!create) {
			...
			if("oracle".equalsIgnoreCase(this.DBType.getExtendItemID())) {
				...
				for (SchemaColumn schemaColumn : scs) {
					this.list.add("COMMENT ON COLUMN "+tableCode+"."+schemaColumn.getColumnName()+" IS '"+schemaColumn.getMemo()+"'");
				}
			}
		}
	}
...
//		this.list.clear();
	...
	public String getAllSQL() {
		...
		for (int i = 0; i < this.list.size(); i++) {
			...
			if ((this.DBType.getExtendItemID().equalsIgnoreCase("MSSQL")) || (this.DBType.getExtendItemID().equalsIgnoreCase("SYBASE")))
				sb.append("\ngo\n");
			else {
				sb.append(";\n");
			}
	...
	public static String dropTable(String tableCode, IDBType dbType) {
		String dropSQL = null;
		if ((dbType.getExtendItemID().equalsIgnoreCase("MSSQL")) || (dbType.getExtendItemID().equalsIgnoreCase("SYBASE"))) {
