...
/**
 * @class org.ark.framework.jaf.spi.AliasLoader
 * 类信息加载器
 * 
 * @author Darkness
...
public class AliasLoader {
	...
	private static void scanAllAnnotation() {
		...
		if (lastTime == 0L) {
			LogUtil.info("----" + Config.getAppCode() + "(" + LangUtil.get(Config.getAppName()) + "): Scan Alias used " + (System.currentTimeMillis() - t) + " ms----");
		}
		lastTime = System.currentTimeMillis();
	}
	 ...
	 * 
	 * 忽略对象：
	 * 	1、"org.ark.framework"包下的对象忽略
	 * 	2、"App.ExcludeAliasScan"配置中排除的目录，排除目录用“，”分隔
	 * 
	 * @author Darkness
	 ...
	 * 
	 * 扫描提取规则：
	 * 	1、方法没有标注@Alias注解的，类标注了@Alias注解，
	 * 		将{类别名.方法名： 类名.方法名}存入AliasMapping列表中
	 * 
	 *	2、方法标注了@Alias注解，
	 * 		将{[类别名.]方法别名： 类名.方法名}存入AliasMapping列表中
	 * 
	 * @author Darkness
