...
import java.util.Hashtable;
	...
	public class TablesBasic
	{
		...
		public TablesBasic(String m_TableName, Object m_id)
		{
			...
			try
			{
				String sql = "select * from " + m_TableName + " where "
					+ TableStructList.Get(m_TableName).getPrimary() + "='" + m_id.toString() + "'";
		...
		public int GetInt(ColInfo m_ColInfo)
		{
			try
			{
				if (getL_DataRow() == null || getL_DataRow().get(m_ColInfo.getName()).toString() == null || getL_DataRow().get(m_ColInfo.getName()).toString().length() == 0)
				{
		...
		public String GetString(ColInfo m_ColInfo)
		{
			try
			{
				if (getL_DataRow() == null || getL_DataRow().getString(m_ColInfo.getName()) == null)// || L_DataRow[m_ColInfo.Name].toString().Length == 0)
				{
		...
		{
			if (m_ColInfo.getSimpleType() == SimpleTypes.DATETIME && m_ColInfo.getDefault().toLowerCase().equals("system.datetime.now.toString()"))
			{
		...
		/// <summary>
		/// 取得一列的String型值，此方法主要用于取得“对于采用inner join 方法产生的DataRow数据实例化本对象时，无法确定其ColInfo对象的col的值”
		...
		public String GetString(String m_colName)
		{
			...
			try
			{
				if (getL_DataRow() == null || getL_DataRow().getString(m_colName) == null || getL_DataRow().getString(m_colName).length() == 0)
				{
					return "";
				}
		...
		public double GetDouble(ColInfo m_ColInfo)
		{
			try
			{
				if (getL_DataRow() == null || getL_DataRow().getString(m_ColInfo.getName()) == null || getL_DataRow().getString(m_ColInfo.getName()).length() == 0)
				{
		...
		private String GetCategoryValuesName(ColInfo m_ColInfo , String m_SplitChar)
		{
			...
			return CategoryViews.Child(m_ColInfo.getObjRemark().getCategoryid()).ValuesName(this.GetString(m_ColInfo) , m_SplitChar);
		}
		...
		{
//			return MyDate.GetString(this.GetString(m_ColInfo) , (MyDate.FmtTypes)(m_ColInfo.getObjRemark().getFmttypetime()));
			return this.GetString(m_ColInfo);
		}
		...
		public  void AppendColValue(ColInfo m_ColInfo, Object m_newValue)
		{
			...
			{
				throw new RuntimeException("编码阶段产生的错误：表 " + this.getStruct().getName() + " 中不存在字段 " + m_ColInfo.getName() + " ！");
			}
			//判断所赋的值是否符合该字段的存储要求
			//未完成
			...
			{
				throw new RuntimeException("编码阶段或表单绑定阶段产生的错误：adder、addtime、moder、modtime这样的字段不需要进行额外赋值，系统会自动取当前时间及操作人员为其赋值 ！");
			}
			...
			if (m_ColInfo.getSimpleType() == SimpleTypes.INT && m_colValue.length() > 0 && m_ColInfo.getObjRemark().getCategoryid().length() > 0 && CategoryViews.Child(m_ColInfo.getObjRemark().getCategoryid()).getIsExists())
			{
				if (MyString.IsDouble(m_colValue) == false)
				{
					DataRow[] drs = CategoryViews.Child(m_ColInfo.getObjRemark().getCategoryid()).getDtValues().select("chinaname='" + m_colValue.replace(" " , "") + "'");
					if (drs.length > 0)
					{
			...
			if (m_ColInfo.getSqlType() == SqlTypes.TINYINT && (MyString.IsInt(m_colValue) == false || Integer.parseInt(m_colValue) < 0 || Integer.parseInt(m_colValue) > 255))
			{
				m_colValue = "0";
			}
		...
		/// 
		/// 此方法有待进一步加判断：例如，若为数字型，则插入值时无需 insert into table (intvalue) values ('3')，而只要insert into table (intvalue) values(3)
		/// 诸如此类的细化操作，这有助于更多数据类型的支持，例如money类型可能在数据库语句中不能使用 insert into table (moneyvalue) values ('3')，而只能是insert into table (moneyvalue) values (3)
		/// 
	    ...
	    private String SingleInsert()
	    {
	        ...
	        for(int i=0;i<Cols.length;i++)
	        {
	            if (LDataNew.contains(Cols[i]))
	            {
	                ...
	                {
	                    m_columns += "," + Cols[i];
//						m_values += ",'" + MyString.SqlEncode(LDataNew[Cols[i]].toString()) + "' as " + Cols[i];
...
//					+ " select isnull(max(" + this.getStruct().getPrimary().getName() + "),0)+1 as " + this.getStruct().getPrimary().getName() + "," + m_values + " from " + this.getStruct().Name;
	            m_idtmp = SqlHelper.ExecuteScalarString("select isnull(max(" + this.getStruct().getPrimary().getName() + "),0)+1 as " + this.getStruct().getPrimary().getName() + " from " + this.getStruct().getName());
	            sql = "insert into " + getStruct().getName()
	                + " (" + this.getStruct().getPrimary().getName() + "," + m_columns + ") values ("
	                + "'" + m_idtmp  + "'," + m_values + ")";
	        }
	        else if (LDataNew.contains(this.getStruct().getPrimary().getName())==false
	            && this.getStruct().getPrimary().getSqlType() == SqlTypes.VARCHAR
	            && this.getStruct().getPrimary().toString().length()>= 38 && this.getStruct().getPrimary().toString().length() <= 50)
	        {
	            m_idtmp = SqlHelper.getGUID();
	            //如果认为该主键varchar型guid值，并且该主键未赋值
//				sql = "insert into " + Struct.Name
//					+ " (" + this.getStruct().getPrimary().getName() + "," + m_columns + ")"
//					+ " select '" + MyString.SqlEncode(m_idtmp) + "' as " + this.getStruct().getPrimary().getName() + "," + m_values + " from " + this.getStruct().Name;
	            sql = "insert into " + getStruct().getName()
	                + " (" + this.getStruct().getPrimary().getName() + "," + m_columns + ") values ("
	                + "'" + m_idtmp  + "'," + m_values + ")";
	        }
...
//					+ " (" + m_columns + ") select " + m_values;
	        ...
	        {
	            SqlHelper.executeNonQuery("insert into tablesbasicDebug (tbl,sqls) values ('" + this.getStruct().getName() + "','" + MyString.SqlEncode(sql) + "')");
//				_ErrorInfo = "试图写入数据时出现错误，如果这个错误不是您预期的，请与开发人员联系！" + e.Source + "\r\n" + e.Message + "\r\n" + e.toString();
	        ...
	        if (m_idtmp.equals(""))
	        {
	            if (this.getStruct().getPrimary().getIsIncrement())
	            {
	                ...
	                && this.getStruct().getPrimary().getSimpleType()==SimpleTypes.INT)
	            {
	                m_idtmp = SqlHelper.ExecuteScalarString("select max(" + this.getStruct().getPrimary().getName() + ") from " + this.getStruct().getName()).toString();	
	            }
	        ...
	        if (m_idtmp.equals("") == false)
	        {
	            ...
	            _OldDataRow = SqlHelper.ExecuteDatarow("select * from " + getStruct().getName() + " where " + getStruct().getPrimary().getName() + " = '" + m_idtmp.toString() + "'");
	        }
	        return m_idtmp;
	    }
		...
		/// 
		/// 此方法有待进一步加判断：例如，若为数字型，则更新值时无需 update table set intvalue='3'，而只要update table set intvalue=3
		/// 诸如此类的细化操作，这有助于更多数据类型的支持，例如money类型可能在数据库查询语句中不能使用update table set moneyvalue='3'，而只能是update table set moneyvalue=3
		/// </summary>
	    ...
	    private boolean SingleUpdate()
	    {
			...
			if (getStruct().ExistsCol("rdeptid") && LDataNew.contains("rdeptid") == false && this.GetInt(this.getStruct().Col("rdeptid")) <= 0)
			{
	        ...
	        try
	        {
	            sql = "update " + getStruct().getName() + " set " + m_columns + " where " + getStruct().getPrimary().getName() + "='" + getL_DataRow().get(getStruct().getPrimary().getName()) + "'";
	            SqlHelper.executeNonQuery(sql);
	        }
	        catch(Exception e)
	        {
	        ...
	        for(int i=0;i<Cols.length;i++)
	        {
	            if (LDataNew.contains(Cols[i]))
	            {
	                Log.Add(getOperateUser(), getDutyUser(), m_OperateGlobalId, getStruct().getName(),getL_DataRow().get(getStruct().getPrimary().getName()),Cols[i],getL_DataRow().get(Cols[i]),LDataNew.get(Cols[i]));
	            }
	        }
	        Clear();
			lastUpdateSql = sql;
	        _OldDataRow = SqlHelper.ExecuteDatarow("select * from " + getStruct().getName() + " where " + getStruct().getPrimary().getName() + "='" + getL_DataRow().get(getStruct().getPrimary().getName()) + "'");
	        return true;
		}
		//#endregion
		...
		public boolean booleanNoConflict()
		{
			...
			if (LDataNew.contains(this.getStruct().getPrimary().getName()))
			{
				if (this.getStruct().getPrimary().getIsIncrement())
				{
					this._ErrorInfo = this.getStruct().getRemarkInfo().getAlias() + " 的 " + this.getStruct().getPrimary().getObjRemark().getAlias() + "(" + this.getStruct().getPrimary().getName() + ") 是自增长字段，不能对其进行赋值！";
					return false;
				}
				String nameString = this.getStruct().getPrimary().getName();
				if (!MyString.TrimHtml(nameString).equals(nameString))
				{
					this._ErrorInfo = this.getStruct().getRemarkInfo().getAlias() + " 的 " + this.getStruct().getPrimary().getObjRemark().getAlias() + "(" + this.getStruct().getPrimary().getName() + ") 不应包含特殊字符！";
					return false;
				}
				sql = this.getStruct().getPrimary().getName() + " = '" + LDataNew.get(this.getStruct().getPrimary().getName()).toString() + "' "
					+ (this.getL_DataRow() == null ? "" : (" and " + this.getStruct().getPrimary().getName() + " <> '" + getL_DataRow().get(this.getStruct().getPrimary().getName()).toString() + "'"));
				if (SqlHelper.DataRowExists(this.getStruct().getName(),sql))
				{
					this._ErrorInfo = this.getStruct().getRemarkInfo().getAlias() + " 的 " + this.getStruct().getPrimary().getObjRemark().getAlias() + "(" + this.getStruct().getPrimary().getName() + ") 的值冲突，当前提供的值是“" + LDataNew.get(this.getStruct().getPrimary().getName()).toString() + "”，该值在系统中已存在！";
					return false;
				}
			}
			//没有组合主键，则返回不冲突（true）
		...
		private boolean booleanPrimaryExNoConflict(String m_primaryEx)
		{
			...
			if (this.getL_DataRow() == null)
			{
				for (int i=0;i<primaryex.length;i++)
				{
					...
					{
						this._ErrorInfo = "向 " + this.getStruct().getRemarkInfo().getAlias() + " 添加数据时 " + this.getStruct().Col(primaryex[i]).getObjRemark().getAlias() + "(" + primaryex[i] + ") 的值需要提供，但现在未能提供！";
						return false;
					}
				}
			}
			...
			for (int i=0;i<primaryex.length;i++)
			{
				...
				if (LDataNew.contains(primaryex[i]))
				{
					...
					{
						this._ErrorInfo = this.getStruct().getRemarkInfo().getAlias() + " 的 " + this.getStruct().Col(primaryex[i]).getObjRemark().getAlias() + "(" + primaryex[i] + ") 的值中不允许为空，但您现试图将其清空！";
						return false;
					}
					//					if (LDataNew[primaryex[i]].toString().indexOf(" ")>=0)
					//					{
					//						this._ErrorInfo = "组合主键字段 " + this.getStruct().Col(primaryex[i]).ObjRemark.Alias + "(" + primaryex[i] + ") 的值中不能有空格，现在提供的值是“" + LDataNew[primaryex[i]].toString() + "”！";
					//						return false;
					//					}
					if (!MyString.TrimHtml(LDataNew.get(primaryex[i]).toString()).equals(LDataNew.get(primaryex[i]).toString()))
					{
						this._ErrorInfo = this.getStruct().getRemarkInfo().getAlias() + " 的 " + this.getStruct().Col(primaryex[i]).getObjRemark().getAlias() + "(" + primaryex[i] + ") 的值不应包含特殊字符！";
						return false;
					}
			...
			String m_where = sql;
			sql = m_where + (this.getL_DataRow() == null ? "" : (" and " + this.getStruct().getPrimary().getName() + " <> '" + getL_DataRow().get(this.getStruct().getPrimary().getName()).toString() + "'")) + (this.getStruct().ExistsCol("delstatus")? " and delstatus=0" : "");
			if (SqlHelper.DataRowExists(this.getStruct().getName(),sql))
			{
//				this._ErrorInfo = this.getStruct().RemarkInfo.Alias + "的组合主键字段 " + this.getStruct().RemarkInfo.Primaryex1 + " 的值冲突（不允许同时相同），您当前提供的是“" + m_where + "”！";
				
				//splash 2006-4-6
				this._ErrorInfo = this.getStruct().getRemarkInfo().getAlias() + " 的 “" + m_primaryExChs + "” 的值冲突（不允许同时相同），您当前提供的是“" + sqls + "”！";
...
//				{
//					if(objValue.Value.toString().trim().equals("") == false && Struct.Col(objValue.Key.toString()).ObjRemark.Openout > 0)
//					{
//						flag = false;
//					}
//				}
				...
				if(flag == false)
				{
					for(int i=0;i<Cols.length;i++)
					{
						...
						{
							if(getStruct().Col(Colname).getIsNullAble() == false && "".equals(LDataNew.get(Colname).toString().trim()))
							{
								Errors = Errors + "字段“" + this.getStruct().Col(getStruct().getCols()[i]).getObjRemark().getAlias() + "”的值没有填写，请填写！\r\n";
							}
							else
							{
								if(this.getStruct().Col(Colname).getObjRemark().getRequired() == 1 && "".equals(LDataNew.get(Colname).toString().trim()))
								{
									Errors = Errors + "字段“" + this.getStruct().Col(getStruct().getCols()[i]).getObjRemark().getAlias() + "”的值没有填写，请填写！\r\n";
								}
							}
...
//								if(LDataNew.ContainsKey(Colname))
//								{
//									if(Struct.Col(Colname).IsNullAble == false && LDataNew[Colname].toString() == String.Empty)
//									{
//										Errors = Errors + "字段“" + this.getStruct().Col(Colname).ObjRemark.Alias + "”的值没有填写，请填写！\n";
//									}
//									else
//									{
//										if(this.getStruct().Col(Colname).ObjRemark.Required == 1 && LDataNew[Colname].toString() == String.Empty)
//										{
//											Errors = Errors + "字段“" + this.getStruct().Col(Struct.Cols[i]).ObjRemark.Alias + "”的值没有填写，请填写！\n";
//										}
//									}
//								}
//							}
...
//			return true;
		...
		/// </summary>
		/// <param name="m_wheres">where 子句，例如"id=1 and lastlogintime>='2001-1-1'"，建议不包含"where"关键字</param>
		/// <param name="m_orders">order 子句，例如"id desc,lastlogintime asc"等，建议不包含"order by"关键字</param>
		/// <returns></returns>
...
//			Configrow config = ConfigrowViews.Child(this.getStruct().Name , this.PKValue , itemname);
//			if (config.IsExists)
//			{
//				return config;
//			}
//			Categoryvalue categoryvalue = CONSTCategory.DEF_CONFIG.Value(itemname);
//			if (categoryvalue.getIsExists() == false || categoryvalue.getExtchar2().toLowerCase().equals(this.getStruct().Name) == false)
//			{
//				this._ErrorInfo = "未能为表“" + this.getStruct().getName() + "”创建配置项“" + itemname + "”，因为系统未注册此配置项！";
//				return config;
//			}
...
//			return ConfigrowViews.Child(this.getStruct().getName() , this.getPKValue() , itemname);
//		}
