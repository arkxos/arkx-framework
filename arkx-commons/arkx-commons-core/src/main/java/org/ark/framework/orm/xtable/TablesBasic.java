...
public class TablesBasic {
    ...
    private String GetCategoryValuesName(ColInfo m_ColInfo, String m_SplitChar) {
        ...
        return CategoryViews.Child(m_ColInfo.getObjRemark().getCategoryid()).ValuesName(this.GetString(m_ColInfo),
                m_SplitChar);
    }
    ...
    public void AppendColValue(ColInfo m_ColInfo, Object m_newValue) {
        ...
        if (m_ColInfo.getSimpleType() == SimpleTypes.INT && m_colValue.length() > 0
            ...
            if (MyString.IsDouble(m_colValue) == false) {
                DataRow[] drs = CategoryViews.Child(m_ColInfo.getObjRemark().getCategoryid()).getDtValues()
    ...
    private String SingleInsert() {
        ...
        for (int i = 0; i < Cols.length; i++) {
            if (LDataNew.contains(Cols[i])) {
                if (m_columns != null) {
                    m_columns += "," + Cols[i];
                    // m_values += ",'" + MyString.SqlEncode(LDataNew[Cols[i]].toString()) + "' as "
                    // + Cols[i];
        ...
        if (m_idtmp.equals("")) {
            if (this.getStruct().getPrimary().getIsIncrement()) {
                ...
                m_idtmp = SqlHelper.GetInsertedPK(getStruct().getName()) + "";
                    ...
                    && this.getStruct().getPrimary().getSimpleType() == SimpleTypes.INT) {
                m_idtmp = SqlHelper.ExecuteScalarString("select max(" + this.getStruct().getPrimary().getName()
                        + ") from " + this.getStruct().getName()).toString();
    ...
    private boolean SingleUpdate() {
        ...
        Clear();
        lastUpdateSql = sql;
        _OldDataRow = SqlHelper.ExecuteDatarow(
                "select * from " + getStruct().getName() + " where " + getStruct().getPrimary().getName() + "='"
    ...
    private boolean booleanPrimaryExNoConflict(String m_primaryEx) {
        ...
        if (SqlHelper.DataRowExists(this.getStruct().getName(), sql)) {
            ...
            // this.getStruct().RemarkInfo.Primaryex1 + " 的值冲突（不允许同时相同），您当前提供的是“" + m_where
            // + "”！";
