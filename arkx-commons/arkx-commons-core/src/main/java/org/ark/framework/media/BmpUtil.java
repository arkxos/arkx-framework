...
public class BmpUtil {
    ...
    private static Image readMap1(FileInputStream fs, BitmapHeader bh) throws IOException {
        int nNumColors = 0;
        if (bh.nclrused > 0)
        ...
        for (int n = 0; n < nNumColors; n++) {
            npalette[n] = (0xFF000000 | (bpalette[(nindex1 + 2)] & 0xFF) << 16 | (bpalette[(nindex1 + 1)] & 0xFF) << 8
                    | bpalette[nindex1] & 0xFF);
            nindex1 += 4;
        }

        int scanLineSize = (bh.nwidth * bh.nbitcount + 31 & 0xFFFFFFE0) >> 3;

        int[] ndata1 = new int[bh.nwidth * bh.nheight];

        byte[] blinedata = new byte[scanLineSize];

        nindex1 = 0;
        for (int j1 = 0; j1 < bh.nheight; j1++) {
            fs.read(blinedata, 0, scanLineSize);
            nindex1 = 0;

            for (int i1 = 0; i1 < bh.nwidth; i1++) {
                if (bh.nwidth * (bh.nheight - j1 - 1) + i1 > bh.nwidth * bh.nheight - 1)
                    break;
                if (nindex1 > scanLineSize * bh.nheight - 1) {
                    break;
                }
                for (int pixPerByte = 0; pixPerByte < 8; pixPerByte++) {
                    int shift = 8 - pixPerByte - 1;
                    ndata1[(bh.nwidth * (bh.nheight - j1 - 1) + i1)] = npalette[(blinedata[nindex1] >> shift & 0x1)];
                    if (pixPerByte != 7) {
                        i1++;
                        if (i1 >= bh.nwidth) {
                            break;
        ...
        Image image = Toolkit.getDefaultToolkit()
                .createImage(new MemoryImageSource(bh.nwidth, bh.nheight, ndata1, 0, bh.nwidth));
        fs.close();
        return image;
    }
    ...
    protected static Image readMap32(FileInputStream fs, BitmapHeader bh) throws IOException {
        ...
        Image image = Toolkit.getDefaultToolkit()
                .createImage(new MemoryImageSource(bh.nwidth, bh.nheight, ndata, 0, bh.nwidth));
        fs.close();
        return image;
    }
