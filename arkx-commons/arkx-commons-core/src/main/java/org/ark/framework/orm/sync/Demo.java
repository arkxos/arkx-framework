...
//		// 初始化同步管理器 - 数据库到数据库模式
//		IncrementalSyncManager syncManager = new IncrementalSyncManager("sourceDb", "targetDb");
//
//		// 配置要同步的表
//		syncManager.registerTableConfig("USER_INFO", TableSyncConfig.basic("UPDATE_TIME", "USER_ID"));
//		syncManager.registerTableConfig("ORDER_DETAIL", TableSyncConfig.basic("MODIFY_TIME", "ORDER_ID"));
//
//		// 配置高级选项的表
...
//		SchemaSetImporter.ImportResult importResult = importer.importFromFile(dbFilePath);
//
//		log.info("从文件 {} 导入了 {} 个表, {} 条记录",
//				dbFilePath, importResult.getTotalTables(), importResult.getTotalRecords());
//
//		// 创建SchemaSet数据源的同步管理器
//		SchemaSetDataSource schemaSetDataSource = new SchemaSetDataSource("FileDataSource");
...
//
////			SchemaColumn[] primaryKeyColumns = SchemaUtil.getPrimaryKeyColumns(SchemaUtil.getPrimaryKeyColumns(schemaSet.getSchema()
////					.getColumns()));
////			//没有设置主键 默认第一个为主键
...
//				.primaryKeyField("USER_ID")
//				// 只同步指定字段
//				.includedFields(ImmutableSet.of("USER_ID", "USER_NAME", "EMAIL", "PHONE", "UPDATE_TIME"))
//				// 启用验证
...
//					result.getError() != null ? result.getError().getMessage() : result.getStatus());
//		}
//	}
//
//	/**
...
//
//		// 创建SchemaSet数据源
//		SchemaSetDataSource schemaSetDataSource = new SchemaSetDataSource("DynamicFileDataSource");
//
//		// 添加所有导入的SchemaSet
//		for (SchemaSet<?> schemaSet : importResult.getSchemaSets()) {
//			String tableCode = schemaSet.getSchema().getTableCode();
...
//			// 默认配置，实际使用时需要根据表结构调整
//			syncManager.registerTableConfig(tableCode, TableSyncConfig.basic("UPDATE_TIME", "ID"));
//		}
//
//		// 执行同步
//		SyncSummary summary = syncManager.syncAllTables();
...
//		SchemaSet<TestSchema> testData = createTestData(sourceTableName, recordCount);
//
...
//
//		log.info("开始串行处理...");
//		ParallelSyncProcessor serialProcessor = new ParallelSyncProcessor(1, recordCount, "default");
//		serialProcessor.addProgressListener(listener);
//		SyncResult serialResult = serialProcessor.processSchemaSet(sourceTableName, testData);
//
//		long serialDuration = System.currentTimeMillis() - startTimeSerial;
//		log.info("串行处理完成，耗时: {}ms, 处理记录数: {}", serialDuration, serialResult.getRecordsProcessed());
//
//		// 2. 并行处理（4线程）
//		long startTimeParallel4 = System.currentTimeMillis();
//
//		log.info("开始4线程并行处理...");
//		ParallelSyncProcessor parallelProcessor4 = new ParallelSyncProcessor(4, 500, "default");
//		parallelProcessor4.addProgressListener(listener);
//		SyncResult parallelResult4 = parallelProcessor4.processSchemaSet(sourceTableName, testData);
//
//		long parallel4Duration = System.currentTimeMillis() - startTimeParallel4;
//		log.info("4线程并行处理完成，耗时: {}ms, 处理记录数: {}", parallel4Duration, parallelResult4.getRecordsProcessed());
//
//		// 3. 并行处理（8线程）
//		long startTimeParallel8 = System.currentTimeMillis();
//
//		log.info("开始8线程并行处理...");
//		ParallelSyncProcessor parallelProcessor8 = new ParallelSyncProcessor(8, 500, "default");
//		parallelProcessor8.addProgressListener(listener);
//		SyncResult parallelResult8 = parallelProcessor8.processSchemaSet(sourceTableName, testData);
//
//		long parallel8Duration = System.currentTimeMillis() - startTimeParallel8;
//		log.info("8线程并行处理完成，耗时: {}ms, 处理记录数: {}", parallel8Duration, parallelResult8.getRecordsProcessed());
//
//		// 性能对比
//		log.info("性能对比:");
//		log.info("串行处理: {}ms", serialDuration);
//		log.info("4线程并行: {}ms (提升: {}%)", parallel4Duration, calculateSpeedup(serialDuration, parallel4Duration));
//		log.info("8线程并行: {}ms (提升: {}%)", parallel8Duration, calculateSpeedup(serialDuration, parallel8Duration));
//
//		// 关闭资源
...
//			schema.setUpdateTime(new Timestamp(baseTime.getTime() + i * 1000)); // 每条记录间隔1秒
//
...
//
//			@Override
//			public void onProgressUpdate(String tableCode, int processedItems, int totalItems) {
...
//
//			@Override
//			public void onChunkComplete(String chunkId, String tableCode, SyncStatus status) {
