...
public class CategoryViews {
        ...
        synchronized (mut) {
            ...
            if (children.size() <= getAll().getRowCount()) {
                ...
                for (int i = 0; i < getAll().getRowCount(); i++) {
                    ...
                    Add(all.getString(i, "constname").toLowerCase(),
                            (Category) children.get(all.getString(i, "id").toLowerCase()));
                    Add(all.getString(i, "chinaname").toLowerCase(),
                            (Category) children.get(all.getString(i, "id").toLowerCase()));
                }
            }
        }
    ...
    public static Category Find(String m_where) {
        if (booleanUseCache == false) {
            return new Category(SqlHelper.ExecuteDatarow("select  top 1 * from Category where "
                    + MyString.RuleSqlWhereClause(m_where) + " order by constname"));
        }
