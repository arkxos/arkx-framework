 ...
 * href="http://www.ietf.org/rfc/rfc1867.txt">RFC 1867</a>. Arbitrarily large amounts of data in the stream can be processed under constant
 * memory usage.
 * <p>
 ...
 * </code>
 * <p>
 * Note that body-data can contain another mulipart entity. There is limited support for single pass processing of such nested streams. The
 * nested stream is <strong>required</strong> to have a boundary token of the same length as the parent stream (see
...
public class MultipartStream {
	 ...
	 *            stream.
	 * @throws IllegalBoundaryException if the <code>boundary</code> has a different length than the one
	 ...
	 * <p>
	 * Headers are returned verbatim to the input stream, including the trailing <code>CRLF</code> marker. Parsing is left to the
	 * application.
	 * <p>
	 * 
	...
	/**
	 * <p>
	 * Reads <code>body-data</code> from the current <code>encapsulation</code> and writes its contents into the output <code>Stream</code>.
	 * <p>
	 * Arbitrary large amounts of data can be processed by this method using a constant size buffer. (see
	 * {@link #MultipartStream(InputStream,byte[],int) constructor}).
	 * 
