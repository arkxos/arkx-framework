...
/**
 * @class org.ark.framework.collection.DataTable 数据表格 使用方式： DataColumn dcUserName = new
 * DataColumn("UserName", DataColumn.STRING); DataColumn dcRealName = new
 * DataColumn("RealName", DataColumn.STRING); DataColumn dcStatusName = new
 * DataColumn("StatusName", DataColumn.STRING); DataColumn dcBranchInnercodeName = new
 * DataColumn("BranchInnercodeName", DataColumn.STRING); DataColumn dcRoleNames = new
 * DataColumn("RoleNames", DataColumn.STRING);
 *
 * DataTable dt = new DataTable(new DataColumn[] { dcUserName, dcRealName, dcStatusName,
 * dcBranchInnercodeName, dcRoleNames }, null); dt.insertRow(new Object[] { "darkness1",
 * "暗之幻影1", "1", "008", "admin" }); dt.insertRow(new Object[] { "darkness2", "暗之幻影2", "1",
 * "008", "admin" });
 * @author Darkness
...
public class DataTable implements Serializable, Cloneable, RandomAccess, Iterable<DataRow> {
	...
	/**
	 * 获取指定顺序的行上的指定顺序的字段的值，并转化为String。 如果isWebMode()为true，则会将null值和空字符串转为&amp;nbsp;返回
	 * @param rowIndex 行顺序
	 * @param columnIndex 字段顺序
	...
	/**
	 * 根据where条件过滤数据
	 * @param whereString 如：“ name='darkness' and password='sky' ”
	 * @return
	 */
