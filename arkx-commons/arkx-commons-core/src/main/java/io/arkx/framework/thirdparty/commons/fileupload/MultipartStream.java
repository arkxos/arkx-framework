...
public class MultipartStream {
    ...
    /**
     * The number of bytes, over and above the boundary size, to use for the keep
     * region.
     */
     ...
     * <p>
     * The boundary token of the nested stream is <code>required</code> to be of the
     ...
     * writes its contents into the output <code>Stream</code>.
     * <p>
     * Arbitrary large amounts of data can be processed by this method using a
     * constant size buffer. (see {@link #MultipartStream(InputStream,byte[],int)
     * constructor}).
     *
     * @param output
     ...
     * (i=0; i<head; i++) System.out.print(" "); System.out.println("h"); for (i=0;
     * i<tail; i++) System.out.print(" "); System.out.println("t");
     * System.out.flush(); } // Main routine, for testing purposes only. //
     * // @param args A String[] with the command line arguments. // @throws
     * Exception, a generic exception. public static void main( String[] args )
     * throws Exception { File boundaryFile = new File("boundary.dat"); int
     * boundarySize = (int)boundaryFile.length(); byte[] boundary = new
     * byte[boundarySize]; FileInputStream input = new
     * FileInputStream(boundaryFile); input.read(boundary,0,boundarySize); input =
     * new FileInputStream("multipart.dat"); MultipartStream chunks = new
     * MultipartStream(input, boundary); int i = 0; String header; OutputStream
     * output; boolean nextChunk = chunks.skipPreamble(); while (nextChunk) { header
     * = chunks.readHeaders(); System.out.println("!"+header+"!");
