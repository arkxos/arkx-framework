...
/**
 * An immutable representation of Money. This class was created to reference sums of
 * money. Due to rounding issues, the use of <code>float</code> and to a lesser extent
 * <code>double</code> are not recommended to represent money. Therefore money should be
 * represented by an integer type, which would require conversion from the more natural
 * floating point type. This class was created to handle all this conversion work and
 * provide a nice simple abstraction.
 *
...
public class Money implements Cloneable, Comparable<Money> {
	...
	/**
	 * Create a new <code>Money</code> instance from the given amount using the given
	 * currency and conversion value.
	 * @param currency the currency of the money.
	 * @param conversion the number to multiply the money to convert from a double to a
	 * long representation.
	...
	/**
	 * Create a new <code>Money</code> instance from the given amount using the default
	 * currency.
	 * @param amount the amount of money.
	 */
	...
	/**
	 * Return a clone of this object. Since <code>Money</code> is immutable, this function
	 ...
	 * @return the value <code>0</code> if the monies are equal; <code>1</code> if this
	 * money is more than the specified money or <code>-1</code> if this money is less
	 * than the specified money.
	 */
	@Override
	...
	/**
	 * Exchange this money for another currency at the given exchange rate. This function
	 * returns a new money object which contains the exchanged money in the given
	 * currency.
	...
	/**
	 * Return a <code>String</code> representation of the given amount of money in the
	 * default currency.
	...
	/**
	 * Create the number we need to multiple the currency to convert it to a long. E.g.
	 * for Euro, with two decimal digits, if we multiply it by 100, we can convert it to a
	 * long. We store the value internally as an integer to avoid rounding issues.
	 ...
	 * representation of money. The former representation is used for calculations as it
	 * performs no rounding and so does not loose precision. The latter is displayed to
	 * the user.
	 ...
	 * representation of money. The latter representation is used for calculations as it
	 * performs no rounding and so does not loose precision. The former is displayed to
	 * the user.
