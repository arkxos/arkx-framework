...
public class ObjectUtil {// NO_UCD
     ...
     *
     * @author Darkness
     * @date 2013-3-7 下午02:38:08
     * @version V1.0
     */
    public static boolean in(Object[] array1, Object[] array2) {
     ...
     * 1、等于null<br>
     * 2、equals("") <br>
     * 3、java.lang.Number的子类且其值等于0 4、数组、Collection、Map、DataTable元素个数等于0
     */
    ...
    public static <T> T[] filter(T[] arr, Filter<T> filter) {
        ...
        T[] a = (T[]) java.lang.reflect.Array.newInstance(arr.getClass().getComponentType(), list.size());
        int i = 0;
        for (T t : list) {
            a[i++] = t;
        }
     ...
     */
    public static <T> List<T> filter(List<T> c, Filter<T> filter) {
        if (c == null || c.size() == 0) {
            return c;
        }
        List<T> list = new ArrayList<T>(c.size());
    ...
    public static <T> T[] sort(T[] arr, Comparator<T> c) {
        ...
        @SuppressWarnings("unchecked")
        T[] a = (T[]) java.lang.reflect.Array.newInstance(arr.getClass().getComponentType(), arr.length);
        int i = 0;
        for (T t : arr) {
            a[i++] = t;
        }
    ...
    public static <T> List<T> sort(List<T> arr, Comparator<T> c) {
        ...
        try {
            @SuppressWarnings("unchecked")
            T[] a = (T[]) java.lang.reflect.Array.newInstance(arr.toArray().getClass().getComponentType(), arr.size());
            a = arr.toArray(a);
            Arrays.sort(a, c);
            @SuppressWarnings("unchecked")
    ...
    public static <K, V> Mapx<K, V> sort(Mapx<K, V> map, Comparator<Entry<K, V>> c) {
        ...
        try {
            @SuppressWarnings("unchecked")
            Entry<K, V>[] a = (Entry<K, V>[]) java.lang.reflect.Array.newInstance(Entry[].class.getComponentType(),
                    map.size());
            @SuppressWarnings("unchecked")
            Mapx<K, V> map2 = map.getClass().newInstance();
    ...
    public static <T> T[] copyOf(T[] arr, int length) {
        @SuppressWarnings("unchecked")
        T[] copy = (T[]) java.lang.reflect.Array.newInstance(arr.getClass().getComponentType(), length);
    ...
    public static <T> List<List<T>> groupBatch(List<T> datas, int batchSize) {
        List<List<T>> batchLists = new ArrayList<>();
        List<T> list = null;
        int i = 0;
        for (T data : datas) {
            if (i++ % batchSize == 0) {
    ...
    public static <T> void println(List<T> datas) {
        println(datas, 5);
    }

    public static <T> void println(List<T> datas, int count) {
        for (T data : first(datas, count)) {
            System.out.println(data);
        }
        System.out.println(".............................");
        System.out.println(".............................");
        System.out.println(".............................");
        for (T data : last(datas, count)) {
            System.out.println(data);
        }
    }

    public static <T> List<T> first(List<T> datas, int count) {
        if (count > datas.size()) {
            count = datas.size();
        }
    ...
    public static <T> List<T> last(List<T> datas, int count) {
        int from = datas.size() - count;
        if (from < 0) {
            from = 0;
        }
        return datas.subList(from, datas.size());
    }
}
