...
public class FastTable {
	...
	private void init(File tableFile) {
		...
		if (files != null) {
			for (File file : files) {
				...
				//
				// try {
				// RandomAccessFile raf = new RandomAccessFile(sectionFile.getPath(),
				// "rw");
	...
	public void insert(FastDataTable dataTable) {
		...
		IndexGroupFile indexGroupFile = new IndexGroupFile(
				this.tableDataFolderPath + File.separator + "index." + INDEX_GROUP_EXT);
		try {
		...
		if (totalRecordCount > freeCount) {
			...
			// FastDataTable dataTable2 = new FastDataTable(dataTable.getTableName(),
			// dataTable.getDataColumns());
			// dataTable2.insertRows(Arrays.asList(dataTable.getDataRows(sourceRecordIndex,
			// lastCount)));
	...
	public void rebuildIndex() {
		...
		for (FastColumn column : tableInfo.getIndexColumns()) {
			...
			// indexGroupFile = new IndexGroupFile(this.tableDataFolderPath +
			// File.separator
			// + "index." + INDEX_GROUP_EXT);
	...
	public <T extends IFastTable> T select(String filter, Class<T> tableType) {
		...
		FastTableFile tableFile = new FastTableFile(this.tableFilePath);
		FastTableHeader tableInfo = tableFile.readTableInfo();
		// System.out.println("load table file:" + (System.currentTimeMillis()-start) +
		// "ms");
		...
		if (filterInfo != null) {
			for (FastColumn column : tableInfo.getIndexColumns()) {
				if (filterInfo.needFilter(column.getName())) {
					start = System.currentTimeMillis();
					ColumnIndexFile columnIndexFile = new ColumnIndexFile(
							this.tableDataFolderPath + File.separator + column.getName() + "." + TABLE_INDEX_EXT);

					Multimap<Object, Long> indexPositions = columnIndexFile.readValueRowIndexes(column);
					System.out.println(
							"load index [" + column.getName() + "]:" + (System.currentTimeMillis() - start) + "ms");
					start = System.currentTimeMillis();
					...
					System.out.println(
							"filter index [" + column.getName() + "]:" + (System.currentTimeMillis() - start) + "ms");
				}
			}
		...
		// System.out.println("filter table ["+loadRecords.size()+"]:" +
		// (System.currentTimeMillis()-start) + "ms");
		start = System.currentTimeMillis();
		FastTableRecordFile dataTableFile = new FastTableRecordFile(tableRecordFile(1));
		T data = dataTableFile.readDataTable(tableInfo, tableType, filterInfo, loadRecords);
		// System.out.println("load filtered table ["+loadRecords.size()+"]:" +
		// (System.currentTimeMillis()-start) + "ms");
		return data;
	}
