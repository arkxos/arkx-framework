...
public abstract class AbstractLayer {
     ...
     *            已经解码帧头信息的帧头对象。
     * @param audio
     *            音频输出对象。如果指定为 <b>null</b> 则调用 {@link #outputPCM()}
     ...
     * 尝试输出解码结果。
     * <p>
     * 多相合成滤波输出的PCM数据写入缓冲区，当缓冲区至少填入解码4帧得到的PCM数据才产生一次输出， 但调用者并不需要知道当前缓冲区是否已经填入足够数据。
     * 防止缓冲区溢出，每解码最多4帧应调用本方法1次， 当然也可以每解码1帧就调用本方法1次。
     * <p>
     * 若产生音频输出将，将从缓冲区取走解码4帧得到的PCM数据。
     * <p>
     * <b>可能产生阻塞：</b>若音频输出已经停止，对本方法的调用将被阻塞，直到开始音频输出。如果输入流解码完，阻塞被自动清除。
     *
     * @see #startAudio()
     */
    protected int outputPCM() {
        synchronized (audioLock) {
            // if (debug1)
            // System.out.printf("outputPCM(1): thread=%s,
            ...
            if ((readCursor += w) >= pcmbuf.length)
                resetOffset();
            // if (debug1)
            // System.out.printf("outputPCM(2), thread=%s,
     ...
     *
     * @param interrupted
     *            是否需要以立即方式关闭。若为 <b>false</b>，等待音频输出缓冲区的全部内容刷向音频输出对象后返回；若为
     *            <b>true</b> 以立即方式返回。
     */
