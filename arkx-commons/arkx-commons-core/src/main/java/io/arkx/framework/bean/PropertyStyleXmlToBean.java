        ...
        if (childs.size() > 0) {
            for (String key : childs.keySet()) {
                ...
                if (childList.size() > 0) {
                    ...
                    Method addChildMethod = ReflectionUtil.getDeclaredMethod(bean, addChildMethodName,
                            childList.get(0).getClass());
                    if (addChildMethod != null) {
                        for (Object childBean : childList) {
                            ReflectionUtil.invokeMethod(bean, addChildMethodName,
                                    new Class[]{childList.get(0).getClass()}, new Object[]{childBean});
                        }
                    }
                }
            }
