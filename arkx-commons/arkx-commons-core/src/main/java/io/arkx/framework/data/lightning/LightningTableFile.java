...
public class LightningTableFile extends MappedFile {
     ...
     * @since infinity 1.0
     */
    protected static <T extends ILightningTable> int caculateTableInfoLength(String tableName,
            LightningColumn[] columns) {
        int headerLength = 0;
     ...
     * @since infinity 1.0
     */
    public static <T extends ILightningTable> ByteBuffer buildTableInfoByteBuffer(String tableName,
            LightningColumn[] columns, int rowCount) {
    ...
    public <T extends ILightningTable> void save(T dt) {
        ...
        try {
            ...
            if (fileLength == 0) {// 文件不存在，写入文件头
                ...
                MappedByteBuffer mappedByteBuffer = fileChannel.map(FileChannel.MapMode.READ_WRITE, rowCountIndex,
                        LONG_LENGTH);
