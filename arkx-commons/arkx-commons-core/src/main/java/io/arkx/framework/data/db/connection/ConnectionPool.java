					...
					if (!conn.longTimeFlag) {
						if (now - conn.lastSuccessExecuteTime > dcc.MaxConnUsingTime) {
							...
							try {
								...
								conns[i] = conn;
								LogUtil.info(dcc.PoolName + ":create a new connection,total is " + dcc.ConnCount
										+ " line 215");
								setCaller(conn);
								...
								e.printStackTrace();
								throw new DatabaseException("ConnectionPool," + dcc.PoolName
										+ "create new connection failed:" + e.getMessage());
							}
						}
					}
			...
			if (dcc.ConnCount < dcc.MaxConnCount) {
				try {
					...
					throw new DatabaseException(
							"ConnectionPool," + dcc.PoolName + ":create new connection failed:" + e.getMessage());
				}
			}
			else {
