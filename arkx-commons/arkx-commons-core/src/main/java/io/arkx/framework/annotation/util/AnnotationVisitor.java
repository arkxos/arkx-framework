...
public class AnnotationVisitor implements IBuiltResourceVisitor {
    ...
    public boolean match(BuiltResource br) {
        ...
        if ((!fullName.endsWith("BaseUIFacade.class") && fullName.indexOf(FRAMEWORK) >= 0)
                || fullName.indexOf(SCHEMA) >= 0) {
            return false;
        }
        return fullName.endsWith("BaseUIFacade.class") || fullName.endsWith("UI.class")
                || fullName.endsWith("Method.class") || fullName.indexOf("UI$") > 0;
    }
            ...
            for (i = 0; i < node.visibleParameterAnnotations.length; i++) {
                ...
                if (list != null) {
                    for (Iterator<AnnotationNode> iterator1 = list.iterator(); iterator1.hasNext();) {
                        ...
                        if (an != null && an.desc != null && an.desc.equals(paramAnnoation) && an.values != null
                                && an.values.size() > 1) {
