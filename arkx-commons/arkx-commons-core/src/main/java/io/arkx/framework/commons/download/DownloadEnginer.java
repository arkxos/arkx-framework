...
public class DownloadEnginer {
	...
	public static DownloadMission recoverMissionFromProgressFile(String progressFilePath) throws IOException {
		try {
			...
			File targetSaveFile = new File(
					FileUtils.getSafeDirPath(mission.folder + File.separator + mission.fileName));
			...
			for (RemoteHttpPartFile runnable : mission.getDownloadWorkers()) {
				recoveryRunnableInfos.add(new RecoveryWorkerInfo(runnable.getStartPosition(),
