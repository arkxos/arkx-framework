...
/**
 * An immutaable representation of Currency. This class represents a currency, such as the
 * Australian Dollar or the British Pound, rather than an acutal amount of money. We use
 * our own representation because the Java API's Currency class has no way of returning a
 * NumberFormat object. So we could not simply pass a Currency object to our Money object
 * and have the Money object be able to print properly. We would have to pass both the
 * locale and the currency around, which is heavy handed.
 *
...
public class Currency implements Cloneable, Comparable<Currency> {
	...
	/**
	 * Create an instance of the currency associated with the given locale. If the given
	 * locale has no associated currency, e.g. Antartica, this function will default to
	 * the US dollar.
	...
	/**
	 * Create an an instance of the currency using the currency described by the given ISO
	 * 4217 currency code.
	...
	/**
	 * Return the number of fraction digits used to display this currency. For examle, the
	 * Australian Dollar has 2 fraction digits (e.g. $12.95) while the Japanese Yen has
	 * zero (e.g. 100 yen).
	...
	/**
	 * Compare this currency to the given object. If the given object is not a currency,
	 * it will throw a <code>ClassCastException</code>. Currencies are ordered
	 * alphabetically by their ISO 4217 currency codes.
	 * @param object currency object to be compared
	 * @return 0 if the two objects are identical, -1 if the argument currency comes
	 * alphabetically after this currency, 1 if the argument currency comes alphabetically
	...
	/**
	 * Compare this currency to the given object. If the given object is not a currency,
	 * it will throw a <code>ClassCastException</code>.
	 * @param object currency object to be compared
