...
public class Oracle extends AbstractDBType {
    ...
    public java.sql.Connection createConnection(ConnectionConfig dcc) throws SQLException, ClassNotFoundException {
        ...
        return DriverManager.getConnection(StringUtil.isEmpty(dcc.ConnectionURL) ? getJdbcUrl(dcc) : dcc.ConnectionURL,
                props);
    ...
    public void setBlob(Connection conn, PreparedStatement ps, int i, byte[] v) throws SQLException {
        ...
        try {
            Method m = blobClass.getMethod("createTemporary",
                    new Class[]{java.sql.Connection.class, boolean.class, int.class});
    ...
    public void setClob(Connection conn, PreparedStatement ps, int i, Object v) throws SQLException {
        ...
        Object oc = conn.getPhysicalConnection();
        try {
            Method m = clobClass.getMethod("createTemporary",
                    new Class[]{java.sql.Connection.class, boolean.class, int.class});
    ...
    public String[] toSQLArray(AddColumnCommand c) {
        String sql = "alter table " + c.Table + " add " + c.Column + " "
    ...
    public String[] toSQLArray(ChangeColumnLengthCommand c) {
        String sql = "alter table " + c.Table + " modify " + c.Column + " "
                + toSQLType(DataTypes.valueOf(c.DataType), c.Length, c.Precision);
        if (c.Mandatory) {
            sql += " not null";
        }
    ...
    public boolean isTableExist(String databaseName, String tableName) {
        ...
        String sql = "SELECT lower(table_name) AS tablename FROM user_all_tables where lower(table_name) ='"
