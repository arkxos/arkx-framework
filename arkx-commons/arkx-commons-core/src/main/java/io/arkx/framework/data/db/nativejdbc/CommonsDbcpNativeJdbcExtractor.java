...
package io.arkx.framework.data.db.nativejdbc;
...
/**
 * 移植自Spring：<br>
 * Implementation of the {@link NativeJdbcExtractor} interface for the Jakarta Commons
 * DBCP connection pool, version 1.1 or higher.
 * <p>
 * Returns the underlying native Connection, Statement, etc to application code instead of
 * DBCP's wrapper implementations. The returned JDBC classes can then safely be cast, e.g.
 * to <code>oracle.jdbc.OracleConnection</code>.
 * <p>
 * This NativeJdbcExtractor can be set just to <i>allow</i> working with a Commons DBCP
 * DataSource: If a given object is not a Commons DBCP wrapper, it will be returned as-is.
 * <p>
 * Note that this version of CommonsDbcpNativeJdbcExtractor will work against the original
 * Commons DBCP in <code>org.apache.commons.dbcp</code> as well as against Tomcat 5.5's
 * relocated Commons DBCP version in the <code>org.apache.tomcat.dbcp.dbcp</code> package.
 *
...
public class CommonsDbcpNativeJdbcExtractor {
	...
	/**
	 * Extracts the innermost delegate from the given Commons DBCP object. Falls back to
	 * the given object if no underlying object found.
