 ...
 * 树形结构中的一个节点
 *
 * // 创建根节点 TreeNode<String> root = new TreeNode<>("Root");
 *
 * // 添加子节点 TreeNode<String> child1 = new TreeNode<>("Child1"); TreeNode<String> child2 =
 * new TreeNode<>("Child2"); root.addChild(child1); root.addChild(child2);
 *
 * // 添加孙子节点 child1.addChild(new TreeNode<>("Grandchild1")); child1.addChild(new
 * TreeNode<>("Grandchild2"));
 *
 * // 打印树结构 root.printTree();
 *
 * 输出： └── Root │ └── Child1 │ │ └── Grandchild1 │ │ └── Grandchild2 │ └── Child2
 *
 *
 * // 前序遍历 System.out.println("前序遍历: " + root.preorderTraversal()); // [Root, Child1,
 * Grandchild1, Grandchild2, Child2]
 *
 * // 后序遍历 System.out.println("后序遍历: " + root.postorderTraversal()); // [Grandchild1,
 * Grandchild2, Child1, Child2, Root]
 *
 * // 深度查询 System.out.println("树深度: " + root.getDepth()); // 2
 *
 * @author Nobody
...
public class TreeNode<K, T> implements Serializable, Comparable<TreeNode<K, T>> {
	...
	public String toString() {
		return "TreeNode{" + "value=" + value + ", children=" + children.size() + '}';
	}
