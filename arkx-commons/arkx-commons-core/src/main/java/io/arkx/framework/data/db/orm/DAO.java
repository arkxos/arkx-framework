...
import io.arkx.framework.config.RWSpliting;
import io.arkx.framework.config.ReadOnlyDB;
...
public abstract class DAO<T extends DAO<T>> {
	...
	public boolean insert() {
		...
		try {
			pstmt = conn.prepareStatement(__metadata.getInsertSQL(db), ResultSet.TYPE_FORWARD_ONLY,
			...
			for (DAOColumn sc : __metadata.getColumns()) {
				if (sc.isMandatory()) {
					if (__metadata.getV(this, sc.getColumnName()) == null) {
						String message = "Error:" + __metadata.getTable() + "'s column " + sc.getColumnName()
								+ " cann't be null";
						// 输出SQL错误日志
	...
	public boolean update() {
		...
		if (__dataAccess.getConnection().getDBConfig().isSybase()) {// todo:
																	// Sybase下的临时措施，将来要改掉
		...
		try {
			...
			for (i = 0; i < columns.length; i++) {
				...
				if (sc.isPrimaryKey()) {
					...
					if (v == null) {
						String message = __metadata.getTable() + "'s primary key column " + sc.getColumnName()
								+ " can't be null";
	...
	public int delete2() {
		...
		IDBType db = DBTypeService.getInstance().get(__dataAccess.getConnection().getDBConfig().DBType);
		try {
			pstmt = conn.prepareStatement(__metadata.getDeleteSQL(db), ResultSet.TYPE_FORWARD_ONLY,
					ResultSet.CONCUR_READ_ONLY);
			for (int i = 0, j = 0; i < columns.length; i++) {
				...
				if (sc.isPrimaryKey()) {
					...
					if (v == null) {
						String message = "Error:" + __metadata.getTable() + "'s primary key column "
	...
	public String backup(String backupOperator, String backupMemo) {
		...
		Connection conn = __dataAccess.getConnection();
		IDBType db = DBTypeService.getInstance().get(__dataAccess.getConnection().getDBConfig().DBType);
		// backupOperator = StringUtil.isEmpty(backupOperator) ? Account.getUserName() :
		// backupOperator;
	...
	public boolean deleteAndBackup(String backupOperator, String backupMemo) {
		...
		// backupOperator = ObjectUtil.empty(backupOperator) ? Account.getUserName() :
		// backupOperator;
		...
		try {
			...
			for (int i = 0, j = 0; i < columns.length; i++) {
				...
				if (sc.isPrimaryKey()) {
					Object v = getV(i);
					if (v == null) {
						// String message = "Error:" + __metadata.getTable() + "'s primary
						// key column "
						// + sc.getColumnName()
						// + " can't be null";
	...
	private void setVAll(Connection conn, DAO<?> dao, ResultSet rs) throws Exception {
		...
		if (__operateColumnFlag) {
			for (int order : __operateColumnOrders) {
				...
				Object v = db.getValueFromResultSet(rs, index, DataTypes.valueOf(columns[order].getColumnType()),
						latin1Flag);
				dao.setV(order, v);
			...
			for (int i = 0; i < columns.length; i++) {
				...
				Object v = db.getValueFromResultSet(rs, index, DataTypes.valueOf(columns[i].getColumnType()),
						latin1Flag);
				dao.setV(i, v);
				dao.__oldValues[i] = v;
			}
