package io.arkx.framework.data.excel.writer;
...
import io.arkx.framework.data.excel.Const;
...
public class Excel2003Writer extends ExcelWriter {
     ...
     * @param Border
     *            边框 如：HSSFCellStyle.WHITE.index
     * @param foregroundColor
     *            前置背景色 如：CellStyle.BORDER_THIN
     * @param fillPattern
     *            填充模式 如：CellStyle.SOLID_FOREGROUND
     * @return
     */
    @Override
    public CellStyle createCellStyle(Workbook workbook, Font font, HorizontalAlignment alignment,
            VerticalAlignment verticalAlignment, BorderStyle Border, short foregroundColor,
     ...
     *            表体数据集
     * @param headerNum
     *            表头行数(从第一行开始)，大于零则有表头，否则无表头只有表体
     */
    @Override
    public Workbook writeWorkbook(Workbook workbook, String sheetTitle, List<List<String>> tableDataList,
            int headerNum) {
        if (workbook == null)
            workbook = new HSSFWorkbook();
        int exportRecordNum = tableDataList.size(); // 导出的总记录数
        int tableStartRowNum = headerNum > 0 ? headerNum : 0; // 表体数据开始行数
        int sheetNum = exportRecordNum / (Const.EXCEL_MAX_EXPORT_NUM - headerNum) + 1; // 工作表的页数
        ...
        columnStyle.setFillBackgroundColor(HSSFColorPredefined.GREEN.getIndex());
        ...
        if (tableDataList.size() > (tableStartRowNum + 1)) {
            for (int i = tableStartRowNum; i < exportRecordNum; i++) {
                ...
                HSSFRow row = null;
                int currentSheet = i / Const.EXCEL_MAX_EXPORT_NUM; // 当前工作表的页数
                HSSFSheet sheet = sheetList.get(currentSheet);
                int rowIndex = i - Const.EXCEL_MAX_EXPORT_NUM * currentSheet;
     ...
     * @param workbook
     * @param mutilSheetMap
     *            key为sheet名称sheetTitle， value为表体数据tableDataList
     * @return
     */
    @Override
    public Workbook writeWorkbookMutilDatasoure(Workbook workbook, Map<String, List<List<String>>> mutilSheetMap,
            int headerNum) {
        if (workbook == null)
            workbook = new HSSFWorkbook();
        if (!ObjectUtil.isEmpty(mutilSheetMap)) {
