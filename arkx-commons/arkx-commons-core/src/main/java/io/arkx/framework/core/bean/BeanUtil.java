...
public class BeanUtil {
        ...
        for (Object k : values.keySet()) {
            if (k != null) {
                ...
                if (keyPrefix != null && key.startsWith(keyPrefix)) {// 根据前缀重组map
                    map.put(key.substring(keyPrefix.length()), values.get(k));
                }
            }
        }
    ...
    public static Object create(Class<?> type, Class<?> paramTypes) {// NO_UCD
        try {
            ...
            if (c == null) {
                throw new BeanInitException("No constructor with parameters [" + StringUtil.join(paramTypes)
                        + "] in Bean class " + type.getName() + " n!");
            }
    ...
    /**
     * 将各个源Bean实例中的各个属性的值逐个复制到目标Bean实例中的属性上。 目标Bean中没有但源Bean中有的属性直接忽略。
     *
     * @param srcBean
     *            源Bean实例
     * @param targetBean
     ...
     *            Bean实例
     * @param pairFlag
     *            是否只转对称的属性（即canRead()和canWrite()同时为true）
     */
    ...
    private static Object convert(Object v, boolean pairFlag, HashMap<Object, Object> convertedMap) {
        ...
        if (v.getClass().isArray()) {
            ...
            for (Entry<?, ?> e : ((Map<?, ?>) v).entrySet()) {
                map.put(e.getKey() == null ? null : e.getKey().toString(),
                        convert(e.getValue(), pairFlag, convertedMap));
            }
        } else {
