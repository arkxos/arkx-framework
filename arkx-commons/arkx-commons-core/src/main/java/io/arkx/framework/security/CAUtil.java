...
public class CAUtil {
    ...
    // String issuerDN, int limit)
    // throws Exception
    // {
    // Date today = new Date(System.currentTimeMillis());
    // Date endDay = DateUtil.addMonth(today, limit);
    // SubjectPublicKeyInfo info = new SubjectPublicKeyInfo(
    // new DefaultSignatureAlgorithmIdentifierFinder().find(pubKey.getAlgorithm()),
    // pubKey.getEncoded());
    //
    // X509v3CertificateBuilder builder = new X509v3CertificateBuilder(new
    // X500Name(issuerDN),
    // BigInteger.valueOf(System.currentTimeMillis()), today, endDay, new
    // X500Name(issuerDN), info);
    //
