...
/**
 *
 * // Override if there is a preference setting for another language String languageCode =
 * PreferencesManager.getLanguageCode(); Locale.setLocale(languageCode);
 *
 * This class implements internationalisation support for Venice. Each text message
 * displayed by Venice is represented by a string symbol (e.g. "OPEN") which describes the
 * error message being displayed. This symbol is then matched to a internationalistaion
 * file which will then retrieve the local language version of that error message.
 * <p>
 * If a match could not be found in the current language, it will try to match the symbol
 * in the english (default) language. If it could not find a match there, it will display
 * the string <code>Locale.UNKNOWN</code>.
 * <p>
...
public class Locale {
	...
	private static synchronized void loadResourceBundles() {
		if (!resourceBundlesLoaded) {
			...
			try {
				...
				if (locale == null) {
					primaryResourceBundle = ResourceBundle.getBundle("i18n.evolution", java.util.Locale.CHINESE);
				}
				else {
					primaryResourceBundle = ResourceBundle.getBundle("i18n.evolution", locale);
				}
			...
			try {
				secondaryResourceBundle = ResourceBundle.getBundle("i18n.evolution", java.util.Locale.ENGLISH);
			}
			catch (Exception e) {
	...
	/**
	 * Set the localization as got from saved preferences. If no preference is set for the
	 * language it get the current system language.
	 */
	...
	/**
	 * Return the current language translation of the text associated with the given key.
	 * Insert the given argument into the text translation. For example if the text object
	 * in the internationalisation file looks like:
	 *
	 * <pre>
	 * Generation % 1
	 * </pre>
	...
	/**
	 * Return the current language translation of the text associated with the given key.
	 * Insert the given arguments into the text translation. For example if the text
	 * object in the internationalisation file looks like:
	 *
	 * <pre>
	 * Generation %1 of %2
	 * </pre>
