...
public interface IAudio {
	 ...
	 * 如果调用者试图写入多于当前可写入数据量的数据，则此方法在 写入所请求数据量之前一直阻塞。即使要写入的请求数据量大于数据行的缓冲区大小 ，此方法也
	 * 适用。不过，如果在写入请求的数据量之前数据行已关闭、停止或刷新，则该方法不再阻塞。
	 * <p>
	 * 本方法应该由管理PCM缓冲区的{@link AbstractLayer#outputPCM()}方法调用。
	 * @param b 待写入的PCM源数据。
	 * @param off 源数据b的偏移量。
	 * @param size 写入的数据长度。
	 ...
	 * <p>
	 * 初始状态为向混频器器写入数据，首次调用后将暂停向混频器器写入数据。
