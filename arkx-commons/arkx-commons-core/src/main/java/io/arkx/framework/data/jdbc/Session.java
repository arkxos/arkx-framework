...
public class Session {
    ...
    public <T extends Entity> int save(List<T> entities) {
        ...
        if (BaseEntity.class.isAssignableFrom(entities.get(0).getClass())) {
            sortOrder = new JdbcTemplate(getConnection()).executeLong(
                    "SELECT max(SORT_ORDER) FROM " + EntityAnnotationManager.getTableName(entities.get(0).getClass()));
        }
    ...
    public <T extends Entity> int delete(String tableName, String ids) {
        ...
        // ExtendManager.invoke(EntityDeleteExtendAction.ExtendPointID, new
        // Object[]{tableName, ids});
