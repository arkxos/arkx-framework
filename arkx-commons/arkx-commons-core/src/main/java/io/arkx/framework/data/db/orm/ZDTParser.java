...
public class ZDTParser {
	...
	public void parse() {
		...
		try {
			...
			for (ZDTTableInfo ti : Tables) {
				if (VERSION_1.equals(Version) || VERSION_2.equals(Version)) {
					DataTable dt = readOneTable(ti, braf, ti.Positions.get(ti.Positions.size() - 1));
					ti.RowCount = (ti.Positions.size() - 1) * DBExporter.PageSize + dt.getRowCount();
				}
			}
		}
		catch (Exception e1) {
	 ...
	 * @throws Exception
	 */
	private static DataTable readOneTable(ZDTTableInfo ti, RandomAccessFile braf, long pos) throws Exception {
		braf.seek(pos);
		byte[] bs = new byte[4];
		braf.read(bs);
		int len = NumberUtil.toInt(bs);
	...
	private static Object[] getOneRow(DAOColumn[] scs, String str) {
		...
		for (String v : arr2) {
			...
			else if (ObjectUtil.in(dataTypes, DataTypes.STRING, DataTypes.CLOB)) {
				if (v.equals("null")) {
					vs[i] = null;
				}
				else {
			...
			else if (ObjectUtil.in(dataTypes, DataTypes.INTEGER, DataTypes.SMALLINT)) {
			...
			else if (ObjectUtil.in(dataTypes, DataTypes.DOUBLE, DataTypes.DECIMAL)) {
		...
		try {
			...
			for (long pos : ti.Positions) {
				...
				for (int i = 1; i <= rowCount; i++) {
					String data = readUTF(braf);
					Object[] vs = getOneRow(ti.Columns, data);
					List<Object> values = new ArrayList<Object>(ti.Columns.length);
	 ...
	 * @return
	 */
	private boolean checkNull(ZDTTableInfo ti, boolean isOracle, Object[] vs, List<Object> values) {
		for (int j = 0; j < vs.length; j++) {
			Object v = vs[j];
			if (isOracle) {
				// NotNull的列允许空字符串，但Oracle不允许
				if (ti.Columns[j].isMandatory() && (v == null || v.equals(""))) {
					LogUtil.warn(ti.Name + "'s column " + ti.Columns[j].getColumnName() + " can't be empty");
					return false;
				}
			}
			values.add(v);
