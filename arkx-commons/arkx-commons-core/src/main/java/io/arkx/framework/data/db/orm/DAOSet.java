...
public class DAOSet<T extends DAO<T>> implements Serializable, Cloneable, Set<T> {
    ...
    public boolean insert() {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(meta.getInsertSQL(db), ResultSet.TYPE_FORWARD_ONLY,
            ...
            for (T dao : elementData) {
                ...
                for (DAOColumn sc : meta.getColumns()) {
                    if (sc.isMandatory()) {
                        if (dao.getV(i) == null) {
                            LogUtil.warn(
                                    meta.getTable() + "'s mandatory column " + sc.getColumnName() + " can't be null");
                            return false;
                        }
                    }
                    Object v = dao.getV(i);
            ...
            for (T dao : elementData) {
                ...
                for (int i = 0, j = 0; i < meta.getColumns().length; i++) {
                    ...
                    if (sc.isPrimaryKey()) {
                        ...
                        if (v == null) {
                            LogUtil.warn(
                                    meta.getTable() + "'s primary column " + sc.getColumnName() + " can't be null");
                            return false;
                        } else {
                            if (operateColumnFlag) {
                                DAOUtil.setParam(dao, meta.getColumns()[i], pstmt, conn, j + operateColumnOrders.length,
                                        v);
                            } else {
                                DAOUtil.setParam(dao, meta.getColumns()[i], pstmt, conn, j + meta.getColumns().length,
                                        v);
                            }
                        }
                        j++;
    ...
    public boolean delete() {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(meta.getDeleteSQL(db), ResultSet.TYPE_FORWARD_ONLY,
                    ResultSet.CONCUR_READ_ONLY);
            for (T dao : elementData) {
                for (int i = 0, j = 0; i < meta.getColumns().length; i++) {
                    DAOColumn sc = meta.getColumns()[i];
        ...
        if (operateColumnNames != null) {
            ...
            for (int i = 0, k = 0; i < operateColumnNames.length; i++) {
                ...
                for (int j = 0; j < meta.getColumns().length; j++) {
                    if (operateColumnNames[i].toString().toLowerCase()
                            .equals(meta.getColumns()[j].getColumnName().toLowerCase())) {
