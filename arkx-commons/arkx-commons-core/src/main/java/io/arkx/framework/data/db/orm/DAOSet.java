			...
			for (T dao : elementData) {
				...
				for (DAOColumn sc : meta.getColumns()) {
					if (sc.isMandatory()) {
						if (dao.getV(i) == null) {
							LogUtil
								.warn(meta.getTable() + "'s mandatory column " + sc.getColumnName() + " can't be null");
							return false;
						}
					}
					Object v = dao.getV(i);
			...
			for (T dao : elementData) {
				...
				for (int i = 0, j = 0; i < meta.getColumns().length; i++) {
					...
					if (sc.isPrimaryKey()) {
						...
						if (v == null) {
							LogUtil
								.warn(meta.getTable() + "'s primary column " + sc.getColumnName() + " can't be null");
							return false;
						}
						else {
							if (operateColumnFlag) {
		...
		if (operateColumnNames != null) {
			...
			for (int i = 0, k = 0; i < operateColumnNames.length; i++) {
				...
				for (int j = 0; j < meta.getColumns().length; j++) {
					if (operateColumnNames[i].toString()
						.toLowerCase()
