 ...
 * 轻量级解码器。
 * <p>
 * AbstractDecoder的 {@link #id3v1()}， {@link #id3v2()}，
 * {@link #vbr(Header)}，{@link #done()}， {@link #cooperate()}
 * 方法内部什么也没做，要执行相应的具体功能，须在子类中重载实现。 AbstractDecoder 定义这些方法，是为了保证这些方法在适当的时候被解码器调用。
 *
 * @version 0.400
 */
public abstract class AbstractDecoder {
    ...
    public final void run() {
        ...
        if (layer != null) {
            ...
            // System.out.println("\nAbstractDecoder.run: eof = " + eof + ", isInterrupted =
            // " + isInterrupted);
     ...
     *
     * @param h
     *            帧头对象，该帧头对象已经被解码器正确初始化。该方法被调用时表明解码器已经成功定位到第一帧并且完成了帧头解码。
     */
     ...
     *
     * @return 返回值能够控制 {@link #run() run} 方法的执行流程。如果返回值为 <b>true</b> 则 {@link #run()
     *         run} 循环解码下一帧，如果返回值为 <b>false</b> 则退出循环结束解码过程。
     */
