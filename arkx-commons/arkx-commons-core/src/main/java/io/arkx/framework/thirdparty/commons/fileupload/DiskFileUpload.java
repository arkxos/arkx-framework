 ...
 * </p>
 * <p>
 * Individual parts will be stored in temporary disk storage or in memory, depending on
 * their size, and will be available as {@link FileItem}s.
 * </p>
 *
...
public class DiskFileUpload extends FileUploadBase {
	...
	/**
	 * Sets the factory class to use when creating file items. The factory must be an
	 * instance of <code>DefaultFileItemFactory</code> or a subclass thereof, or else a
	...
	/**
	 * Returns the location used to temporarily store files that are larger than the
	...
	/**
	 * Sets the location used to temporarily store files that are larger than the
	 * configured size threshold.
	 ...
	 * <code>multipart/form-data</code> stream. If files are stored on disk, the path is
	 * given by <code>getRepository()</code>.
