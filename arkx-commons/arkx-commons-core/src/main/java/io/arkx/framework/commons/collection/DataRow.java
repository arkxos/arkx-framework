...
/**
 * @class org.ark.framework.collection.DataRow 数据行，表示DataTable中的一行数据
 * @author Darkness
...
public class DataRow implements Serializable, Cloneable {
	...
	// if ((this.columns[i].getColumnType() == DataColumnType.DATETIME) && ((v
	// instanceof Date))) {
	...
	public void fill(Object... values) {// NO_UCD
		...
		if (values.length != getColumnCount()) {
			throw new UtilityException(
					"Parameter's length is " + values.length + "，bit DataRow's length is " + getColumnCount());
		}
