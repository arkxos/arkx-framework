 ...
 * 从输入的字节流获得位流。
 * <p>
 * 对于Layer3文件解码， BitStream 仅用于读取帧边信息，要读取Layer3文件的主数据，必须使用{@link BitStreamMainData}。
 * <p>
...
public class BitStream {
	 ...
	 * @param extra 缓冲区尾部空出的字节数，防止哈夫曼解码时位流有错误导致缓冲区溢出， 尾部空出512字节(part2_3_length长度，2^12位)。
	 */
	...
	/**
	 * 将缓冲指定为b，缓冲区初始偏移量由off指定。与 {@link #append(byte[], int, int)} 方法的区别是，本方法不会从源数据b复制数据。
	 * @param b 源数据。
	 * @param off 源数据偏移量。
	 */
