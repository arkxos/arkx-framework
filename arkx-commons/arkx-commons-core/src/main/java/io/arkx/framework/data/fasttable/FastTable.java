...
public class FastTable extends MappedFile {
	...
	public <T, K extends Comparable<? super K>, R> R query(ISearcher<T, K, R> searchWarpper,
			RecordFunction<T> converter, int recordLength) throws IOException {
		...
		R r = searchWarpper.search(result.first, result.second, result.third, result.fourth, this, converter,
				recordLength);
	...
	private <T, K extends Comparable<? super K>, R> FourTuple<T, Long, Long, Long> binarySearch(
			ISearcher<T, K, R> searchWarpper, long total, RecordFunction<T> converter, int recordLength)
			throws IOException {
	...
	public <T> List<T> queryN(long index, long count, RecordFunction<T> converter, int recordLength)
			throws IOException {
		if (index < 0) {
			index = 0;
		}
	...
	public <T> T queryOne(long index, RecordFunction<T> converter, int recordLength) throws IOException {
		// TimeWatch timeWatch = TimeWatch.create().startWithTaskName("query one " +
		// index);
