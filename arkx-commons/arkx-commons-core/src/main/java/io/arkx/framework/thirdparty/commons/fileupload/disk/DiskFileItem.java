 ...
 * <p>
 * After retrieving an instance of this class from a {@link DiskFileUpload DiskFileUpload}
 * instance (see {@link DiskFileUpload
 * #parseRequest(javax.servlet.http.HttpServletRequest)} ), you may either request all
 * contents of file at once using {@link #get()} or request an {@link java.io.InputStream
 * InputStream} with {@link #getInputStream()} and process the file without attempting to
...
public class DiskFileItem implements FileItem {
	...
	/**
	 * Default content charset to be used when no explicit charset parameter is provided
	...
	/**
	 * The size of the item, in bytes. This is used to cache the size when a file item is
	 ...
	 * @return An {@link java.io.InputStream InputStream} that can be used to retrieve the
	 * contents of the file.
	...
	/**
	 * Returns the contents of the file as an array of bytes. If the contents of the file
	 * were not yet cached in memory, they will be loaded from the disk storage and
	 * cached.
	 ...
	 * @throws UnsupportedEncodingException if the requested character encoding is not
	 * available.
	 */
	...
	/**
	 * A convenience method to write an uploaded item to disk. The client code is not
	 * concerned with whether or not the item is stored in memory, or on disk in a
	 * temporary location. They just want to write the uploaded item to a file.
	 * <p>
	 * This implementation first attempts to rename the uploaded item to the specified
	 * destination file, if the item was originally written to disk. Otherwise, the data
	 * will be copied to the specified file.
	 * <p>
	 * This method is only guaranteed to work <em>once</em>, the first time it is invoked
	 * for a particular item. This is because, in the event that the method renames a
	 * temporary file, that file will no longer be available to copy or rename again at a
	 * later time.
	 ...
	 * temporary disk file. Although this storage will be deleted automatically when the
	 * <code>FileItem</code> instance is garbage collected, this method can be used to
	 * ensure that this is done at an earlier time, thus preserving system resources.
	 */
	public void delete() {
		cachedContent = null;
		File outputFile = getStoreLocation();
	...
	/**
	 * Returns the name of the field in the multipart form corresponding to this file
	 * item.
	...
	/**
	 * Returns an {@link java.io.OutputStream OutputStream} that can be used for storing
	 * the contents of the file.
	 * @return An {@link java.io.OutputStream OutputStream} that can be used for storing
	 * the contensts of the file.
	 * @throws IOException if an error occurs.
	 */
	...
	/**
	 * Returns the {@link java.io.File} object for the <code>FileItem</code>'s data's
	 * temporary location on the disk. Note that for <code>FileItem</code>s that have
	 * their data stored in memory, this method will return <code>null</code>. When
	 * handling large files, you can use {@link java.io.File#renameTo(java.io.File)} to
	 * move the file to new location without copying the data, if the source and
	 * destination locations reside within the same logical volume.
	 ...
	 * temporary file in the configured repository path. The lifetime of the file is tied
	 * to the lifetime of the <code>FileItem</code> instance; the file will be deleted
	...
	/**
	 * Returns an identifier that is unique within the class loader used to load this
	 * class, but does not have random-like apearance.
	...
	public String toString() {
		return "name=" + this.getName() + ", StoreLocation=" + String.valueOf(this.getStoreLocation()) + ", size="
				+ this.getSize() + "bytes, " + "isFormField=" + isFormField() + ", FieldName=" + this.getFieldName();
	}
