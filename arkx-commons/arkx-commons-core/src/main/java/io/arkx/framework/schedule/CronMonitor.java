...
public class CronMonitor extends TimerTask {
    ...
    private CronMonitor() {
        es = new ThreadPoolExecutor(1, Integer.MAX_VALUE, 120L, TimeUnit.SECONDS, new SynchronousQueue<Runnable>(),
                new ThreadFactory() {
                    @Override
                    ...
                    if (isOnTime(current, tm.getTaskCronExpression(id))) {
                        ...
                        if (taskStatus.getBoolean(taskKey)) {
                            ...
                            es.submit(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        ExtendManager.invoke(BeforeCronTaskExecutedAction.ID,
                                                new Object[]{tm.getExtendItemID(), id});
                                        tm.execute(id); // 扩展点,主要用于定时任务监测
                                        ExtendManager.invoke(AfterCronTaskExecutedAction.ID,
                                                new Object[]{tm.getExtendItemID(), id});
                                    } finally {
