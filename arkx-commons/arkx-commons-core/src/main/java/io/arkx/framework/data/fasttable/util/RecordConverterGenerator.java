...
public class RecordConverterGenerator {
	...
	public static String generate(Class<?> clazz) {
		// String generateFolder = System.getProperty("user.dir") + File.separator +
		// "fastdb-converter-generate-temp";
	...
	private static String generateFastColumn(Field field) {
		...
		if (field.getType() == String.class) {
			...
			return "	FastColumn " + filedName + "Column = new FastColumn(FastColumnType.String, \"" + filedName
					+ "\", " + length + ");\r\n";
		}
	...
	private static String generateText(Class<?> clazz, List<Field> fastColumnFields) {
		...
		String result = "import java.nio.ByteBuffer;\r\n" + "import java.time.LocalDate;\r\n" + "\r\n"
				...
				+ classSimpleName + "RecordConverter extends RecordConverter<" + classSimpleName + "> {\r\n" + "\r\n";
		...
		result += "	@Override\r\n" + "	public void writeEntity2Buffer(" + classSimpleName
				+ " entity, ByteBuffer recordBuffer) {\r\n";
		...
		result += "	public " + classSimpleName + " builderObject(ByteBuffer recordBuffer) {\r\n" + "		"
		...
		for (Field field : fastColumnFields) {
			...
			if (field.getType() == String.class) {
				result += "		String " + fieldName + " = readString(recordBuffer, " + fieldName
						+ "Column.getLength());\r\n";
