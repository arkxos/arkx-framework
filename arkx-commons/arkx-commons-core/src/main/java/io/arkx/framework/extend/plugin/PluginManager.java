...
/**
 * @class org.ark.framework.extend.plugin.PluginManager 插件管理器
 * @private
 * @author Darkness
...
public class PluginManager {
     ...
     * 加载插件配置
     *
     * 加载规则： 1、加载lib下的包含“-plugin-*.jar”的jar，读取jar中的“.plugin”文件
    ...
    private List<PluginConfig> loadAllConfig(List<String> pluginIds) {
        ...
        for (String pluginId : pluginIds) {
            File statusFile = new File(Config.getClassesPath() + File.pathSeparator + "plugin" + File.separator
                    + pluginId + "status.config");
        ...
        for (PluginConfig pc : configList) {
            ...
            for (String pluginID : pc.getRequiredPlugins().keySet()) {
                ...
                if (!isVersionCompatible(need, v)) {
                    LogUtil.error("Plugin " + pluginID + "'s version is " + v + ", but " + need + " is needed by "
                            + pc.getID() + "!");
                }
            }
            if (!requiredFlag) {
