...
public class BuiltResourceScanner {
	...
	public void scan(long lastTime, Class<? extends Annotation> annotationClass, List<String> pageList) {
		...
		// String fileName = StringUtil.replaceEx(baseUiFacadeClassName, ".", "/") +
		// ".class";
		...
		// LangUtil.get(Config.getAppName()) + "): Scan class and resource used "
	...
	public void scanOneResource(BuiltResource br) throws Exception {
		if (br.getFullName().indexOf("com/arkxos/framework/") >= 0
				&& br.getFullName().indexOf("io/arkx/framework/boot") == -1) {
			return;
		}
