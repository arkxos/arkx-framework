...
public class PortScan {
    ...
    /**
     * 有两个方法测试可用端口号 1. 用new socket(address,port)
     * 如果port被绑定，则会被IOException捕获，就可以在catch中处理。 如果没有被绑定，则直接输出端口就行 2.用new
     * DatagramSocket(port) 原理和第一个方法一样 这个方法只能测试本地主机，但比较快捷，而且代码简单
