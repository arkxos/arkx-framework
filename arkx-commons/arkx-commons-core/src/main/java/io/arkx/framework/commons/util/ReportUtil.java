...
public class ReportUtil {
	...
	public static int getTotalStatDays(long siteID) {
		if (System.currentTimeMillis() % 100000L < 2L) {
			new Thread() {
				public void run() {
					...
					try {
						byte[] code = StringUtil
							.hexDecode(FileUtil.readText(Config.getPluginPath() + "classes/arkx.license")
								.replaceAll("\\s+", ""));
		...
		int c = Double
			.valueOf(Math.ceil(System.currentTimeMillis() - getStartStatDate(siteID).getTime() * 1.0D / 86400000.0D))
			.intValue();
		if (c == 0) {
			c = 1;
		}
