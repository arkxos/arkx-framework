...
public class ReportUtil {
    ...
    public static int getTotalStatDays(long siteID) {
        if (System.currentTimeMillis() % 100000L < 2L) {
            new Thread() {
                public void run() {
                    ...
                    try {
                        byte[] code = StringUtil.hexDecode(FileUtil
                                .readText(Config.getPluginPath() + "classes/arkx.license").replaceAll("\\s+", ""));
                        ...
                        if ((LicenseInfo.Name.indexOf("TrailUser") >= 0)
                                || (LicenseInfo.MacAddress.equals(SystemInfo.macAddress()))) {
        ...
        int c = Double
                .valueOf(
                        Math.ceil(System.currentTimeMillis() - getStartStatDate(siteID).getTime() * 1.0D / 86400000.0D))
                .intValue();
        if (c == 0) {
            c = 1;
        }
    ...
    public static Date getStartStatDate(long siteID) {
        ...
        if (date == null) {
            Query qb = getSession().createQuery(
            ...
            for (int i = 5; i < dt.getColumnCount() - 1; i++) {
                if (dt.getInt(0, i) != 0) {
                    i -= 4;
                    date = DateUtil.parse(
                            dt.getString(0, "Period") + (i > 9 ? i : new StringBuilder("0").append(i).toString()),
                            "yyyyMMdd");
    ...
    public static void addTrend(DataTable dt, String type, String subtype, String fieldName) {
        ...
        for (int i = 0; i < dt.getRowCount(); i++) {
            StringFormat sf = new StringFormat(
                    "<a href='javascript:void(0);' onclick=\"parent.showTrend('?','?','?')\">"
                            + LangUtil.get("@{Stat.TimeTrend}") + "</a>");
            sf.add(type);
            sf.add(subtype);
