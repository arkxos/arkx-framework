...
import com.rapidark.framework.config.LoginMethod;
import com.rapidark.framework.core.JsonResult;
import com.rapidark.framework.core.exception.UIMethodNotFoundException;
...
import com.rapidark.framework.cosyui.web.mvc.Dispatcher.DispatchException;
...
import com.rapidark.framework.extend.action.AfterUIMethodInvokeAction;
import com.rapidark.framework.extend.action.BeforeUIMethodInvokeAction;
...
public class AjaxHandler extends AbstractHtmlHandler {
	...
	public static final String ID = "com.rapidark.framework.core.AjaxHandler";
	...
	public boolean execute(String url, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			// 如果调用的是ZAction方法
			if (cs.length > 0 && (ArrayUtils.contains(cs, ZAction.class))){//&& cs[0].isAssignableFrom(ZAction.class)) {
				isAction = true;
				action = new ZAction();
...
//			e.printStackTrace();
	...
	public static Object[] fixMethodParamsValue(Method m, Object[] args) {
		...
		String[] paramNames = com.rapidark.framework.commons.util.lang.ClassUtil.getMethodParamNames(m.getDeclaringClass(), m.getName());
...
//					result[j] = schema;
//				} else 
				if(com.rapidark.framework.commons.collection.Mapx.class.isAssignableFrom(cs[j])) {
					result[j] = request;
				}
