...
import com.arkxos.framework.data.db.command.RenameColumnCommand;
...
public class MySql extends AbstractDBType {
	...
	public String[] toSQLArray(AdvanceChangeColumnCommand c) {
		String sql = "alter table " + c.Table + " modify column " + c.Column + " " + toSQLType(DataTypes.valueOf(c.DataType), c.Length, c.Precision);
		if (c.Mandatory) {
			sql += " not null";
		}
	...
	public boolean isTableExist(String databaseName, String tableName) {
		...
		String sql = "SELECT count(1) FROM information_schema.tables t WHERE LCASE(t.table_name) = LCASE(?) AND t.table_schema = '"+databaseName+"'";
