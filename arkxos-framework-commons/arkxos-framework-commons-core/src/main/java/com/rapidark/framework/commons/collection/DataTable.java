...
import com.rapidark.framework.commons.util.UtilityException;


/**
 * @class org.ark.framework.collection.DataTable
 * 数据表格
 * 使用方式：
 *  DataColumn dcUserName = new DataColumn("UserName", DataColumn.STRING);
	DataColumn dcRealName = new DataColumn("RealName", DataColumn.STRING);
	DataColumn dcStatusName = new DataColumn("StatusName", DataColumn.STRING);
	DataColumn dcBranchInnercodeName = new DataColumn("BranchInnercodeName", DataColumn.STRING);
	DataColumn dcRoleNames = new DataColumn("RoleNames", DataColumn.STRING);

	DataTable dt = new DataTable(new DataColumn[] { dcUserName, dcRealName, dcStatusName, dcBranchInnercodeName, dcRoleNames }, null);
...
//		insertColumn(new DataColumn(columnName, DataTypes.STRING), columnValue, index);
//	}
...
//		}
	...
	public void union(DataTable anotherDT) {
		...
		if (getColumnCount() != anotherDT.getColumnCount()) {
			throw new UtilityException("This's column count is " + getColumnCount() + " ,but parameter's column column count is "
		...
		return filter(new Filter<DataRow>() {
			...
			public boolean filter(DataRow dr) {
				for (int i = 0; i < params.length; i++) {
					...
					if (!dr.getString(paramInfo[0]).equals(
							paramInfo[1].replaceAll("\'", ""))) {
						return false;
					}
				}
