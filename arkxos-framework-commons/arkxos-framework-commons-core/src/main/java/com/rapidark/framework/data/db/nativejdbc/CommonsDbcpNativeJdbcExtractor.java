 ...
 * or higher.
 * <p>
 * Returns the underlying native Connection, Statement, etc to application code instead of DBCP's wrapper implementations. The returned JDBC
 * classes can then safely be cast, e.g. to <code>oracle.jdbc.OracleConnection</code>.
 * <p>
 * This NativeJdbcExtractor can be set just to <i>allow</i> working with a Commons DBCP DataSource: If a given object is not a Commons DBCP
