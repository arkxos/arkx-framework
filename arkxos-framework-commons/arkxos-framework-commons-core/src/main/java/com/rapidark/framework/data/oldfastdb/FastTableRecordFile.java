...
import com.rapidark.framework.commons.collection.ThreeTuple;
...
import com.rapidark.framework.data.fasttable.MappedFile;
import com.rapidark.framework.data.oldfastdb.BatchUtil.PageInfo;
...
//			openReadFileChannel();
			...
			dt = lightningTableConvertor.createTable(lightningTableType, tableInfo.getTableName(), columns.toArray(new FastColumn[0]));
...
//			long rowSizeLength = tableInfo.rowSize;
	...
	protected RecordUnit readRow(FastTableHeader tableInfo, FileChannel fileChannel, 
			int rowDataLength, int recordStart, FilterInfo whereString) throws IOException {
		TimeWatch timeWatch = new TimeWatch();
//		timeWatch.startWithTaskName("read row:recordStart:"+recordStart);
//		int headerLength = tableInfo.length;
//		fileChannel.position(INT_LENGTH + headerLength + recordStart * rowDataLength);
...
//		rowBuffer.position(0);
		...
		boolean isNeedRead = true;//new RecordFilter().filter(recordUnit, tableInfo, whereString);
	...
	public <T extends IFastTable> TwoTuple<PkList, Map<String, Multimap<Object, Integer>>> save(T dt) {
		...
		try {
			...
			IFastTableConvertor<T> lightningTableConvertor = (IFastTableConvertor<T>)FastTableConvertRegister.get(dt.getClass());
...
//				System.out.println("start: " + start + ", limit: " + limit);
				ThreeTuple<PkList, List<ByteBuffer>, Map<String, Multimap<Object, Integer>>> temp = lightningTableConvertor.buildRowByteBuffers((int)existRowCount, fixColumns, fastDataTable.getDataRows(start, limit), typeLength);
				existRowCount += limit;
				...
				MappedByteBuffer rowMappedByteBuffer = fileChannel.map(FileChannel.MapMode.READ_WRITE, fileLength, limit*rowDataLength);
				allRowBuffer.flip();
