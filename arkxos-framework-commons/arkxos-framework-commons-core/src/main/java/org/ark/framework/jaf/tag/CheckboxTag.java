 ...
 * <h2>Checkbox标签</h2>
 * <br/>
 * <img src="images/CheckboxTag_1.png"/>
 * <br/>&lt;ark:checkbox method="IndexList.getIndexTypes" value="Article" id="IndexType" />
 * @author Darkness
...
public class CheckboxTag extends BodyTagSupport {
	...
	public String getHtml() {
		...
		DataTable dt = null;//SelectTag.getCodeData(this.code, this.method, this.options, this.pageContext, map);
		...
		String html = "<input type=\"" + this.type + "\" ${Disabled} ${Checked} id=\"" + this.name + hasRowNo + "\" name=\"" + this.name + "\" value=\"${Key}\" onclick=\"${OnClick}\"";
		if (StringUtil.isNotEmpty(this.theme)) {
			...
			html = html + "<script>if(Ark." + jsClassName + "){new Ark." + jsClassName + "('" + this.name + hasRowNo + "');}</script>";
		} else {
			html = html + "><label for=\"" + this.name + hasRowNo + "\"";
			html = html + ">${Value}</label>";
		}
