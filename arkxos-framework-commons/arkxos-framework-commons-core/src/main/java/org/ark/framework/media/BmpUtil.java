...
public class BmpUtil
{
  ...
  private static Image readMap1(FileInputStream fs, BitmapHeader bh)
    throws IOException
  {
    int nNumColors = 0;
    if (bh.nclrused > 0)
    ...
    for (int n = 0; n < nNumColors; n++) {
      npalette[n] = 
        (0xFF000000 | 
        (bpalette[(nindex1 + 2)] & 0xFF) << 16 | 
        (bpalette[(nindex1 + 1)] & 0xFF) << 8 | 
    ...
    for (int j1 = 0; j1 < bh.nheight; j1++) {
      ...
      {
        if (bh.nwidth * (bh.nheight - j1 - 1) + i1 > bh.nwidth * 
          bh.nheight - 1)
          break;
    ...
    Image image = Toolkit.getDefaultToolkit().createImage(
      new MemoryImageSource(bh.nwidth, bh.nheight, ndata1, 0, 
      bh.nwidth));
    fs.close();
    return image;
  }
  ...
  public static BufferedImage read(String sdir) {
    try {
      ...
      return read(fs); } catch (IOException ex) {
    }
    return null;
  }
