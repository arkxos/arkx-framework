...
public class TableUpdater {
	...
	public static String createIndex(String tableName, String indexInfo, int i, IDBType dbType) {
		...
		if (((dbType.getExtendItemID().equalsIgnoreCase("ORACLE")) || (dbType.getExtendItemID().equalsIgnoreCase("DB2"))) && (name.length() > 15)) {
	...
	public static class AlterKeyInfo extends TableUpdateInfo implements Serializable {
		...
		public String[] toSQLArray(IDBType dbType) {
			...
			if ("MSSQL".equalsIgnoreCase(dbType.getExtendItemID())) {
				sql = "alter table " + this.TableName + " add constraint PK_" + this.TableName + " primary key  NONCLUSTERED(" + this.NewKeys + ")";
			}
			return new String[] { sql };
		}
	}
	...
	public static class AlterTableInfo extends TableUpdateInfo implements Serializable {
		...
		public String[] toSQLArray(IDBType dbType, ArrayList<TableUpdateInfo> togetherColumns, Mapx<String, String> exclusiveMapx) {
			...
			if (dbType.getExtendItemID().equalsIgnoreCase("MYSQL")) {
				if (this.Action.equalsIgnoreCase("add")) {
					...
					for (int i = 0; i < togetherColumns.size(); i++) {
						...
						String sql = "alter table " + info.TableName + " add column " + info.OldColumnName + " " + TableCreator.toSQLType(info.ColumnType, info.Length, info.Precision, dbType);
						if (info.Mandatory) {
							sql = sql + " not null";
						}
			...
			if (dbType.getExtendItemID().equalsIgnoreCase("ORACLE")) {
				if (this.Action.equalsIgnoreCase("add")) {
					...
					if (StringUtil.isNotEmpty(this.AfterColumn)) {
						...
						sb.append("create table " + this.TableName + "_TMP as select " + StringUtil.join(list) + " from " + this.TableName + "\n");
						for (int j = 0; j < togetherColumns.size(); j++) {
							...
							if (this.Mandatory) {
								if (info.ColumnType == 1) {
									...
									sb.append("update " + info.TableName + "_TMP set " + info.OldColumnName + "='1970-01-01 00:00:00'\n");
								} else {
									sb.append("update " + info.TableName + "_TMP set " + info.OldColumnName + "=null\n");
									sb.append("alter table " + info.TableName + "_TMP modify " + info.OldColumnName + " "
											+ TableCreator.toSQLType(info.ColumnType, info.Length, info.Precision, dbType) + "\n");
							...
							sb.append("alter table " + info.TableName + "_TMP modify " + info.OldColumnName + " " + TableCreator.toSQLType(info.ColumnType, info.Length, info.Precision, dbType)
						...
						for (int j = 0; j < togetherColumns.size(); j++) {
							AlterTableInfo info = (AlterTableInfo) togetherColumns.get(j);
							sb.append("alter table " + info.TableName + " add " + info.OldColumnName + " " + TableCreator.toSQLType(info.ColumnType, info.Length, info.Precision, dbType));
							if (info.Mandatory) {
								sb.append(" not null");
							}
							sb.append("\\n");
				...
				if ((this.Action.equalsIgnoreCase("modify")) || (this.Action.equalsIgnoreCase("change"))) {
					if ((StringUtil.isNotEmpty(this.NewColumnName)) && (!this.NewColumnName.equalsIgnoreCase(this.OldColumnName))) {
						Schema schema = SchemaUtil.findSchema(this.TableName);
						SchemaColumn[] scs = (SchemaColumn[]) schema.Columns.clone();
						ArrayList list = new ArrayList();
						for (int i = 0; i < scs.length; i++) {
			...
			if (dbType.getExtendItemID().equalsIgnoreCase("MSSQL")) {
				if (this.Action.equalsIgnoreCase("add")) {
					...
					if (StringUtil.isNotEmpty(this.AfterColumn)) {
							...
							for (int j = 0; j < togetherColumns.size(); j++) {
								...
								if (scs[i].getColumnName().equalsIgnoreCase(columnName)) {
									SchemaColumn sc = new SchemaColumn(
											info.OldColumnName, info.ColumnType, i, info.Length, 
						...
						sqlList.add("if exists(select * from " + this.TableName + ") exec ('insert into " + this.TableName + "_TMP (" + StringUtil.join(listInsert) + ") select "
						...
						for (int j = 0; j < togetherColumns.size(); j++) {
							AlterTableInfo info = (AlterTableInfo) togetherColumns.get(j);
							String sql = "alter table " + info.TableName + " add " + info.OldColumnName + " " + TableCreator.toSQLType(info.ColumnType, info.Length, info.Precision, dbType);
							if (info.Mandatory) {
								sql = sql + " not null";
							}
							sqlList.add(sql);
				...
				if ((this.Action.equalsIgnoreCase("modify")) || (this.Action.equalsIgnoreCase("change"))) {
					...
					sqlList.add(sql);
					if ((StringUtil.isNotEmpty(this.NewColumnName)) && (!this.NewColumnName.equalsIgnoreCase(this.OldColumnName))) {
						sqlList.add(" sp_rename '" + this.TableName + "." + this.OldColumnName + "','" + this.NewColumnName + "','column'");
					}
					if (column.isPrimaryKey()) {
						SchemaColumn[] scs = SchemaUtil.findSchema(this.TableName).Columns;
						List pkList = SchemaUtil.getPrimaryKeyColumnsList(scs);
						sqlList.add("alter table " + this.TableName + " add constraint PK_" + this.TableName + " primary key NONCLUSTERED(" + StringUtil.join(pkList) + ")");
					}
					String[] arr = new String[sqlList.size()];
					for (int i = 0; i < arr.length; i++) {
						arr[i] = ((String) sqlList.get(i));
