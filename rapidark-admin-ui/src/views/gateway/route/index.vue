      ...
      <div>
        <Form ref="routeForm" :model="formItem" :rules="formItemRules" :label-width="100">
          <FormItem label="路由名称" prop="routeDesc">
            <Input v-model="formItem.routeDesc" placeholder="请输入内容"/>
          </FormItem>
          <FormItem label="路由标识" prop="routeName">
            <Input v-model="formItem.routeName" placeholder="默认使用服务名称{application.name}"/>
          </FormItem>
          <FormItem label="路由前缀" prop="path">
            <Input v-model="formItem.path" placeholder="/{path}/**"/>
          </FormItem>
          <FormItem label="路由方式" prop="routeType">
            <Select v-model="formItem.routeType">
              <Option value="service" label="负载均衡(serviceId)"/>
              <Option value="url" label="反向代理(url)"/>
            </Select>
          </FormItem>
          <FormItem v-if="formItem.routeType==='service'" label="负载均衡" prop="serviceId"
                    :rules="{required: true, message: '服务名称不能为空', trigger: 'blur'}">
            <Input v-model="formItem.serviceId" placeholder="服务名称application.name"/>
          </FormItem>
          <FormItem v-if="formItem.routeType==='url'" label="反向代理" prop="url"
            ...
            <Input v-model="formItem.url" placeholder="http://localhost:8080"/>
          </FormItem>
          <FormItem label="忽略前缀">
