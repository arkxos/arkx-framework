    ...
    <Card shadow>
      ...
      <Form style="margin-top: 20px;" ref="form" :model="formItem" :rules="formItemRules" :label-width="140">
        ...
        <FormItem v-show="current===0" label="驱动名称" prop="driverName">
          <Input v-model="formItem.driverName" placeholder="请输入内容"/>
        </FormItem>
        <FormItem v-show="current===0" label="连接地址" prop="url">
          <Input v-model="formItem.url" placeholder="请输入内容"/>
        </FormItem>
        <FormItem v-show="current===0" label="用户名" prop="username">
          <Input v-model="formItem.username" placeholder="请输入内容"/>
        </FormItem>
        <FormItem v-show="current===0" label="密码" prop="password">
          <Input v-model="formItem.password" type="password" placeholder="请输入内容"/>
        </FormItem>
        <Row>
          <Col span="10">
          <FormItem v-show="current===1" label="模块名称" prop="moduleName">
            <Input v-model="formItem.moduleName" placeholder="请输入内容"/>
          </FormItem>
          <FormItem v-show="current===1" label="顶级包名" prop="parentPackage">
            <Input v-model="formItem.parentPackage" placeholder="请输入内容"/>
          </FormItem>
          <FormItem v-show="current===1" label="作者" prop="author">
            <Input v-model="formItem.author" placeholder="请输入内容"/>
          </FormItem>
          </Col>
          <Col span="14">
