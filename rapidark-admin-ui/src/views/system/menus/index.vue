      ...
      <Card shadow>
        <tree-table style="max-height:700px;overflow: auto"
          ...
          <template slot="status" slot-scope="scope">
            ...
            <ark-icon :icon="scope.row.icon" size="16"/>
          </template>
        </tree-table>
      </Card>
      </Col>
          ...
          <Form ref="menuForm" :model="formItem" :rules="formItemRules" :label-width="80">
            <FormItem label="前端应用" prop="serviceId">
                <Select v-model="formItem.serviceId" filterable clearable>
                  <Option :title="item.serviceName" v-for="(item,index) in selectServiceList"  @click.native="handleOnSelectService(item)"
                          :value="item.serviceId" :label="item.serviceName" :key="index">
            ...
            <FormItem label="图标">
              <Input v-model="formItem.icon" placeholder="请输入内容">
                <ark-icon size="28" :icon="formItem.icon" slot="prepend"/>
                <Poptip width="600" slot="append" placement="bottom" class="icon-selector-container">
                  <Button icon="ios-search"/>
                  <div slot="content" class="icon-selector-popper">
...
<!--                    <ul class="icons">
            ...
            <FormItem label="描述">
              <Input v-model="formItem.menuDesc" type="textarea" placeholder="请输入内容"/>
            </FormItem>
            <FormItem>
