            ...
            <Form v-show="current == 'form2'" ref="form2" :model="formItem" :label-width="100" :rules="formItemRules">
              <FormItem label="分配角色" prop="grantRoles">
                <CheckboxGroup v-model="formItem.grantRoles">
                  <Checkbox v-for="(item,index) in selectRoles" :label="item.roleId" :key="index"><span>{{ item.roleName }}</span></Checkbox>
                </CheckboxGroup>
              </FormItem>
            </Form>
          </TabPane>
                ...
                <tree-table
                  ...
                  <template slot="operation" slot-scope="scope">
                    <CheckboxGroup v-model="formItem.grantActions">
                      <Checkbox :disabled="item.disabled" v-for="(item,index) in scope.row.actionList" :label="item.authorityId" :key="index">
