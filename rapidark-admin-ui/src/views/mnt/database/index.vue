<template>
  <div class="app-container">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible="crud.status.cu > 0" :title="crud.status.title" width="530px">
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" highlight-current-row stripe style="width: 100%" @selection-change="handleCurrentChange">
      ...
      <el-table-column v-if="hasAuthority(['admin','database:edit','database:del'])" label="操作" width="150px" align="center">
...
export default {
  ...
  cruds() {
    return CRUD({ title: '数据库', url: 'base/database', crudMethod: { ...crudDatabase }})
  },
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          testDbConnect(this.form).then((res) => {
            this.loading = false
            this.crud.notify(res.data ? '连接成功' : '连接失败', res.data ? 'success' : 'error')
          }).catch(() => {
            this.loading = false
          })
        }
