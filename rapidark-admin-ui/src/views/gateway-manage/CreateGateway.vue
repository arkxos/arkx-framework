          ...
          <div slot="header" class="clearfix">
            ...
            <el-popover trigger="click" placement="bottom">
              <div style="font-size: 10pt;">
                ...
                <span>2.路由前缀，请根据实际API调用路径设置，不含服务host和port，可添加自定义前缀。</span><br/>
                <span>3.过滤StripPrefix根据值，截取路由前缀多节斜杠后内容，拼接到服务URL上。</span><br/>
                ...
                <span>&nbsp;&nbsp;b.路由前缀示例：/route/producer/** 或 /producer/api。</span><br/>
          ...
          <el-row style="margin-top: 20px;">
            <el-col :span="24">
              <div style="float: left; margin-right: 10px;">
                <el-popover placement="bottom" trigger="click">
                  <el-radio-group v-model="form.type" size="mini" @change="handleSelectedType">
                    <el-radio-button v-for="item in routeTypeOptions" :key="item.value" :label="item.value">{{item.label}}</el-radio-button>
                  </el-radio-group>
                  <el-button slot="reference">路由方式:{{routeTypeName}}<i class="el-icon-caret-bottom el-icon--right"></i></el-button>
                </el-popover>
              </div>
              <div style="float: left;">
                <el-input placeholder="请输入网关服务URL,示例:http://server:port、http://server.com、lb://xxx"
                          v-model="form.uri" class="input-with-select" style="width: 696px;">
              ...
              <div style="float: left; margin-left: 10px;">
                <el-popover placement="bottom" width="500" trigger="click">
                  ...
                  <el-button slot="reference">路由前缀={{form.path}}<i class="el-icon-caret-bottom el-icon--right"></i></el-button>
                </el-popover>
              </div>
              <div style="float: left; margin-left: 10px;">
                <el-popover placement="bottom" width="500" trigger="click">
...
<script>
// import gatewayJson from '@/api/gateway-manage/json/gateway.json'
import {addRoute,updateRoute} from '@/api/gateway-manage/gatewayAppRoute.js'
...
export default {
  data() {
    return {
      ...
      routeTypeOptions: this.GLOBAL_VAR.routeTypes
    }
  },
  created: function() {
    //在组件创建完毕后加载
  ...
  methods:{
    init(route) {
      if (route && route.form){
        ...
        this.handleSelectedType(this.form.type);
    ...
    handleSelectedType(val){
      let size = this.routeTypeOptions.length;
      for (var i=0;i <size; i++){
        if (this.routeTypeOptions[i].value === val){
          this.routeTypeName = this.routeTypeOptions[i].label;
          break;
        }
      }
    },
