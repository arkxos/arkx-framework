          ...
          <div slot="header" class="clearfix">
            ...
            <span style="margin-left: 50px;">
							...
							<span style="font-size: 11pt;">
								<el-tag size="mini" style="font-weight: bold;">{{form.appName}}</el-tag> -
								<el-tag size="mini" type="warning" style="font-weight: bold;">{{form.appId}}</el-tag> -
...
export default {
  ...
  methods:{
    init(route) {
      ...
      console.log('route.form', route.form)
      this.regClientList();
    },
    goBack() {
      console.log('go back');
    ...
    startAll(){
      ...
      this.$confirm('确认要允许当前服务下所有已注册的客户端访问？').then(_ => {
        startAllRegClient({routeId: _this.form.appId,}).then(function(result){
          _this.GLOBAL_FUN.successMsg();
          _this.regClientList();
        });
      }).catch(_ => {});
    ...
    stopAll(){
      ...
      this.$confirm('确认要禁止当前服务下所有已注册客户端访问？').then(_ => {
        stopAllRegClient({routeId: _this.form.appId,}).then(function(result){
          _this.GLOBAL_FUN.successMsg();
          _this.regClientList();
        });
      }).catch(_ => {});
    ...
    clientList(){
      let _this = this;
      notRegClientPageList({routeId: this.form.appId, currentPage: this.clientCurrentPage, pageSize: this.clientPageSize}).then(function(result){
        console.log(result);
