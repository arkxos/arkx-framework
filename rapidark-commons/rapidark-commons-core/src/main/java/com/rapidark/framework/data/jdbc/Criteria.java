...
import com.rapidark.framework.commons.collection.IPageData;
import com.rapidark.framework.commons.collection.IPageInfo;
import com.rapidark.framework.commons.collection.PageDataTable;
...
public class Criteria {
	...
	private Class<? extends com.rapidark.framework.data.jdbc.Entity> domainObjectClass;
	private List<Restrictions> restrictions = new ArrayList<Restrictions>();
	private List<Order> orders = new ArrayList<Order>();

	private Query qb;

	 Criteria(Transaction transaction, Class<? extends com.rapidark.framework.data.jdbc.Entity> domainObjectClass) {
	...
	public IPageData page(IPageInfo pageInfo) {
		...
		if (pageInfo.isPageEnabled()) {
			...
			List<? extends com.rapidark.framework.data.jdbc.Entity> entities = qb.findPagedEntities(domainObjectClass,pageInfo.getPageSize(), pageInfo.getPageIndex());
			
			result.setData(DataTableUtil.toDataTable(entities));
		} else {
			
			List<? extends com.rapidark.framework.data.jdbc.Entity> entities = qb.findEntities( domainObjectClass);
