...
ALTER TABLE `route` ADD COLUMN `host` VARCHAR(100) NULL COMMENT '断言主机' AFTER `method`;
ALTER TABLE `route` ADD COLUMN `remoteAddr` VARCHAR(20) NULL COMMENT '断言远程地址' AFTER `host`;
ALTER TABLE `route` ADD COLUMN `rewritePath` VARCHAR(200) NULL COMMENT '重写Path路径' AFTER `requestParameter`;
ALTER TABLE `route` ADD COLUMN `header` VARCHAR(200) NULL COMMENT '断言Headers' AFTER `remoteAddr`;

-- 添加systemCode系统代号
ALTER TABLE `route` ADD COLUMN `systemCode` VARCHAR(40) NOT NULL COMMENT '系统代号' AFTER `id`;
