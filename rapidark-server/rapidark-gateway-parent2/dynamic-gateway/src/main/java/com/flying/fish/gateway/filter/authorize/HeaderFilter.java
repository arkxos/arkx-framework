...
public class HeaderFilter extends FilterHandler {
    ...
    public void handleRequest(ServerHttpRequest request){
        ...
        if (gatewayAppRoute.getFilterAuthorizeName().contains("header")){
            log.info("处理网关路由请求{},执行header过滤 ", gatewayAppRoute.getId());
            String accessHeader = gatewayAppRoute.getAccessHeader();
