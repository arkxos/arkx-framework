 ...
 * 
 * @author Darkness
 * @date 2013-1-31 下午12:36:19 
 * @version V1.0
 */
public class SSIServletExternalResolver implements SSIExternalResolver {
	protected final String[] VARIABLE_NAMES = { "AUTH_TYPE", "CONTENT_LENGTH", "CONTENT_TYPE", "DOCUMENT_NAME", "DOCUMENT_URI", "GATEWAY_INTERFACE", "HTTP_ACCEPT", "HTTP_ACCEPT_ENCODING",
			"HTTP_ACCEPT_LANGUAGE", "HTTP_CONNECTION", "HTTP_HOST", "HTTP_REFERER", "HTTP_USER_AGENT", "PATH_INFO", "PATH_TRANSLATED", "QUERY_STRING", "QUERY_STRING_UNESCAPED", "REMOTE_ADDR",
	...
	private static Mapx<String, Long> timeMap = new Mapx(5000);
	private static Mapx<String, byte[]> fileMap = new Mapx(5000);
	...
	private static byte[] getFileContent(String path) {
		...
		String realPath = Config.getContextRealPath() + path;
		Long lastTime = (Long) timeMap.get(realPath);
		byte[] bs = (byte[]) null;
		if (lastTime == null)
			synchronized (mutex) {
				lastTime = Long.valueOf(current);
				timeMap.put(path, lastTime);
				if (new File(realPath).exists())
					bs = FileUtil.readByte(realPath);
				else {
					bs = ("Include file " + realPath + " is not exists!").getBytes();
				}
				fileMap.put(path, bs);
			}
