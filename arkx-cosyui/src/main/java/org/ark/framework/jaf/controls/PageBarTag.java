...
public class PageBarTag extends BodyTagSupport {
	...
	public static String getDefault(PageBarTag pageBarTag, String target, int total, int pageSize, int pageIndex) {
		...
		sb.append("<input type='button' id='_PageBar_JumpBtn_")
			...
			.append(target)
			.append("').value;if(!/^\\d+$/.test(v)")
			.append("||v<1||v>Number(Ark.DataList.getParam('")
			.append(target)
			.append("', Constant.PageTotal))/Number(Ark.DataList.getParam('")
			.append(target)
			.append("', Constant.Size))+1){alert('")
			.append(error)
		...
		if ((pageBarTag.action instanceof DataListAction))
			sb.append("Ark.DataList.setParam('")
				.append(target)
				.append("','")
				.append("_ARK_PAGEINDEX")
				.append("',pageIndex);Ark.DataList.loadData('")
				.append(target)
				.append("');");
		else {
