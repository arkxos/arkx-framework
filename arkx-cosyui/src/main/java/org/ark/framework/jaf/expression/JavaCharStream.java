...
/**
 * @class org.ark.framework.jaf.expression.JavaCharStream
 * 
 * @author Darkness
 * @date 2013-1-31 下午12:47:37 
 * @version V1.0
 */
	...
	public char readChar() throws IOException {
		...
		if ((this.buffer[this.bufpos] = c = ReadByte()) == '\\') {
			...
			try {
				...
				char tmp289_288 = (char) (hexval(c) << 12 | hexval(ReadByte()) << 8 | hexval(ReadByte()) << 4 | hexval(ReadByte()));
				c = tmp289_288;

				this.buffer[this.bufpos] = tmp289_288;
	...
	public void adjustBeginLineColumn(int newLine, int newCol) {
		...
		do {
			...
			if (i >= len)
				break;
			start++;
		} while (this.bufline[(j = start % this.bufsize)] == this.bufline[(k = start % this.bufsize)]);
