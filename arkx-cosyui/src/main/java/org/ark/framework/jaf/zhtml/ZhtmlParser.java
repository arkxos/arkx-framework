...
public class ZhtmlParser {
    ...
    public static final Pattern PInclude = Pattern.compile("\\<\\%\\s*@\\s*include\\s+file\\=\\\"(.*?)\\\".*?\\%\\>",
            34);
    ...
    private void include() {
        ...
        while (m.find(lastIndex)) {
            String current = this.fileName.indexOf("/") > 0
    ...
    public void parse() throws ZhtmlCompileException, NotPrecompileException {
        ...
        for (int i = 0; i < cs.length; i++) {
            ...
            if (scriptStartIndex >= 0) {
                ...
                if ((c != '<') || (i >= cs.length - 1))
                    continue;
                if ((cs[(i + 1)] == '!') && (lowerTemplate.indexOf("<!--", i) == i)
                    ...
                    if (tagStartFlag) {
                        ...
                        if ((cs[(i + 1)] == '/') && (isRegisteredTagEnd(lowerTemplate, i))) {
                            ...
                            if ((!prefix.equalsIgnoreCase(tf.TagPrefix)) || (!tagName.equalsIgnoreCase(tf.TagName))) {
                                Errorx.addError("Error on line " + currentLineNo + ":" + tagEnd + " cann't match <"
    ...
    public void parseTagAttributes(ZhtmlFragment tf, String tagHTML) throws ZhtmlCompileException {
        ...
        if (nameEnd > 0) {
            ...
            if (tag == null) {
                String message = StringFormat.format("Error on line ?:<?:?> not registered.",
                        new Object[]{Integer.valueOf(tf.StartLineNo), prefix, tagName});
            ...
            for (int i = 0; i < cs.length; i++) {
                ...
                if ((c == '=') && (lastStringChar == 0)) {
                    ...
                    if (!tag.hasAttribute(key)) {
                        String message = StringFormat.format("Error on line ?:<?:?> no attribute ?.",
                                new Object[]{Integer.valueOf(tf.StartLineNo), prefix, tagName, key});
                        Errorx.addError(message);
                        throw new ZhtmlCompileException(message);
                    }
                    nameStartIndex = 0;
