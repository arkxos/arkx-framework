...
/**
 * @class org.ark.framework.MainFilter 框架过滤器
 * @author Darkness
...
public class MainFilter implements Filter {
     ...
     * @version V1.0
     */
    public static void onFilterStart(HttpServletRequest request, HttpServletResponse response)
                        ...
                        if (cs != null) {
                            for (int i = 0; i < cs.length; i++) {
                                ...
                                if (cs[i].getName().equals("JSESSIONID")) {
                                    ...
                                    if (u != null) {
                                        flag = false;
                                        io.arkx.framework.cosyui.web.mvc.SessionListener.setSession(session.getId(),
                                                session);
                                        break;
                                    }
                                }
    ...
    public void doFilter(ServletRequest req, ServletResponse rep, FilterChain chain)
            throws IOException, ServletException { // Byte
                                                   // code:
        ...
        Object[] results = ExtendManager.invoke(AfterMainFilterAction.ExtendPointID,
    ...
    public static void onFilterEnd(HttpServletRequest request, HttpServletResponse response)
            throws IOException, ServletException {
        try {
            if ((!"true".equals(request.getParameter("_ARK_NOSESSION")))
                    && (!"true".equals(request.getAttribute("_ARK_NOSESSION")))
        ...
        if (Current.containsVariable("_PageExecuteStartTime")) {
            ...
            if ((cost > 100L) && ("true".equals(Config.getValue("App.Log.PageServerCost")))
            ...
            if ((!"false".equals(Current.getVariable("_AddClientCostScript")))
                ...
                response.getWriter().write("\n<script>var t=" + System.currentTimeMillis()
