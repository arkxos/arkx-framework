...
/**
 * @class org.ark.framework.jaf.Current 当前线程处理器，存储当前线程的UIFacade实例、当前线程的变量集合等
 *
 * @author Darkness
...
public class Current {
     ...
     *
     * 设置之前： __>show current.get(); __>{}
     *
     * Mapx<String, Object> personInfo = new Mapx<String, Object>();
     * params.put("name", "darkness"); params.put("age", "26");
     *
     * Current.setVariable("person", personInfo);
     *
     * 设置之后： __>show current.get(); __>{"person.name": darkness, "person.age": "26"}
     *
     * @param key
     *            键
     * @param value
     ...
     * @param method
     *            准备调用的UIFacade方法，如："com.ark.user.UserUI.initForm"
     * @param types
     *            准备调用的UIFacade方法的参数类型
     ...
     * @version V1.0
     */
    public static Method prepareMethod(HttpServletRequest request, HttpServletResponse response, String method,
            Class<?>[] types) {
        ...
        try {
            ...
            String msg = "Error occurs in WebCurrent.prepareMethod:" + method;
            LogUtil.warn(msg);
            e.printStackTrace();
