    ...
    public void appendScript(DataGridAction dga, FastStringBuilder scriptSB) {
            ...
            for (String key : dga.getParams().keySet()) {
                ...
                if (!key.equals(Constant.TagBody) && v != null) {
                    if (v instanceof DataTable || v instanceof DAOSet) {// 处理后台传递到前台的DataTable
                        ...
                        scriptSB.append("dg.setParam('").append(key).append("',\"")
                                .append(StringUtil.javaEncode(v.toString())).append("\");");
                    } else if (Primitives.isPrimitiveArray(v)) {
                        scriptSB.append("dg.setParam('").append(key).append("',[").append(StringUtil.join(v))
                                .append("]);");
                    } // 其他的对象过滤掉
                }
            }
            scriptSB.append("dg.setParam('").append(Constant.DataGridMultiSelect).append("','")
                    .append(dga.isMultiSelect()).append("');");
            scriptSB.append("dg.setParam('").append(Constant.DataGridAutoFill).append("','").append(dga.isAutoFill())
                    .append("');");
            scriptSB.append("dg.setParam('").append(Constant.DataGridScroll).append("','").append(dga.isScroll())
                    .append("');");
            scriptSB.append("dg.setParam('").append(Constant.Lazy).append("','").append(dga.isLazy()).append("');");
            if (dga.getCacheSize() > 0) {
                scriptSB.append("dg.setParam('").append(Constant.CacheSize).append("','").append(dga.getCacheSize())
                        .append("');");
            }
            scriptSB.append("dg.setParam('").append(Constant.Method).append("','").append(dga.getMethod())
                    .append("');");
            scriptSB.append("dg.setParam('").append(Constant.Page).append("','").append(dga.isPageEnabled())
                    .append("');");
            scriptSB.append("dg.setParam('").append(Constant.TagBody).append("', '").append(dga.getTagBody().getUID())
                    .append("');");
        ...
        if (dt != null) {
            if (dga.isPageEnabled()) {
                scriptSB.append("dg.setParam('").append(Constant.DataGridPageIndex).append("',")
                        .append(dga.getPageIndex()).append(");");
                scriptSB.append("dg.setParam('").append(Constant.DataGridPageTotal).append("'," + dga.getTotal())
                        .append(");");
                scriptSB.append("dg.setParam('").append(Constant.Size).append("',").append(dga.getPageSize())
                        .append(");");
            }
            // 所有下划线开始的列都不输出到JS中
        ...
        if (!dga.isAjaxRequest()) {
            scriptSB.append("Ark.Page.onLoad(function(){DataGrid_").append(id).append("_Init(DataGrid_").append(id)
                    .append("_Update);},9);");
        }
    }
}
