...
public class GridTree extends AbstractGridFeature {
	...
	public void rewriteTD(DataGridAction dga, HtmlTD th, HtmlTD td) {
		...
		if (!StringUtil.isEmpty(field)) {
			// 树的节点的值
			cellSB.append("<input type='checkbox'  name='")
				.append(dga.getID())
				.append("_TreeRowCheck' id='")
				.append(dga.getID())
				.append("_TreeRowCheck_${i}")
				.append("' value='${" + field + "}' ")
				.append(checked)
				.append(" level='${")
				.append(TreeLevelField)
	...
	public static DataTable sortTreeDataTable(DataTable dt, GridTreeParam gtp) {
		...
		if (dt.getDataColumn(gtp.ParentIdentifierColumnName) == null) {
			LogUtil
				.warn("DataGridAction.sortTreeDataTable():Parent column not found:" + gtp.ParentIdentifierColumnName);
		}
		...
		Treex<String, DataRow> tree = Treex.dataTableToTree(dt, gtp.IdentifierColumnName,
				gtp.ParentIdentifierColumnName);
