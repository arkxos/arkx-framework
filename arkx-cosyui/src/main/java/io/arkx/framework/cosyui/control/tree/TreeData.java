...
/**
 * 树形数据重排工具类<br>
 * 注意： 1、如果设置有level属性，bindData(DataTable
 * dt)中的dt会有level+1层数据，这样做是为了让控件能够知道哪些节点有子节点，以便于进行延迟加载。
...
public class TreeData {
    ...
    public DataTable rewrite(TreeAction ta) {
        ...
        Treex<String, DataRow> tree = Treex.dataTableToTree(dt, ta.getIdentifierColumnName(),
                ta.getParentIdentifierColumnName());
     ...
     * 1、计算层级图标 2、计算节点图标 3、处理单选多选 4、将节点HTML包裹上合适的dd/dl
     */
    private void rewriteData(TreeNode<String, DataRow> node) {
        ...
        if (icon.startsWith("icons/")) {
            ...
            if (className.indexOf('/') > 0) {
                ...
                sb.append("' style='background:url(").append(Config.getContextPath()).append(icon)
                        .append(") no-repeat center center;'>");
            } else {
                sb.append(imagePrefix);
        ...
        if (StringUtil.isNotEmpty(checkbox)) {
            ...
            if (radio.equals("all") || radio.equals("true") || radio.equals("branch") && node.hasChildren()
                    || radio.equals("leaf") && !node.hasChildren()) {
                sb.append("<input type='radio' value='").append(dr.getString(_ID)).append("'").append(" name='")
