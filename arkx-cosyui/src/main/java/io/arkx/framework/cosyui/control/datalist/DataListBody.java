...
public class DataListBody {
	...
	void getScript(DataListAction dla, FastStringBuilder sb) {
		...
		if (StringUtil.isNotEmpty(dla.getDragClass())) {
			sb.append("dl.setParam('")
				.append(Constant.DragClass)
				.append("','")
				.append(dla.getDragClass())
				.append("');");
		}
		...
		sb.append("<ark:if condition='${!_DataListAction.AjaxRequest}'>");
		sb.append("Ark.Page.onLoad(function(){DataList_")
			.append(ID)
			.append("_Init(DataList_")
			.append(ID)
			.append("_Update);}, 8);");
		sb.append("</ark:if>");
	}
