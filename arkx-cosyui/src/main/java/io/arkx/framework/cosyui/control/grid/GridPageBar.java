...
public class GridPageBar extends AbstractGridFeature {
    ...
    public void rewriteTR(DataGridAction dga, HtmlTR tr) {
        ...
        if (!DataGridBody.TR_SIMPLEPAGEBAR.equalsIgnoreCase(ztype)
                && !DataGridBody.TR_PAGEBAR.equalsIgnoreCase(ztype)) {
            return;
        }
        // 重置colspan
    ...
    public void beforeDataBind(DataGridAction dga, AbstractExecuteContext context, DataTable dataSource) {
        ...
        String pagebarHtml = getPageBarHtml(dga.getID(), dga.getParams(), dga.getTotal(), dga.getPageIndex(),
                dga.getPageSize(), simple, Integer.parseInt(type));
    ...
    public String getPageBarHtml(String id, Mapx<String, Object> params, int total, int pageIndex, int pageSize,
        ...
        sb.append("<span class='first'>").append("<a href='javascript:void(0);' onclick=\"DataGrid.firstPage('")
                .append(id).append("');\">").append(first).append("</a>").append("</span>");
        sb.append("<span class='previous'>").append("<a href='javascript:void(0);' onclick=\"DataGrid.previousPage('")
                .append(id).append("');\">").append(prev).append("</a>").append("</span>");

        sb.append("<span class='next'>").append("<a href='javascript:void(0);' onclick=\"DataGrid.nextPage('")
                .append(id).append("');\">").append(next).append("</a>").append("</span>");
        sb.append("<span class='last'>").append("<a href='javascript:void(0);' onclick=\"DataGrid.lastPage('")
                .append(id).append("');\">").append(last).append("</a>").append("</span>");
        ...
        if (!simpleFlag) {
            ...
            sb.append("<input type='button' id='_PageBar_JumpBtn_").append(id)
                    ...
                    .append("', Constant.Size)),l=Math.floor(t/s)+(t%s===0?0:1);if(!/^\\d+$/.test(v)")
                    .append("||v<1||v>l){alert('").append(error).append("');document.getElementById('_PageBar_Index_")
                    .append(id).append("').focus();}else{var pageIndex = ($V('_PageBar_Index_").append(id)
                    .append("')-1)>0?$V('_PageBar_Index_").append(id).append("')-1:0;DataGrid.gotoPage('").append(id)
                    .append("',pageIndex);}\" value='' title='").append(gotoButton).append("'>");
        }
        sb.append("</div></div>");
        ...
        sb.append(new StringFormat(pagebar, "<span class=\"js_total\">" + total + "</span>",
                "<span class=\"inline-block pageSizeGroup\">" + "<ul class=\"sizeList\" style=\"position:absolute;\">"
                        + "<li onclick=\"Ark.DataGrid.changePageSize('" + id + "',10)\">10</li>"
                        + "<li onclick=\"Ark.DataGrid.changePageSize('" + id + "',15)\">15</li>"
                        + "<li onclick=\"Ark.DataGrid.changePageSize('" + id + "',20)\">20</li>"
                        + "<li onclick=\"Ark.DataGrid.changePageSize('" + id + "',30)\">30</li>"
                        + "<li onclick=\"Ark.DataGrid.changePageSize('" + id + "',50)\">50</li>" + "</ul>"
                        + "<span class=\"js_pageSize\">" + pageSize + "</span>" + "</span>",
                "<span class=\"js_pageIndex\">" + (totalPages == 0 ? 0 : pageIndex + 1) + "</span>",
