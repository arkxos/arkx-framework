...
public class GridPageBar extends AbstractGridFeature {
	...
	public String getPageBarHtml(String id, Mapx<String, Object> params, int total, int pageIndex, int pageSize,
		...
		if (!simpleFlag) {
			sb.append("&nbsp;&nbsp;")
				.append(gotoPage)
				.append("&nbsp;<input id='_PageBar_Index_")
				.append(id)
				.append("' type='text' class='inputText gotopage' value=\"")
				.append(pageIndex + 1)
				.append("\"");
			sb.append(
			...
			sb.append("<input type='button' id='_PageBar_JumpBtn_")
				.append(id)
				.append("' class='pageJumpBtn' onclick=\"var v=document.getElementById('_PageBar_Index_")
				.append(id)
				.append("').value,t=Number(Ark.DataGrid.getParam('")
				.append(id)
				.append("', Constant.PageTotal)),s=Number(Ark.DataGrid.getParam('")
				.append(id)
				.append("', Constant.Size)),l=Math.floor(t/s)+(t%s===0?0:1);if(!/^\\d+$/.test(v)")
				.append("||v<1||v>l){alert('")
				.append(error)
				.append("');document.getElementById('_PageBar_Index_")
				.append(id)
				.append("').focus();}else{var pageIndex = ($V('_PageBar_Index_")
				.append(id)
				.append("')-1)>0?$V('_PageBar_Index_")
				.append(id)
				.append("')-1:0;DataGrid.gotoPage('")
				.append(id)
				.append("',pageIndex);}\" value='' title='")
				.append(gotoButton)
				.append("'>");
		}
		sb.append("</div></div>");
