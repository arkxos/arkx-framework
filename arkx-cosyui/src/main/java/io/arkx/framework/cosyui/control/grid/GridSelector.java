...
public class GridSelector extends AbstractGridFeature {
    ...
    public void rewriteTD(DataGridAction dga, HtmlTD th, HtmlTD td) {
        ...
        if (dga.isMultiSelect()) {
            th.setInnerHTML("<input type='checkbox' value='*' id='" + id
                    + "_AllCheck' onclick=\"Ark.DataGrid.onAllCheckClick('" + id + "', this)\" autoComplete='off' />");
        }
        String type = dga.isMultiSelect() ? "checkbox" : "radio";
        td.setInnerHTML("<input type='" + type + "' name='" + id + "_RowCheck' id='" + id + "_RowCheck_${i}' value='${"
                + field + "}' onclick=\"return false\" autoComplete=\"off\" />");
