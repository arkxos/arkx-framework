...
public class PageBarTag extends ArkTag {
    ...
    private String getDefault(String target, int type, int total, int pageSize, int pageIndex) {
        ...
        if (pageIndex > 0) {
            if (action instanceof DataListAction || action instanceof DataGridAction) {
                sb.append(
                        "<span class=\"first\"><a href='javascript:void(0);' onclick=\"Ark." + jsClass + ".firstPage('")
                        .append(target).append("');\">").append(first).append("</a></span>");
                sb.append("<span class=\"previous\"><a href='javascript:void(0);' onclick=\"Ark." + jsClass
                        + ".previousPage('").append(target).append("');\">").append(prev).append("</a></span>");
        ...
        if (totalPages != 0 && pageIndex + 1 != totalPages) {
            if (action instanceof DataListAction || action instanceof DataGridAction) {
                sb.append("<span class=\"next\"><a href='javascript:void(0);' onclick=\"Ark." + jsClass + ".nextPage('")
                        .append(target).append("');\">").append(next).append("</a></span>");
                sb.append("<span class=\"last\"><a href='javascript:void(0);' onclick=\"Ark." + jsClass + ".lastPage('")
        ...
        sb.append("<input type='button' id='_PageBar_JumpBtn_").append(target)
                ...
                .append(target).append("').focus();}").append("else{var pageIndex = v>0?v-1:0;");
        if (action instanceof DataListAction || action instanceof DataGridAction) {
        ...
        sb.append(new StringFormat(pagebar, "<span class=\"js_total\">" + total + "</span>",
                "<span class=\"inline-block pageSizeGroup\">" + "<ul class=\"sizeList\" style=\"position:absolute;\">"
                        + "<li onclick=\"Ark.DataList.changePageSize('" + target + "',10)\">10</li>"
                        + "<li onclick=\"Ark.DataList.changePageSize('" + target + "',15)\">15</li>"
                        + "<li onclick=\"Ark.DataList.changePageSize('" + target + "',20)\">20</li>"
                        + "<li onclick=\"Ark.DataList.changePageSize('" + target + "',30)\">30</li>"
                        + "<li onclick=\"Ark.DataList.changePageSize('" + target + "',50)\">50</li>" + "</ul>"
