...
public class PageBarTag extends ArkTag {
	...
	private String getDefault(String target, int type, int total, int pageSize, int pageIndex) {
		...
		sb.append("<input type='button' id='_PageBar_JumpBtn_")
			...
			.append(target)
			.append("').value,t=Number(Ark.DataList.getParam('")
			.append(target)
			.append("', Constant.PageTotal)),s=Number(Ark.DataList.getParam('")
			.append(target)
		...
		if (action instanceof DataListAction || action instanceof DataGridAction) {
			sb.append("Ark." + jsClass + ".setParam('")
				.append(target)
				.append("','")
				.append(Constant.DataGridPageIndex)
				.append("',pageIndex);Ark." + jsClass + ".loadData('")
				.append(target)
				.append("');");
		}
		else {
