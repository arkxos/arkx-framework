...
import io.arkx.framework.core.ExceptionCatcherService;
...
@WebServlet(name = "ArkDispatchServlet", loadOnStartup = 2,
        initParams = {@WebInitParam(name = "noFilterPath", value = "/MqttServlet;/weixin;/front;/ueditor;/druid")// 在服务器初始化的时候会打印，因为调用Init方法
        }, urlPatterns = {"/ajax/invoke", "*.zhtml", "/logout.zaction", "/qrlogin/*", "*.zaction"})
public class DispatchServlet extends HttpServlet {

    private static final String DEFAULT_SERVLET_COMMON = "default";// Tomcat, Jetty,
                                                                   // JBoss, and
                                                                   // GlassFish
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        // String externalConfigFileLocation =
        // classLoader.getResource("logback.xml").getPath();
        // try {
        ...
        try {
            ...
            try {
                if (!"true".equals(request.getParameter(Constant.NoSession))
                        && !"true".equals(request.getAttribute(Constant.NoSession))) {
    ...
    private void handleURL(String url, HttpServletRequest request, HttpServletResponse response)
    ...
    private void forwardWelcome(String url, HttpServletRequest request, HttpServletResponse response)
            throws IOException, ServletException {
        if (!url.endsWith("/")) {
