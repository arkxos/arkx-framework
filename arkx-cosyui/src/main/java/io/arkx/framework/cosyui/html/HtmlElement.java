...
public class HtmlElement extends HtmlElementContainer {
    ...
    public void parseHtml(String html) {
        ...
        if (ele.getTagName().equalsIgnoreCase(getTagName())) {
            ...
            throw new HtmlParseException("HtmlElement.parseHtml() failed,need a tag '" + getTagName() + "',but found a "
                    + ele.getTagName() + "!");
        }
    }
    ...
    void format(FastStringBuilder sb, String prefix) {// 如果prefix为null，则不换行
        ...
        if (innerHTML == null && getChildren().size() == 0
                && ObjectUtil.in(tagName.toLowerCase(), HtmlParser.NO_CHILD_TAGS)) {
            sb.append(" />");
        } else {
            sb.append(">");
            getInnerHTML(sb, prefix == null ? null : prefix + "\t");
            if (prefix != null && children.size() > 0
                    && (children.size() > 1 || children.get(0).getType() == ELEMENT)) {
                sb.append("\n");
                sb.append(prefix);
            }
            sb.append("</");
