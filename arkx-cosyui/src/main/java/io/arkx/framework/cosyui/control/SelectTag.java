...
public class SelectTag extends ArkTag {
	...
	public static void getOptionHtml(FastStringBuilder sb, String text, String value, boolean flag) {
		...
		sb.append("<a href=\"javascript:void(0);\" onclick=\"Ark.Selector.onItemClick(this);\"")
			.append(" onmouseover=\"Ark.Selector.onItemMouseOver(this)\" ")
			.append(flag ? "selected=\"true\"" : "")
			.append(" hidefocus value=\"")
			.append(value)
			.append("\">")
			.append(text)
			.append("</a>");
			...
			for (int i = 0; i < dt.getRowCount(); i++) {
				...
				if (!showValue) {
					sb.append("_data.push(['")
						.append(dt.getString(i, 0))
						.append("','")
						.append(dt.getString(i, 1))
						.append("']);");
				}
				else {
					sb.append("_data.push(['")
	...
	public String rewrite(String content) {
		...
		if (isAttributeSet("listURL")) {
			sb.append(" listurl=\"")
				.append(Config.getContextPath() + getAttributeOriginalValue("listURL"))
				.append("\"");
		}
		...
		sb.append("};");
		sb.append("if(Ark.Page.isReady){")
			.append("Combox_")
			.append(id)
			.append("_Init();}else{Ark.Page.onReady(Combox_")
			.append(id)
			.append("_Init);}");
		sb.append("</script>");
