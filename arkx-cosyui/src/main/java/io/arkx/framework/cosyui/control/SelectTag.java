...
public class SelectTag extends ArkTag {
    ...
    public int doStartTag() throws TemplateRuntimeException {// 准备环境变量
        ...
        if ((StringUtil.isNotEmpty(this.code)) || (StringUtil.isNotEmpty(this.method))
                || (StringUtil.isNotEmpty(this.rest)) || (StringUtil.isNotEmpty(this.options))
    ...
    private void getScript4Data(FastStringBuilder sb) {
        ...
        if (dt != null) {
            for (int i = 0; i < dt.getRowCount(); i++) {
                ...
                } else if (selectedIndex != -1) {// 2013-10-17
                                                 // 如果没有设值但有设selectedIndex，则更新value
                    if (i == selectedIndex) {
                        value = dt.getString(i, 0);
                    }
                }
                if (!showValue) {
                    sb.append("_data.push(['").append(dt.getString(i, 0)).append("','").append(dt.getString(i, 1))
                            .append("']);");
                } else {
                    sb.append("_data.push(['").append(dt.getString(i, 0)).append("','").append(dt.getString(i, 0))
                            .append("-").append(dt.getString(i, 1)).append("']);");
                }
            }
     ...
     */
    public static DataTable getCodeData(String code, String method, String rest, String options,
            AbstractExecuteContext context, Mapx<String, Object> params) {
        DataTable dt = null;
    ...
    public String rewrite(String content) {
        ...
        // 以下为隐藏文本框的属性
        sb.append("<input type=\"text\" ztype=\"select\" id=\"").append(id)
                .append("\" name=\"" + getAttributeOriginalValue("name") + "\"")
                .append(" tabindex=\"-1\" autocomplete=\"off\" class=\"inputText\"");
        if (isAttributeSet("style")) {
            sb.append(" style=\"").append(getAttributeOriginalValue("style"))
                    .append(";position:absolute;z-index:-1;\"");
        } else {
        ...
        if (isAttributeSet("listURL")) {
            sb.append(" listurl=\"").append(Config.getContextPath() + getAttributeOriginalValue("listURL"))
                    .append("\"");
        }
        ...
        if (isAttributeSet("style")) {
            sb.append(" style=\"vertical-align:middle; cursor:default;").append(getAttributeOriginalValue("style"))
                    .append("\"");
        } else {
        ...
        sb.append("<a id=\"").append(id).append(
                "_spinner\" class=\"z-combox-spinner\" style=\"position:relative; left:-17px; margin-right:-15px; cursor:pointer; width:13px; height:15px;vertical-align:middle;\"><b></b></a>");
            ...
            for (HtmlNode node : select.iterator()) {
                if (node.getType() == HtmlNode.ELEMENT) {
                    ...
                    if (ele.getTagName().equalsIgnoreCase("option") || ele.getTagName().equalsIgnoreCase("span")) {
                        ...
                        if (showValue) {
                            sb.append("_data.push(['").append(value).append("','").append(value).append(
                                    StringUtil.isNotEmpty(text) ? "-" + text.replaceAll("(\n|\r)", "").trim() : "")
                                    .append("']);");
                        } else {
                            sb.append("_data.push(['").append(value).append("','")
        ...
        sb.append("};");
        sb.append("if(Ark.Page.isReady){").append("Combox_").append(id).append("_Init();}else{Ark.Page.onReady(Combox_")
                .append(id).append("_Init);}");
        sb.append("</script>");
