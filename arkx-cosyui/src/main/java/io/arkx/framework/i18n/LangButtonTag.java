    ...
    public int doStartTag() {
        try {
            if (LangUtil.getSupportedLanguages().size() > 0) {
                ...
                sb.append("<input type=\"hidden\" value=\"").append(StringUtil.escape(value))
                        .append("\" id=\"" + target + "_I18N\">");
                sb.append("<img src=\"").append(Config.getContextPath()).append(
                        "icons/extra/i18n.gif\" align=\"absmiddle\" style=\"cursor:pointer\" onclick=\"Ark.Lang.onLangButtonClick('")
                        .append(target).append("')\"/>");
                sb.append("<script>Ark.Page.onReady(function(){Ark.Node.setValue(\"" + target + "\",\""
                        + LangUtil.decode(value) + "\");});</script>");
