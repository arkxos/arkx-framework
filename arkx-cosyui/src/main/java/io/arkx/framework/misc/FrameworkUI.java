...
/**
 * @class org.ark.framework.FrameworkUI 框架内置一些UI方法
 * @author Darkness
...
public class FrameworkUI extends UIFacade {
    ...
    public void sendPageCost(String url, String Cost, String ReadyCost) {
        ...
        if (cost > 100) {
            LogUtil.info("ClientCost\tReady=" + ReadyCost + "ms" + (readyCost > 1000 ? "!" : "") + "\tLoad=" + Cost
                    + "ms" + (cost > 3000 ? "!" : "") + "\t" + url);
        }
    }
    ...
    // LogUtil.warn("Framework.callRemoteMethod() error::" + pm.getStatusCode() +
    // ";URL=" + url);
    // } else {
    ...
    // }
    // } catch (Exception e) {
    // LogUtil.warn("Framework.callRemoteMethod() error:" + e.getMessage() + ";URL="
    // + url);
    //
    // i++;
    // }
