...
import io.arkx.framework.security.exception.NoPrivException;
...
public class PrivCheck {
    ...
    public static void check(IMethodLocator m) {
        ...
        if (flag) {
            ...
            check(methodPriv.login(), methodPriv.loginType(), methodPriv.userType(), privVlaue,
                    WebCurrent.getExecuteContext());
        } else {// 必须有@Priv注解，否则禁止访问
            throw new NoPrivException("Method hasn't @Priv annotation");
        }
    }

    public static void check(boolean login, LoginType loginType, String userType, String priv,
        ...
        for (String item : StringUtil.splitEx(priv, Privilege.Or)) {
            ...
            if (item.indexOf(",") > 0) {
                ...
                for (String id : StringUtil.splitEx(ids, ",")) {
                    arr = ArrayUtils.addAll(arr,
                            ExtendManager.invoke(PrivExtendAction.ExtendPointID, new Object[]{pre_item + id}));
                }
