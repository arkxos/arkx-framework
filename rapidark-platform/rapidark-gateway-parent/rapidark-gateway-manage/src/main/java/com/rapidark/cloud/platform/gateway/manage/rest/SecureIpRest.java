...
public class SecureIpRest extends BaseRest {
    ...
    public ResponseResult add(@RequestBody SecureIp secureIp) {
    ...
    public ResponseResult delete(@RequestParam String ip) {
    ...
    public ResponseResult update(@RequestBody SecureIp secureIp) {
        Assert.notNull(secureIp, "未获取到对象");
        Assert.isTrue(StringUtils.isNotBlank(secureIp.getIp()), "IP值不能为空");
    ...
    public ResponseResult findById(@RequestParam String ip) {
        Assert.isTrue(StringUtils.isNotBlank(ip), "IP值不能为空");
        return ResponseResult.ok(secureIpService.findById(ip));
    }
    ...
    public ResponseResult pageList(@RequestBody SecureIpReq secureIpReq){
        ...
        return ResponseResult.ok(secureIpService.pageList(secureIp,currentPage, pageSize));
    }
