...
public class RequestComponentGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        return DataBufferUtils.join(request.getBody())
                ...
                .switchIfEmpty(Mono.just(dataBufferFactory.wrap(EMPTY_BYTES)))
                .flatMap(dataBuffer -> {
                    try {
                        ...
                        exchange.getResponse().getHeaders().add(Constants.RULE_ERROR, Constants.FAILED);
                        return HttpResponseUtils.writeError(exchange.getResponse(), "网关转发客户端【" + clientIp + "】路由请求【" +routeId + "】，执行验证异常：" + e.getTargetException().getMessage());
                    } catch (Exception e) {
                        log.error("网关转发客户端【{}】路由请求【{}】，执行Groovy规则引擎验证异常：", clientIp, routeId, e);
                        exchange.getResponse().getHeaders().add(Constants.RULE_ERROR, Constants.FAILED);
                        return HttpResponseUtils.writeError(exchange.getResponse(), "网关转发客户端【" + clientIp + "】路由请求【" + routeId + "】，执行验证异常：" + e.getMessage());
                    }
                });
    }
    ...
    private ServerHttpRequest replaceRawQuery(ServerHttpRequest request, Map<String, String> queryMap){
        ...
        try {
            URI newUri = UriComponentsBuilder.fromUri(request.getURI()).replaceQuery(query.substring(1)).build(true).toUri();
