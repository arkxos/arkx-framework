...
public class ParameterFilter extends FilterHandler {
    ...
    public void handleRequest(ServerHttpRequest request){
        ...
        if (routeConfig.getFilterAuthorizeName().contains("parameter")){
            log.info("处理网关路由请求{},执行parameter过滤 ", routeConfig.getId());
            String accessParameter = routeConfig.getAccessParameter();
