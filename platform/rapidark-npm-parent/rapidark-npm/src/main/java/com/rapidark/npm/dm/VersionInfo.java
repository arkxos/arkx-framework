...
import com.rapidark.npm.ILogger;
...
public class VersionInfo extends AbstractArtifactInfo implements Comparable<VersionInfo>{
	...
	public Completable downloadTarball(final boolean useCache) {
		return Completable.defer(() ->{
            File folder = JNPMService.instance()
                .getSettings()
                .getDownloadDirectory()
                .resolve(getName()).toFile();
            if(!folder.exists()) {
                folder.mkdirs();
            }
	...
	public File getLocalTarball() {
        String tarballPath = this.getName() + File.separator + getDist().getTarballName();
