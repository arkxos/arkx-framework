...
public class StatementProxyHandler implements InvocationHandler {
	...
	public StatementProxyHandler(Statement realStatement, String sql, MonitorConfigService configService,
	...
	private static final Set<String> MONITOR_TABLES = Set.of("monitor_trace", "monitor_slow_sql",
			"monitor_slow_method");
	...
	private Object handleExecuteMethod(Method method, Object[] args) throws Throwable {
		...
		try {
			...
			if (sqlNode.getDuration() > configService.getSlowThreshold() * 1_000_000) {
				log.warn("Slow SQL detected ({}ms): {}", TimeUnit.NANOSECONDS.toMillis(sqlNode.getDuration()),
						sqlNode.getFullSql());
			}
		}
	}
