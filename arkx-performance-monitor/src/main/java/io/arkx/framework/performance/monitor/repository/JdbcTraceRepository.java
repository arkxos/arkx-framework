...
public class JdbcTraceRepository implements TraceRepository {
	...
	private static final String INSERT_TRACE = "INSERT INTO monitor_trace ("
			+ " trace_id, parent_id, node_type, class_name, method_name, signature," + // 新增
																						// signature
			" raw_sql, sql_parameters, full_sql, depth, start_time, end_time," + // 调整字段顺序
			" duration, success, error_message, request_id, session_id, endpoint" + // 新增
																					// session_id
			") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"; // 18个?
	...
	private static final String INSERT_SLOW_SQL = "INSERT INTO monitor_slow_sql ("
			...
			+ "occurrence_count = occurrence_count + 1, " + "last_occurrence = NOW()";
	...
	// 新增查询SQL
	private static final String SELECT_BY_REQUEST_ID = "SELECT * FROM monitor_trace WHERE request_id = ? ORDER BY start_time ASC";
	...
	public void saveBatch(List<TraceNode> nodes) throws SQLException {
		...
		try (Connection conn = dataSource.getConnection();
				...
				PreparedStatement sqlStmt = config.isCaptureSlowSql() ? conn.prepareStatement(INSERT_SLOW_SQL) : null;
				PreparedStatement methodStmt = config.isCaptureSlowMethods() ? conn.prepareStatement(INSERT_SLOW_METHOD)
						: null) {
	...
	private boolean isSlowSql(TraceNode node) {
		return "SQL".equals(node.getType()) && node.getDuration() > config.getSlowThreshold() * 1_000_000;
	}

	private boolean isSlowMethod(TraceNode node) {
		return "METHOD".equals(node.getType()) && node.getDuration() > config.getSlowThreshold() * 1_000_000;
	}
	...
	private void bindSlowSqlParameters(PreparedStatement stmt, TraceNode node) throws SQLException {
		String sql = StringUtils.isNotBlank(node.getFullSql()) ? node.getFullSql() : node.getRawSql();
	...
	private String truncateSqlTemplate(String sql) {
		if (sql == null)
			return "";
		// 简化实现：删除带引号的内容
		return sql.replaceAll("'[^']*'", "?").replaceAll("\"[^\"]*\"", "?").replaceAll("\\d+", "?");
	}
